...
Source tables are under `/home/jingxin/code/dmg-research/project/benchmark/manuscript/r1/tables`:

- `R1_model_basin_delta_kge.csv`
- `R1_model_performance_summary.csv`
- `R1_ensemble_summary.csv`
- `R1_temporal_AB_model_basin.csv`
- `R1_temporal_AB_model_summary.csv`
- `R1X_model_estimator_tendency.csv`
- `R1X_basin_estimator_tendency.csv`
- `Fig1c_basin_effect_spread.csv`
- `R1X_two_way_decomposition.csv`
- `R1X_temporal_basin_persistence_summary.csv`
- `R1X_basin_difficulty_checks.csv`
- `TableS1A_R1_effect_magnitude_summary.csv`
- `TableS1B_R1_model_summary.csv`
- `TableS1_R1_performance_context.md`

The primary matrix contains 19,116 cells, 36 models, and 531 basins. Model rows use the canonical registry order:

`alpine1, alpine2, australia, collie1, collie2, collie3, flexb, flexi, flexis, gr4j, gsfb, hbv96, hillslope, hymod, ihacres, modhydrolog, mopex1, mopex2, mopex3, mopex4, mopex5, newzealand1, newzealand2, penman, plateau, simhyd, smar, susannah1, susannah2, tank, tcm, topmodel, us1, vic, wetland, xinanjiang`

The estimand is `ΔKGE = KGE_dPL − KGE_IC`; positive values indicate higher dPL KGE. Temporal panel (e) uses A=`1995-10-01..2003-03-31` and B=`2003-04-01..2010-09-30`.
...
- Basin location source: `/home/jingxin/code/dmg-research/data/camels_loc/camels_671_loc.dbf`, following the hydrodiag GIS convention of reading `gage_id`, `lat`, and `lon` and joining by zero-padded `basin_id`.
- Boundary source: `/home/jingxin/code/dmg-research/project/hydrodiag/manuscript/cache/gis/us_states.geojson`; a read-only copy is cached at `/home/jingxin/code/dmg-research/project/benchmark/manuscript/r1/cache/gis/us_states.geojson`.
- Final map metadata: `/home/jingxin/code/dmg-research/project/benchmark/manuscript/r1/cache/Fig1c_map_metadata.json`; the derived table is `/home/jingxin/code/dmg-research/project/benchmark/manuscript/r1/tables/Fig1c_basin_effect_spread.csv`.
- Projection: EPSG:5070 Albers Equal Area, with light CONUS state boundaries and a national outline.
- Matched points: **531/531**; missing coordinates: **0**; duplicate basin joins: **0**; geometry conflicts: **0**.
...
- Selected symmetric color limit: **L=0.18** from Q95(|M_b|)=0.1727; clipped for display: **13 below**, **11 above**, **24 total**.
- Inset summaries: median |M_b|=0.0294; fraction with |M_b| < S_b=0.5254. Existing `P_b^+` remains in the source table support but is not encoded in panel (c).
...
- Revised deliverable: `/home/jingxin/code/dmg-research/project/benchmark/manuscript/r1/figures/Fig1_R1_main_final.png` (one assembled PNG, 600 dpi).
...
- **Supplementary Table S1 is established under `/home/jingxin/code/dmg-research/project/benchmark/manuscript/r1/tables`:**
