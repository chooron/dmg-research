#!/usr/bin/env python3
"""Render the sixth revised R1 Figure 1 as one PNG-only composite.
...
OUT_PATH = FIGURES_DIR / "Fig1_R1_main_revised_v6.png"
BLUE = "#2166AC"
ORANGE = "#D6604D"
DARK = "#243447"
MID = "#7B8794"
...
def draw_panel_b(ax: plt.Axes, data: pd.DataFrame, highlight_positive: np.ndarray) -> None:
    y = np.arange(len(data))
    ...
    for yi, is_positive in zip(y, highlight_positive):
        if bool(is_positive):
            ax.axhspan(yi - 0.5, yi + 0.5, facecolor="#EEF1F3", edgecolor="none", alpha=0.78, zorder=0)
...
def draw_panel_c(fig: plt.Figure, ax: plt.Axes, data: pd.DataFrame, states: gpd.GeoDataFrame, national: gpd.GeoDataFrame, map_metadata: dict) -> None:
    ...
    sm.set_array([])
    cbar_ax = ax.inset_axes([0.775, 0.775, 0.20, 0.065], zorder=4)
    ...
    inset = ax.inset_axes([0.735, 0.045, 0.255, 0.61], zorder=5)
    inset.set_facecolor("white")
    inset.scatter(data["M_b_median_deltaKGE"], data["S_b_MAD_deltaKGE"], s=12, color=SLATE_DARK, edgecolor="white", linewidth=0.25, alpha=0.72)
    xs = np.linspace(-0.50, 0.50, 200)
    inset.plot(xs, np.abs(xs), color=MID, ls=(0, (3, 2)), lw=0.68)
    ...
    inset.set_title("Effect vs spread", loc="left", fontsize=8.7, pad=2)
    inset.tick_params(labelsize=7.7, length=2, pad=1)
...
def load_tables() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, dict]:
    ...
    if not required_c.issubset(c.columns) or not np.isfinite(c[["M_b_median_deltaKGE", "S_b_MAD_deltaKGE", "P_b_positive", "latitude", "longitude"]].to_numpy(float)).all():
        raise ValueError("Figure 1 v6 GIS table is incomplete or non-finite")
    if int(map_metadata["n_basins"]) != 531 or int(map_metadata["n_models"]) != 36:
        raise ValueError("Figure 1 v6 map metadata has invalid coverage")
...
def main() -> None:
    ...
    draw_panel_a(ax_a, a)
    highlight_positive = (a["dPL_median_KGE"] > a["IC_median_KGE"]).to_numpy()
    draw_panel_b(ax_b, b, highlight_positive)
