...
The 36 reconstructed differentiable models underwent a four-tier verification protocol:

1. **Water Balance Closure**: Mass conservation was evaluated across 12 synthetic and historical climate regimes under double-precision (`float64`) and single-precision (`float32`) arithmetic. Across the 36 models, 35 achieve strict water balance closure with maximum absolute daily storage residuals below $1.0 \times 10^{-3}\text{ mm/day}$ (typical double-precision residuals $\sim 10^{-11}\text{ mm/day}$). One model (`vic`) exhibits a minor boundary clipping residual of $0.274\text{ mm/day}$ under extreme synthetic stress tests due to hard store boundaries.
...
4. **Time-Step and Euler Convergence**: Substep convergence was tested across Euler discretization levels ($K = 1, 2, 4, 8$ substeps per day). Twenty-one models exhibit nominal first-order convergence rates ($p \in [0.85, 1.15]$); 5 models recover first-order scaling at finer substeps; and 18 models display threshold-dominated non-smooth convergence behavior characteristic of conceptual bucket thresholding.

Table S1 summarizes the verification status and architecture of all 36 models.

### S1.3 Status of Matched Reference Cross-Validation
`[[UNRESOLVED: External MARRMoT cross-validation unexecuted on matched CAMELS-531 setup]]`

*Forensic Audit Note*: A paired hydrograph cross-validation against external MARRMoT implementations (MATLAB MARRMoT v1.x or the Python port `pymarrmot`) was evaluated but determined not to be supportable under genuinely matched conditions. Pymarrmot contains known implementation defects (unit hydrograph convolution is stubbed to identity routing, causing fundamental hydrograph timing mismatches). Direct parameter injection from MATLAB MARRMoT calibrations into differentiable models results in systematic drift driven by solver differences (adaptive continuous ODE solvers in MATLAB vs. discrete-time Euler formulations with smooth sigmoidal transitions in autograd-compatible architectures). Because no paired CAMELS-531 simulation dataset exists with identical parameters, forcing, and solver contracts, an external reference comparison figure cannot be constructed without introducing uncontrolled confounding. As confirmed in the repository, Manuscript Figure S1 presents the complete $36 \times 531$ empirical matrix of raw performance differences ($\Delta\text{KGE} = \text{KGE}_{\text{dPL}} - \text{KGE}_{\text{IC}}$).
