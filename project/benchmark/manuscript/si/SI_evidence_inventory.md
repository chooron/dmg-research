# Supplementary Information (SI) Evidence Inventory
...
**Audit Scope**: Multi-agent repository audit covering `dmotpy` and `project/benchmark`.

---

## 1. Inventory Summary

| Section | Total Claims Audited | VERIFIED | CONFLICT | UNRESOLVED |
| :--- | :---: | :---: | :---: | :---: |
| **S1. Differentiable Model Reconstruction & Verification** | 18 | 17 | 0 | 1 |
| **S2. Data & Parameter-Estimation Configuration** | 22 | 22 | 0 | 0 |
| **S3. Parameter-Space, Information-Organization & OOB** | 28 | 28 | 0 | 0 |
| **Total** | **68** | **67** | **0** | **1** |

*Note on Unresolved Item*: External MARRMoT/pymarrmot numerical equivalence cross-validation (for a hypothetical Figure S1) is marked `UNRESOLVED` because external reference implementations have never been evaluated on genuinely matched configurations (pymarrmot has stubbed unit hydrographs and ODE solver differences preclude direct parameter transfer). Manuscript Figure S1 is confirmed as the raw $\Delta\text{KGE}$ model-basin matrix (`R1_model_basin_delta_kge.csv`).

---

## 2. Section S1 Evidence Ledger: Reconstruction & Numerical Verification

| Item / Claim | Value / Definition | Authoritative Repository Evidence Path | Code Identifier / Lines | Status |
| :--- | :--- | :--- | :--- | :---: |
| **36 Model Names** | Complete 36 canonical models | `dmotpy/models/registry.py` | `STFN_INFO.keys()` (lines 1–15) | **VERIFIED** |
| **MARRMoT Reference IDs** | Canonical MARRMoT IDs (`m_01_collie1_1p_1s` to `m_40_smar_8p_6s`) | `dmotpy/tests/core_model_registry.py` | `REFERENCE_ID_MAP` (lines 20–60) | **VERIFIED** |
| **Calibrated Parameters Total** | Exactly **271** parameters across 36 models | `dmotpy/models/registry.py` | `NPARAM_INFO` (sum = 271) | **VERIFIED** |
...
| **Mass-Conserving Topology** | $\mathbf{S}_{t+1} = \mathbf{S}_t + \Delta t \sum \mathbf{F}_{\text{in}} - \Delta t \sum \mathbf{F}_{\text{out}}$ | `dmotpy/models/core/*.py`, `dmotpy/models/flux/*.py` | Model step methods | **VERIFIED** |
| **Smooth Gating Operators** | Sigmoidal soft gates $\sigma\left(\frac{k}{\tau} (S - S_{\text{thresh}})\right)$ | `dmotpy/models/flux/smooth.py` | `soft_gate_storage_above`, `soft_gate_temperature_below` (lines 5–46) | **VERIFIED** |
| **Smooth Bounding Operators** | `smooth_relu`, `smooth_min`, `smooth_cap_flux` | `dmotpy/models/flux/smooth.py` | Lines 51–86 | **VERIFIED** |
| **Safe Division Epsilon** | Offset `nearzero = 1e-6` | `dmotpy/models/flux/smooth.py`, `dmotpy/models/core/modhydrolog.py` | Line 9 | **VERIFIED** |
| **Unit Hydrograph Routing** | 9 differentiable convolution kernels (`uh_0` to `uh_8`) | `dmotpy/models/unithydro/` | Kernels 0..8; `EndpointUHModel`, `IntermediateUHModel` | **VERIFIED** |
| **Water Balance Closure** | 35/36 models pass strict closure (residual $< 10^{-3}$ mm/d, typical $\sim 10^{-11}$) | `dmotpy/tests/test_core_water_balance.py`, `dmotpy/tests/core_water_balance_utils.py` | Lines 1–85 | **VERIFIED** |
| **VIC Water Balance Warning** | Minor residual ($0.274$ mm/d) under extreme synthetic test | `dmotpy/models/core/vic.py`, `dmotpy/tests/test_core_water_balance.py` | Hard clipping boundaries | **VERIFIED** |
| **Forward Stability** | 100% (36/36) models produce finite outputs (no `NaN`/`Inf`) | `dmotpy/tests/test_training_regression_smoke.py` | Lines 1–45 | **VERIFIED** |
| **End-to-End Autograd** | 36/36 models compute valid non-zero loss gradients | `dmotpy/tests/test_model_gradient_end_to_end.py` | Lines 1–110 (36/36 passed) | **VERIFIED** |
| **FP64 Gradcheck** | 13 representative models pass finite-difference gradcheck | `dmotpy/tests/test_model_gradcheck_representative.py` | Lines 1–95 (13/13 passed) | **VERIFIED** |
| **Gradient Sparsity Audit** | 7 models exhibit parameter subsets with zero gradients under nominal dry forcing | `dmotpy/scripts/standalone_validation_36.py` | `alpine2`, `gr4j`, `hbv96`, `modhydrolog`, `newzealand2`, `plateau`, `smar` | **VERIFIED** |
| **Euler Time-Step Convergence** | 21 models nominal $p \in [0.85, 1.15]$, 5 recover at finer substeps, 18 threshold-dominated | `dmotpy/scripts/standalone_validation_36.py`, `dmotpy/tests/test_euler_substep_convergence.py` | Standalone script lines 1–250 | **VERIFIED** |
| **Matched Reference Cross-Validation** | External MARRMoT/pymarrmot hydrograph comparison | `dmotpy/tests/test_dmot_calibrated_accuracy_audit.py`, `dmotpy/tests/test_tost_uh_inclusive_results_artifacts.py` | Pymarrmot UH stubbed; ODE vs Euler drift; TOST aborted | **UNRESOLVED** |

---

## 3. Section S2 Evidence Ledger: Data & Training Configuration

| Item / Claim | Value / Setting | Authoritative Repository Evidence Path | Code Identifier / Lines | Status |
| :--- | :--- | :--- | :--- | :---: |
...
| **Calibration / Training Period** | `1980-10-01` to `1995-09-30` (5,478 days, 15 years) | `project/benchmark/configs/full_run_10starts_300gen_dpl_aligned_1980_1995.yaml`, `dmotpy/data_contract.py` | YAML line 31, `data_contract.py:89` | **VERIFIED** |
| **Evaluation / Test Period** | `1995-10-01` to `2010-09-30` (5,479 days, 15 years) | `project/benchmark/configs/full_run_10starts_300gen_dpl_aligned_1980_1995.yaml`, `dmotpy/data_contract.py` | YAML line 33, `data_contract.py:91` | **VERIFIED** |
...
| **DOY Calendar Forcing** | Day of year channel for `mopex4`, `mopex5`, `vic` | `dmotpy/data_contract.py`, `project/benchmark/scripts/canonical_v2/run_canonical_v2_model.py` | `CALENDAR_MODELS` (lines 14, 30–50), script lines 218–223 | **VERIFIED** |
...
| **IC Initialization & Transform** | LHS in logit latent space, $\sigma_0 = 0.10$, sigmoid mapping | `project/benchmark/src/batched_cmaes.py`, `project/benchmark/src/parameter_transform.py` | `batched_cmaes.py:16-22`, `parameter_transform.py:16-24` | **VERIFIED** |
...
| **Parameter Mapping Census** | Exactly **53 log-mapped** (span ratio $\ge 100$) and **218 linear-mapped** out of 271 | `dmotpy/models/hydrology_model.py`, `dmotpy/models/registry.py` | `_should_use_log_mapping` (lines 160–230) | **VERIFIED** |

---

## 4. Section S3 Evidence Ledger: Parameter-Space, Information-Organization & OOB
...
| **Contraction Ratio ($CR$) Sensitivity** | $CR = \text{IQR}(\text{dPL}) / \text{IQR}(\text{IC})$; canonical $0.6140$ vs consensus $0.9795$ vs self-ref $1.0043$ | `project/benchmark/manuscript/r2/scripts/04_distribution_reference_sensitivity.py`, `project/benchmark/manuscript/r2/tables/F2_CR_REFERENCE_DEFINITIONS.md` | Lines 12–42, report lines 9–48 | **VERIFIED** |
