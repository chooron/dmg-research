"""
Fig. S6d — Parameter-std (seed spread) spatial maps for delta_dist

Mirrors figS6c (delta_mcd std) exactly: same layout (4×4), same rocket
colormap, same PARAMETER_STD_BOUNDS, same draw_map_panel logic.
Only the model changes: distributional instead of mc_dropout.
"""
...
def main() -> None:
    ...
    stds_dist = load_parameter_stds(basin_ids, "distributional")
    pts_dist  = basins.merge(stds_dist, on="basin_id", how="inner")
    make_figure("figS6d_spatial_maps_dist_std", pts_dist, conus, extent,
                cmap_std, PARAMETER_STD_BOUNDS, "(4)")
