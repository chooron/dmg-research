...
**Date:** 2026-08-23  
**Status:** **PASS WITH QUALIFICATION (GO for Discussion Writing)**  
**Target Scope:** Results 3.1–3.5 (R1–R5) numerical consistency, denominator stability, conditional associations, timing endpoints, cross-host coherence, and Discussion 4.1–4.4 claim strength alignment.
...
```
========================================================================================
Audit Section     Target Scope               Verdict                   Discussion Status
========================================================================================
Part A            R3 Gap Recovery Ratios     PASS WITH QUALIFICATION   GO (Sec 4.2)
Part B            R3 Conditional Assoc       PASS                      GO (Sec 4.3)
Part C            R1 S5 Endpoint Timing      PASS                      GO (Sec 4.1)
Part D            R4 Figure 8 TGD Timing     PASS                      GO (Sec 4.3)
Part E            R5 Cross-Host Coherence    PASS (Estimand Aligned)   GO (Sec 4.1, 4.4)
Part F            Provenance & Wording       PASS                      GO (Sec 4.1–4.4)
========================================================================================
```

### Key Qualifications for Discussion Writing:
1. **Raw Gains as Primary, Ratios as Secondary:** Under R3 known-truth experiments, parameter refitting yields a small raw KGE gain ($G_{\mathrm{Base}} \approx +0.003$ IC / $+0.007$ dPL), while generic temperature-conditioned storage mitigation yields $G_{\mathrm{TGD}} \approx +0.039$ IC / $+0.036$ dPL. Normalized recovery fractions ($F_{\mathrm{close}} \approx 0.10$, $F_{\mathrm{TGD}} \approx 0.52\text{--}0.55$) are robust secondary summaries on denominator-valid catchments ($D_b > 10^{-6}$), but should not be reported in isolation from raw gains.
2. **No Bound or Process Attribution Language:** The $\Delta F = F_{\mathrm{TGD}} - F_{\mathrm{close}} \approx +0.44\text{--}0.46$ contrast demonstrates that temperature-conditioned generic storage absorbs substantially more deficit than 15-parameter readjustment alone. It must **not** be framed as an upper/lower bound or an isolated attribution of snow physics.
...
### A1. Canonical Definitions & Data Sources
...
- **Normalized TGD Recovery Fraction:**  
  ...
  *(Corresponds to `F_TGD_star` in pipeline code, replacing legacy incremental `F_tgd2`).*
...
### A2. Full & Stratum-Level Statistical Summary (Test Period)
...
We evaluated the sensitivity of the ratio medians across threshold cutoffs from $10^{-6}$ to $0.10$:
...
1. **Zero near-zero positive density:** In both IC and dPL test sets, exactly 0 catchments fall in $(0, 10^{-6}]$ or $(10^{-6}, 10^{-4}]$. $D_b \le 0$ occurs strictly in low-snow S1/S2 catchments where snow removal has no physical effect on runoff (fitting noise around zero).
2. **Extreme Ratio Stability:** Across the entire threshold span from $10^{-6}$ to $0.10$, $F_{\mathrm{close}}$ stays within $[0.10, 0.14]$, $F_{\mathrm{TGD}}$ stays within $[0.52, 0.58]$, and the paired structural advantage $\Delta F = F_{\mathrm{TGD}} - F_{\mathrm{close}}$ remains virtually invariant at $+0.44\text{--}+0.46$ ($>92\%$ positive across all basins).
3. **Verdict:** `PASS WITH QUALIFICATION`. The ~0.10 vs 0.52–0.55 order of magnitude is exceptionally robust. Raw gains (+0.003 vs +0.039) must remain the primary reported evidence; ratios serve as normalized secondary descriptors.

---

## 3. Part B — R3 Outlet Recovery vs Internal Recovery Conditional Association

Audit of rank associations between outlet recovery ($G_{\mathrm{Base}}, G_{\mathrm{TGD}}$) and internal error metrics (15-parameter excess error $E^{\mathrm{param,excess}}$ and total tension water state excess $\Delta E(W_t)$):
...
Within individual strata S1–S5, raw rank correlations fluctuate around zero with no consistent positive sign (e.g. IC $G_{\mathrm{Base}}$ vs $E^{\mathrm{param,excess}}$ by stratum: S1: -0.02, S2: +0.02, S3: +0.18, S4: +0.28, S5: -0.07; IC $G_{\mathrm{Base}}$ vs $\Delta E(W_t)$: S1: -0.05, S2: -0.05, S3: +0.16, S4: +0.26, S5: -0.34).

### Final Wording Guidance:
...
1. **Population Eligibility:** Primary S1–S5 analysis is strictly based on the **full paired 531-catchment population** (S1=165, S2=156, S3=121, S4=34, S5=55), without KGE threshold screens.
2. **Canonical CT Definition:** Mass centroid of annual hydrograph within water years.
3. **Sign Convention:** $\Delta |\mathrm{CT}|^{\mathrm{Base}-\mathrm{CN}} = |\mathrm{CT}_{\mathrm{Base}}| - |\mathrm{CT}_{\mathrm{CN}}|$ (positive = CN error reduction).
4. **Endpoint Numbers:**
   - **S1 Median Reduction:** $+0.07$ d [$-0.13, +0.33$] (IC) / $-0.13$ d [$-0.33, +0.07$] (dPL) $\approx \mathbf{0\text{ d}}$.
   - **S5 Median Reduction:** $+47.47$ d [$+41.60, +51.40$] (IC) / $+46.13$ d [$+39.00, +48.00$] (dPL) $\approx \mathbf{47\text{ d}\text{ (IC) }/\text{ }46\text{ d}\text{ (dPL)}}$.
...
5. **Verdict:** `PASS`. All numbers match the draft text, tables, and Figure 2 exactly.
...
1. **Representative vs Population:** Figure 8 panel (a) representative case is strictly Basin `09306242` (WY 2004, annual SWE peak 388 mm, external SWE burden 359 mm), selected by deterministic distance minimization. It is never conflated with population medians.
...
### Reconciliation of Competing Estimands:
Historically, two estimands existed in R5 scripts:
1. **Legacy Estimand:** $P(\mathrm{KGE}_{\mathrm{CN}} - \mathrm{KGE}_{\mathrm{TGD}} > 0)$ across 3 hosts (`r5_cross_model_agreement_table.csv`).
2. **Canonical Estimand in Figure 9e:** $P(\Delta |\mathrm{CT}|^{\mathrm{Base}-\mathrm{CN}} > 0)$ across 3 hosts (XAJ, GR4J, SIMHYD) (`r5_figure9_primary_agreement.csv`).

The canonical timing estimand is strictly preferred because Section 3.5 evaluates whether the **primary output timing diagnostic replicates across contrasting host structures**.
...
In Section 3.5 text of `hess_results_R1_R5_reframed_v2.md` (lines 120–130), replace the legacy KGE numbers with the canonical timing numbers above.
