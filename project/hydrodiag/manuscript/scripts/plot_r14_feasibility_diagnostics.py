...
DEFAULT_R14_DIR = Path("/home/jingxin/orca/workspaces/dmg-research/flex-mopex/project/flexmopex/results/feasibility_r14")
...
def generate_figure_r14(r14_dir: Path, out_dir: Path) -> Tuple[Path, Path]:
    ...
    p1_r8_int = p1_df[(p1_df["run_tag"] == "R8_AICDelay") & (p1_df["epoch"] == 2) & (p1_df["process"] == "w_int")]
    y_true = p1_r8_int["oracle_pos"].values.astype(int)
    score_dj = p1_r8_int["delta_J"].values
    score_g = -p1_r8_int["g_fit_local"].values  # negative derivative = positive pull towards ON
    ...
    # Plot ROC curves
    ax_a.plot(fpr_dj, tpr_dj, color="#0077BB", linewidth=1.8, label=f"$\\Delta J$ (Counterfactual, AUC={roc_auc_dj:.3f})")
    ax_a.plot(fpr_g, tpr_g, color="#EE7733", linewidth=1.4, linestyle="--", label=f"$g_{{\\mathrm{{fit}}}}$ (Local Gradient, AUC={roc_auc_g:.3f})")
    ax_a.plot([0, 1], [0, 1], color="#999999", linestyle=":", linewidth=0.9, label="Random Guess (AUC=0.500)")

    # Annotation box for validation metrics
    n_pos = int(y_true.sum())
    n_tot = len(y_true)
    n_on_worse = int((p1_r8_int["oracle_pos"] & (p1_r8_int["fit_imp_endpoint"] < 0)).sum())
    ax_a.text(
        0.42, 0.16,
        ...
        bbox=dict(boxstyle="round,pad=0.35", facecolor="#F7F9FB", edgecolor="#B0C4DE", linewidth=0.7),
    )
    ...
    ax_a.set_title("(a) Structural Target vs Local Gradient (Oracle Agreement)", loc="left", fontweight="bold", fontsize=8.2)
    ax_a.legend(loc="lower right", frameon=True, facecolor="#FFFFFF", framealpha=0.9, fontsize=6.8)
    ...
    b1 = ax_b.bar(x_pos - bar_w, spearman_vals, bar_w, label="Rank Corr (Spearman $r$)", color="#0077BB", alpha=0.88)
    b2 = ax_b.bar(x_pos, jaccard_vals, bar_w, label="Jaccard Overlap", color="#009988", alpha=0.88)
    b3 = ax_b.bar(x_pos + bar_w, flip_vals, bar_w, label="Sign-Flip Rate", color="#EE7733", alpha=0.88)

    # Value labels on top of bars
    for bar in b1:
        h = bar.get_height()
        ax_b.text(bar.get_x() + bar.get_width() / 2.0, h + 0.015, f"{h:.2f}", ha="center", va="bottom", fontsize=6.2)
    for bar in b2:
        h = bar.get_height()
        ax_b.text(bar.get_x() + bar.get_width() / 2.0, h + 0.015, f"{h:.2f}", ha="center", va="bottom", fontsize=6.2)
    for bar in b3:
    ...
    ax_b.set_xticklabels(["Phenology\n($w_{\\mathrm{phen}}$)", "Interception\n($w_{\\mathrm{int}}$)", "Snowmelt\n($w_{\\mathrm{snow}}$)", "Baseflow\n($w_{\\mathrm{sub}}$)"], fontsize=7.2)
    ax_b.set_ylabel("Metric Value [0 – 1]")
    ax_b.set_ylim(0.0, 1.15)
    ax_b.set_title("(b) Temporal Stability of $\\Delta J$ Across R8 Trajectory", loc="left", fontweight="bold", fontsize=8.2)
    ax_b.legend(loc="upper right", frameon=True, facecolor="#FFFFFF", framealpha=0.9, fontsize=6.5, ncol=1)
    ...
    width_c = 0.34
    ax_c.bar(x_pos - width_c / 2, raw_auc, width_c, label="Raw Attributes $X$ (35-D)", color="#888888", alpha=0.85)
    ax_c.bar(x_pos + width_c / 2, h_auc, width_c, label="Learned Representation $h$ (128-D)", color="#0077BB", alpha=0.88)

    # Annotate deltas
    for i, p in enumerate(procs):
        delta = h_auc[i] - raw_auc[i]
        ax_c.text(
            x_pos[i] + width_c / 2, h_auc[i] + 0.015,
            f"+{delta:.2f}" if delta >= 0 else f"{delta:.2f}",
            ha="center", va="bottom", fontsize=6.5, fontweight="bold", color="#0077BB" if delta >= 0 else "#EE7733",
        )
    ...
    ax_c.text(
        ...
        fontsize=6.8,
        verticalalignment="bottom",
        bbox=dict(boxstyle="round,pad=0.35", facecolor="#F0F8FF", edgecolor="#4682B4", linewidth=0.7),
    )

    ax_c.set_xticks(x_pos)
    ax_c.set_xticklabels(["Phenology\n($w_{\\mathrm{phen}}$)", "Interception\n($w_{\\mathrm{int}}$)", "Snowmelt\n($w_{\\mathrm{snow}}$)", "Baseflow\n($w_{\\mathrm{sub}}$)"], fontsize=7.2)
    ax_c.set_ylabel("Out-of-Fold ROC-AUC for $\\Delta J > 0$")
    ax_c.set_ylim(0.5, 1.05)
    ax_c.set_title("(c) Target Regionalizability & Parameter State Robustness", loc="left", fontweight="bold", fontsize=8.2)
    ax_c.legend(loc="lower right", frameon=True, facecolor="#FFFFFF", framealpha=0.9, fontsize=6.8)
    ...
    p5_r8_int = p5_df[(p5_df["checkpoint"] == "R8_AICDelay_ep2") & (p5_df["process"] == "w_int")].iloc[0]

    cand_names = ["Cand A\n(Binary)", "Cand B\n(Margin-Aware)", "Cand C\n(Logistic Soft)"]
    cand_roc = [p5_r8_int["A_oracle_roc_auc"], p5_r8_int["B_oracle_roc_auc"], p5_r8_int["C_oracle_roc_auc"]]
    cand_pr = [p5_r8_int["A_oracle_pr_auc"], p5_r8_int["B_oracle_pr_auc"], p5_r8_int["C_oracle_pr_auc"]]
    cand_ent = [p5_r8_int["A_entropy"], p5_r8_int["B_entropy"], p5_r8_int["C_entropy"]]

    x_cand = np.arange(len(cand_names))
    width_d = 0.28

    ax_d.bar(x_cand - width_d / 2, cand_roc, width_d, label="Oracle ROC-AUC", color="#0077BB", alpha=0.88)
    ax_d.bar(x_cand + width_d / 2, cand_pr, width_d, label="Oracle PR-AUC", color="#009988", alpha=0.88)

    for i in range(len(cand_names)):
        ax_d.text(x_cand[i] - width_d / 2, cand_roc[i] + 0.012, f"{cand_roc[i]:.3f}", ha="center", va="bottom", fontsize=6.2)
        ax_d.text(x_cand[i] + width_d / 2, cand_pr[i] + 0.012, f"{cand_pr[i]:.3f}", ha="center", va="bottom", fontsize=6.2)
    ...
    ovh_epoch = p6_cost["cost_strategies"]["every_epoch_epoch_start"]["epoch_overhead_pct"]
    ...
    ax_d.text(
        ...
        fontsize=6.8,
        verticalalignment="bottom",
        bbox=dict(boxstyle="round,pad=0.35", facecolor="#FFF9E6", edgecolor="#DAA520", linewidth=0.7),
    )
    ...
    ax_d.set_title("(d) Soft-Target Quality & Real Training Feasibility", loc="left", fontweight="bold", fontsize=8.2)
    ax_d.legend(loc="lower right", frameon=True, facecolor="#FFFFFF", framealpha=0.9, fontsize=6.8)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--r14-dir", type=Path, default=DEFAULT_R14_DIR, help="Path to R14 diagnostic results")
    parser.add_argument("--out-dir", type=Path, default=FIGURES_DIR, help="Path to output figures directory")
