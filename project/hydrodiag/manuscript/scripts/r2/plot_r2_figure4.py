...
Output: high-resolution PNG (600 DPI) only, saved to manuscript/figures/;
...
SUPP_FIG_DIR = MANUSCRIPT / "figures"
...
KEY_ROW_SHADE = (
    "#FEF8F1"  # very light warm shading for highlighted-parameter rows in (a)
)
...
GEO_BASIN_SHP = (
    ...
    / "camels_loc"
    / "conus_clipped"
    / "camels_671_loc_conus_clipped.shp"
)
GEO_STATES_SHP = (
    PROJECT.parents[1] / "data" / "camels_loc" / "conus_clipped" / "s_18mr25_conus.shp"
)
GEO_CRS = "EPSG:5070"
...
def panel_a_forest(ax, df_g, paradigm, shared_lim=None):
    ...
    ax.set_xlabel("Snow gradient of paired shift, \u03b2", labelpad=3, fontsize=11.5)
...
def _dz_ridge_pair(ax, df_p, parameter, xlabel=False):
    ...
    for i, reg in enumerate(REGIMES):
        ...
        for paradigm, sign, alpha, fill, edge, ls in [
            ...
            vals = df_p[
                ...
                & (df_p["snow_regime"] == reg)
            ]["delta_base_minus_cn"].to_numpy()
    ...
    if xlabel:
        ax.set_xlabel(
            r"$\Delta z = z_{\mathrm{Base}} - z_{\mathrm{CN}}$",
            labelpad=3,
            fontsize=11.5,
        )
...
def _draw_map(ax, basins, states, delta, title):
    states.plot(
        ax=ax, facecolor="#F6F6F3", edgecolor="#D5D5D0", linewidth=0.25, zorder=1
    )
    ax.scatter(
        basins["x"],
        basins["y"],
...
def build_figure(df_g, df_c, df_p) -> None:
    ...
    gsL = gridspec.GridSpec(
        2, 1, left=c1[0], right=c1[1], top=TOP, bottom=BOT, hspace=0.12
    )
    gsM = gridspec.GridSpec(
        3, 1, left=c2[0], right=c2[1], top=TOP, bottom=BOT, hspace=0.22
    )
    ...
    cilo = min(
        slope_ci_param(df_g, p_, par)[1] for p_ in ["IC", "dPL"] for par in PARAM_ORDER
    )
    cihi = max(
        slope_ci_param(df_g, p_, par)[2] for p_ in ["IC", "dPL"] for par in PARAM_ORDER
    )
    pad = max(0.05, (cihi - cilo) * 0.08)
    lo_r = np.floor((cilo - pad) / 0.5) * 0.5
    hi_r = np.ceil((cihi + pad) / 0.5) * 0.5
    ...
    # Output: high-resolution PNG only, saved to manuscript/figures/
...
def build_ic_maps_supplement(df_p) -> None:
    ...
    gs = gridspec.GridSpec(
        1, 3, wspace=0.10, left=0.02, right=0.92, top=0.92, bottom=0.08
    )
    map_axes = [fig.add_subplot(gs[0, i]) for i in range(3)]
    panel_e_maps(map_axes, df_p, "IC")
    mappable = ScalarMappable(norm=Normalize(-MAP_VLIM, MAP_VLIM), cmap=MAP_CMAP)
    cbar = fig.colorbar(
        mappable, ax=map_axes, orientation="vertical", fraction=0.04, pad=0.01
    )
...
def main() -> None:
    ...
    assert (
        df_p[(df_p["paradigm"] == "dPL") & (df_p["parameter"].isin(KEY_PARAMS))]
        .groupby("basin_id")
        .ngroups
    ) == 531
    ...
    print(
        "Figure 4 final (Base-CN only: snow-gradient overview + ridgelines + GIS) generated."
    )
