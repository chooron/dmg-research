...
def _plot_panel(ax: plt.Axes, frame: pd.DataFrame, value_column: str, x_label: str, title: str, unit_scale: float = 1.0) -> None:
    ...
    for paradigm, color, marker in (("IC", COLOR_CN, "o"), ("dPL", COLOR_BASE, "^")):
        ...
        ax.scatter(sub[value_column].to_numpy(dtype=float) * unit_scale, y, color=color, marker=marker, s=29, zorder=3, label=f"{paradigm} (omit HUC)")
    ...
    ax.set_xlim(left=min(0.0, float(rows[value_column].min()) * unit_scale * 0.98))
...
def build_figure(out_path: Path) -> Path:
    ...
    axes[0].set_xlim(40, 49)
    axes[0].set_xticks(np.arange(40, 50, 2))
    axes[1].set_xlim(-0.02, 0.52)
    axes[2].set_xlim(40, 100)
    axes[2].axvline(50, color=COLOR_ZERO_LINE, linewidth=0.9, zorder=1)
    handles = [
    ...
    chance_handle = Line2D([0], [0], color=COLOR_ZERO_LINE, lw=0.9, label="Chance reference (50%)")
    axes[2].legend(handles=[chance_handle, handles[2], handles[3], handles[0], handles[1]], loc="lower left", frameon=False, fontsize=7.0)
