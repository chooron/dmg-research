...
DEFAULT_OUT = MANUSCRIPT / "supplement" / "figures" / "FigureS4_tgd_response_shape_sensitivity.png"
...
def build_figure(out_path: Path) -> Path:
    ...
    ax_tau.set_title(r"(a) Residence time $\tau(T)$", loc="left", weight="bold", pad=6, fontsize=9.0)
    ...
    ax_ret.set_title(r"(b) Daily retention $r(T)$", loc="left", weight="bold", pad=6, fontsize=9.0)
    ...
    ax_ret.text(0.03, 0.04, "$r(T)=\u2009\exp[-1/\tau(T)]$", transform=ax_ret.transAxes, fontsize=6.6, color="#555555")
