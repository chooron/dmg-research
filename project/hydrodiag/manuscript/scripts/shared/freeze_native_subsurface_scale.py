...
def select_existing_native_parameters(
    ...
    for path in sorted(Path(raw_directory).glob("*.json")):
        ...
        if (
            basin_id not in best
            or score > best[basin_id][0]
            or (
    ...
    if parameters.shape != (len(basin_ids), len(XAJ_PARAM_SPECS)):
        raise RuntimeError(
            f"Unexpected selected native parameter shape {parameters.shape}"
        )
    metadata = {
    ...
    *,
    ...
    with torch.no_grad():
        for timestep in range(nsteps):
            ...
            latent[:, timestep] = ci * qi / (1.0 - ci) + cg * qg / (1.0 - cg)
...
def compute_global_z0(
    ...
    log_ratio = (
        np.log(latent_storage[eligible][:, :][latent_storage[eligible] > 0.0]) - log_z0
    )
...
def audit_native_tau_envelope(
    parameters: np.ndarray, finite_basin: np.ndarray
) -> dict[str, Any]:
    ...
    return {
        ...
        "finite_selected_combined_tau_min": float(
            min(finite_ci.min(), finite_cg.min())
        ),
        "finite_selected_combined_tau_max": float(
            max(finite_ci.max(), finite_cg.max())
        ),
...
def run_freeze(
    *,
    ...
    parameters, source = select_existing_native_parameters(
        native_raw_directory, basin_ids
    )
    latent, finite_basin, positive_count = reconstruct_native_latent_storage(
        forcing, parameters
    )
    ...
    summary.update(
        {
            ...
            "invalid_latent_storage_basin_ids": [
                basin_id
                for basin_id, valid in zip(basin_ids, finite_basin)
                if not valid
            ],
...
def main() -> None:
    ...
    parser.add_argument(
        "--dataset", type=Path, default=_repo_root / "data/camels_dataset"
    )
    parser.add_argument(
        "--gage-ids", type=Path, default=_repo_root / "data/gage_id.npy"
    )
    parser.add_argument(
        "--basin-list", type=Path, default=_repo_root / "data/531sub_id.txt"
    )
    parser.add_argument(
        "--native-raw",
        type=Path,
