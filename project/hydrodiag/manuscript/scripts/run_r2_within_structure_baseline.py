...
def assign_regime(f: float) -> str:
    if f < 0.05: return "S1"
    elif f < 0.15: return "S2"
    elif f < 0.30: return "S3"
    elif f < 0.50: return "S4"
    else: return "S5"

def rms_dist(z1: np.ndarray, z2: np.ndarray) -> float:
    return float(np.sqrt(np.mean((z1 - z2) ** 2)))

def bootstrap_stat(values: np.ndarray, stat_func, n_boot: int = 10000, seed: int = 20260730) -> tuple[float, float, float]:
...
def bootstrap_regression(x: np.ndarray, y: np.ndarray, n_boot: int = 10000, seed: int = 20260730) -> dict:
    ...
    s_low, s_high = float(np.percentile(boot_slopes, 2.5)), float(np.percentile(boot_slopes, 97.5))
    r_low, r_high = float(np.percentile(boot_rhos, 2.5)), float(np.percentile(boot_rhos, 97.5))
    
    return {
        "slope": slope_point, "slope_ci_lower": s_low, "slope_ci_upper": s_high,
        "spearman_rho": float(rho_point), "spearman_rho_ci_lower": r_low, "spearman_rho_ci_upper": r_high,
    }
...
def main() -> None:
    ...
    ic_base_raw = PROJECT / "results" / "xaj_base_cmaes_531_batched_paired_v2" / "raw" / "xaj"
    ic_cn_raw = PROJECT / "results" / "xaj_cn_cmaes_531_batched_paired_v2" / "raw" / "xaj_cn"
    
    ic_z = {b: {"Base": {}, "CN": {}} for b in basins}
    for p in ic_base_raw.glob("*.json"):
        d = json.loads(p.read_text())
        b = str(d.get("basin_id", "")).zfill(8)
        if b not in ic_z: continue
        s = int(d.get("start"))
    ...
    for b in basins:
        ...
        if len(b_starts) != 10 or len(c_starts) != 10:
            raise ValueError(f"Basin {b} IC restarts incomplete: Base={len(b_starts)}, CN={len(c_starts)}")
            
        w_base_dists = [rms_dist(base_dict[s1][0], base_dict[s2][0]) for s1, s2 in combinations(b_starts, 2)]
        w_cn_dists = [rms_dist(cn_dict[s1][0], cn_dict[s2][0]) for s1, s2 in combinations(c_starts, 2)]
        
        w_base = float(np.median(w_base_dists))
        w_cn = float(np.median(w_cn_dists))
        w_pooled = (w_base + w_cn) / 2.0
        
        b_all_dists = [rms_dist(base_dict[s1][0], cn_dict[s2][0]) for s1 in b_starts for s2 in c_starts]
        b_all = float(np.median(b_all_dists))
        
        excess = b_all - w_pooled
        ratio = b_all / w_pooled if w_pooled > 1e-12 else np.nan
        
        matched_dists = [rms_dist(base_dict[s][0], cn_dict[s][0]) for s in b_starts if s in cn_dict]
        ...
        canonical_best_d_rms = rms_dist(base_dict[best_s_base][0], cn_dict[best_s_cn][0])
        
        ic_basin_rows.append({
            "basin_id": b, "paradigm": "IC",
            "within_base": w_base, "within_cn": w_cn, "within_pooled": w_pooled,
            "between_all": b_all, "excess": excess, "ratio": ratio,
            "matched_d_rms": matched_median, "canonical_best_d_rms": canonical_best_d_rms,
        })
        ...
        w5_b = float(np.median([rms_dist(base_dict[s1][0], base_dict[s2][0]) for s1, s2 in combinations(top5_b, 2)]))
        w5_c = float(np.median([rms_dist(cn_dict[s1][0], cn_dict[s2][0]) for s1, s2 in combinations(top5_c, 2)]))
        w5_pool = (w5_b + w5_c) / 2.0
        b5_all = float(np.median([rms_dist(base_dict[s1][0], cn_dict[s2][0]) for s1 in top5_b for s2 in top5_c]))
        ...
        w3_b = float(np.median([rms_dist(base_dict[s1][0], base_dict[s2][0]) for s1, s2 in combinations(top3_b, 2)]))
        w3_c = float(np.median([rms_dist(cn_dict[s1][0], cn_dict[s2][0]) for s1, s2 in combinations(top3_c, 2)]))
        w3_pool = (w3_b + w3_c) / 2.0
        b3_all = float(np.median([rms_dist(base_dict[s1][0], cn_dict[s2][0]) for s1 in top3_b for s2 in top3_c]))
        
        ic_kge_rows.append({
            "basin_id": b,
            "base_best_minus_median_kge": b_best_minus_med, "base_kge_iqr": b_iqr,
            "cn_best_minus_median_kge": c_best_minus_med, "cn_kge_iqr": c_iqr,
            "within_pooled_all10": w_pooled, "within_pooled_top5": w5_pool, "within_pooled_top3": w3_pool,
            "between_all_all10": b_all, "between_all_top5": b5_all, "between_all_top3": b3_all,
            "excess_all10": excess, "excess_top5": b5_all - w5_pool, "excess_top3": b3_all - w3_pool,
        })
    ...
    dpl_seed = df_seed[(df_seed["paradigm"] == "dPL") & (df_seed["structure"].isin(["Base", "CN"]))].copy()
    ...
    for b in basins:
        ...
        if len(b_seeds) != 3 or len(c_seeds) != 3:
            raise ValueError(f"Basin {b} dPL seeds incomplete: Base={len(b_seeds)}, CN={len(c_seeds)}")
            
        w_base = float(np.median([rms_dist(b_dict[s1], b_dict[s2]) for s1, s2 in combinations(b_seeds, 2)]))
        w_cn = float(np.median([rms_dist(c_dict[s1], c_dict[s2]) for s1, s2 in combinations(c_seeds, 2)]))
        w_pooled = (w_base + w_cn) / 2.0
        
        b_all = float(np.median([rms_dist(b_dict[s1], c_dict[s2]) for s1 in b_seeds for s2 in c_seeds]))
        excess = b_all - w_pooled
        ratio = b_all / w_pooled if w_pooled > 1e-12 else np.nan
    ...
    df_basin_all.to_csv(RESULTS_R2 / "r2_within_structure_basin_level.csv", index=False, float_format="%.17g")
    
    df_ic_kge = pd.DataFrame(ic_kge_rows).merge(snow, on="basin_id")
    df_ic_kge.to_csv(RESULTS_R2 / "r2_ic_restart_quality_audit.csv", index=False, float_format="%.17g")
    ...
    metrics = ["within_base", "within_cn", "within_pooled", "between_all", "excess", "ratio", "prop_excess_gt_0", "prop_between_gt_within"]
    ...
    for paradigm in ["IC", "dPL"]:
        for st_name, st_df in strata_list:
            ...
            for m in ["within_base", "within_cn", "within_pooled", "between_all", "excess", "ratio"]:
                vals = sub[m].dropna().to_numpy()
                pt, low, high = bootstrap_stat(vals, np.median, n_boot=10000, seed=20260730)
                summary_rows.append({
                    "paradigm": paradigm, "stratum": st_name, "n_basins": n_b,
                    "metric": m, "median": pt, "ci_lower": low, "ci_upper": high,
                })
                
            # Proportions
            exc_vals = (sub["excess"] > 0).astype(float).to_numpy()
            pt, low, high = bootstrap_stat(exc_vals, np.mean, n_boot=10000, seed=20260730)
            summary_rows.append({
                "paradigm": paradigm, "stratum": st_name, "n_basins": n_b,
                "metric": "prop_excess_gt_0", "median": pt, "ci_lower": low, "ci_upper": high,
            })
            
            bet_vals = (sub["between_all"] > sub["within_pooled"]).astype(float).to_numpy()
            pt, low, high = bootstrap_stat(bet_vals, np.mean, n_boot=10000, seed=20260730)
            summary_rows.append({
                "paradigm": paradigm, "stratum": st_name, "n_basins": n_b,
                "metric": "prop_between_gt_within", "median": pt, "ci_lower": low, "ci_upper": high,
            })

    df_summary = pd.DataFrame(summary_rows)
    df_summary.to_csv(RESULTS_R2 / "r2_within_structure_summary.csv", index=False, float_format="%.17g")
    ...
    for paradigm in ["IC", "dPL"]:
        for st_name, st_df in reg_strata:
            ...
            for dep in ["within_pooled", "between_all", "excess"]:
                ...
                reg_rows.append({
                    "paradigm": paradigm, "stratum": st_name, "dependent_var": dep,
                    **res
                })
                
    df_reg = pd.DataFrame(reg_rows)
    df_reg.to_csv(RESULTS_R2 / "r2_within_structure_regressions.csv", index=False, float_format="%.17g")
    ...
    for paradigm in ["IC", "dPL"]:
        p_base_z = df_canon[(df_canon["structure"] == "Base") & (df_canon["paradigm"] == paradigm)].pivot_table(index="basin_id", columns="parameter", values="z")[COMMON_XAJ]
        p_cn_z = df_canon[(df_canon["structure"] == "CN") & (df_canon["paradigm"] == paradigm)].pivot_table(index="basin_id", columns="parameter", values="z")[COMMON_XAJ]
        
        diff2 = (p_base_z - p_cn_z) ** 2
        sum_diff2 = diff2.sum(axis=1)
        
        for eps in thresholds:
            is_bnd = (p_base_z <= eps) | (p_base_z >= 1.0 - eps) | (p_cn_z <= eps) | (p_cn_z >= 1.0 - eps)
            bnd_diff2 = (diff2 * is_bnd).sum(axis=1)
            frac = np.where(sum_diff2 > 1e-12, bnd_diff2 / sum_diff2, 0.0)
            
            df_frac = pd.DataFrame({"basin_id": p_base_z.index, "frac": frac}).merge(snow, on="basin_id")
            ...
            for st_name, st_sub in bnd_strata:
                ...
                bnd_rows.append({
                    "paradigm": paradigm, "threshold": eps, "stratum": st_name,
                    "median_boundary_contribution_percent": med_pct,
                })
                
    df_bnd = pd.DataFrame(bnd_rows)
    df_bnd.to_csv(RESULTS_R2 / "r2_boundary_contribution_audit.csv", index=False, float_format="%.17g")
