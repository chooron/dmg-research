...
def generate_table4_main(r4_dir: Path, out_dir: Path) -> pd.DataFrame:
    ...
    df_paired = pd.read_csv(r4_dir / "paired_structural_effects.csv", dtype={"basin_id": str})
    df_summary = pd.read_csv(r4_dir / "timing_metrics_basin_summary.csv", dtype={"basin_id": str})
    ...
    row_1 = {"Quantity": "Active-melt state-consistency difference ($\Delta\\rho_{\\text{anom}}$, CN − Base)"}
    # 2. Summer dry-down state consistency difference
    row_2 = {"Quantity": "Summer dry-down state-consistency difference ($\Delta\\rho_{\\text{anom}}$, CN − Base)"}
    # 3. Active-melt consistency superiority in Q3
    row_3 = {"Quantity": f"Active-melt consistency superiority in Q3 (% catchments with CN > Base, n = {q3_n})"}
    # 4. Spring wet-up median signed timing error
    row_4 = {"Quantity": "Spring wet-up median signed error (Base $\\rightarrow$ CN, days)"}
    # 5. Soil-water peak median signed timing error
    row_5 = {"Quantity": "Soil-water peak median signed error (Base $\\rightarrow$ CN, days)"}
    # 6. Spring wet-up baseline MAE reduction
    row_6 = {"Quantity": "Spring wet-up baseline MAE reduction (Base MAE − CN MAE, days)"}
    # 7. Soil-water peak baseline MAE reduction
    row_7 = {"Quantity": "Soil-water peak baseline MAE reduction (Base MAE − CN MAE, days)"}

    for reg, col_name in [("dPL_seed42", "dPL-42"), ("IC_fused", "IC fused")]:
        # Row 1
        sub_am = df_phase[(df_phase["regime"] == reg) & (df_phase["phase_name"] == "Phase_2_Active_Melt_Recharge")]
        ...
        # Row 2
        sub_dd = df_phase[(df_phase["regime"] == reg) & (df_phase["phase_name"] == "Phase_4_Summer_Dry_Down")]
        ...
        pct_am_q3 = 100.0 * (sub_am_q3["cn_anomaly_corr"] > sub_am_q3["base_anomaly_corr"]).mean()
        row_3[col_name] = f"{pct_am_q3:.1f}%"

        # Row 4 & 5
        sub_sum = df_summary[(df_summary["regime"] == reg) & (df_summary["basin_id"].isin(q3_basins))]
        piv_w = sub_sum.pivot(index="basin_id", columns="structure", values="median_wetup_error_days")
        row_4[col_name] = f"{piv_w['Base'].median():+.1f} $\\rightarrow$ {piv_w['CN'].median():+.1f}"

        piv_p = sub_sum.pivot(index="basin_id", columns="structure", values="median_peak_error_days")
        row_5[col_name] = f"{piv_p['Base'].median():+.1f} $\\rightarrow$ {piv_p['CN'].median():+.1f}"
...
def generate_tables7_timing_sensitivity(r4_dir: Path, out_dir: Path) -> pd.DataFrame:
    ...
    for label, p_def, w_def, m_type in rows_def:
        ...
        for reg, prefix in [("dPL_seed42", "dPL-42"), ("IC_fused", "IC")]:
            sub = df_sens[(df_sens["regime"] == reg) & (df_sens["peak_definition"] == p_def) & (df_sens["wetup_definition"] == w_def)].iloc[0]
            if m_type == "wetup":
