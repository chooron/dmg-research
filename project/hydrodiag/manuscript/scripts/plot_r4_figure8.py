...
REGIME_CFG = {
    CANONICAL_DPL: {"label": "dPL (seed 42)", "color": "#882E72", "marker": "o", "ls": "-"},
    "IC_fused": {"label": "IC (fused)", "color": "#333333", "marker": "D", "ls": "-."},
}
STATE_CFG = {
    "Reference": {"label": "ERA5-Land SM$_{100}$ reference", "color": "#555555", "ls": "-", "lw": 1.2},
...
def _water_year_doy_to_date(water_year: int, doy: float) -> pd.Timestamp:
    return pd.Timestamp(year=int(water_year) - 1, month=10, day=1) + pd.Timedelta(days=float(doy) - 1)


def _select_illustrative_basin_year(results_root: Path) -> tuple[str, int, dict]:
    official = results_root / "r4_phase1_soil_official"
    summary = pd.read_csv(official / "timing_metrics_basin_summary.csv", dtype={"basin_id": str})
    state = pd.read_csv(official / "basin_state_consistency.csv", dtype={"basin_id": str})
    basin_swe = state[(state["regime"] == CANONICAL_DPL) & (state["structure"] == "Base")]
    basin_swe = basin_swe.drop_duplicates("basin_id")[["basin_id", "snow_burden_swe_mm"]]
    ...
    fields = ["n_valid_snow_years", "median_abs_wetup_error_days", "median_abs_peak_error_days"]
    ...
    candidates = candidates.reset_index().merge(basin_swe, on="basin_id", validate="one_to_one")
    candidates["wetup_improvement_days"] = (
        candidates["Base_median_abs_wetup_error_days"] - candidates["CN_median_abs_wetup_error_days"]
    )
    candidates["peak_improvement_days"] = (
        candidates["Base_median_abs_peak_error_days"] - candidates["CN_median_abs_peak_error_days"]
    )
    ...
    target = candidates[["wetup_improvement_days", "peak_improvement_days", "snow_burden_swe_mm"]].median()
    scale = candidates[["wetup_improvement_days", "peak_improvement_days", "snow_burden_swe_mm"]].std().replace(0, 1)
    candidates["selection_score"] = sum(
        ((candidates[col] - target[col]).abs() / scale[col])
        for col in ["wetup_improvement_days", "peak_improvement_days", "snow_burden_swe_mm"]
    )
    selected = candidates.sort_values(["selection_score", "basin_id"]).iloc[0]
    basin_id = str(selected["basin_id"]).zfill(8)

    year = pd.read_csv(official / "timing_metrics_basin_year.csv", dtype={"basin_id": str})
    year = year[
    ...
    for water_year, group in year.groupby("water_year"):
        ...
        year_rows.append({
            ...
            "wetup_improvement_days": abs(float(g.loc["Base", "wetup_timing_error_days"])) - abs(float(g.loc["CN", "wetup_timing_error_days"])),
            "peak_improvement_days": abs(float(g.loc["Base", "peak_timing_error_days"])) - abs(float(g.loc["CN", "peak_timing_error_days"])),
        })
    years = pd.DataFrame(year_rows)
    year_target = np.array([selected["wetup_improvement_days"], selected["peak_improvement_days"]], dtype=float)
    year_scale = years[["wetup_improvement_days", "peak_improvement_days"]].std().replace(0, 1).to_numpy()
    years["selection_score"] = (
        (years["wetup_improvement_days"] - year_target[0]).abs() / year_scale[0]
        + (years["peak_improvement_days"] - year_target[1]).abs() / year_scale[1]
    )
    selected_year = int(years.sort_values(["selection_score", "water_year"]).iloc[0]["water_year"])
    audit = {
        ...
        "selected_basin_wetup_improvement_days": float(selected["wetup_improvement_days"]),
        "selected_basin_peak_improvement_days": float(selected["peak_improvement_days"]),
        "selected_water_year": selected_year,
        "selected_year_annual_swe_max_mm": float(years.loc[years.water_year.eq(selected_year), "annual_swe_max_mm"].iloc[0]),
        "selected_year_wetup_improvement_days": float(years.loc[years.water_year.eq(selected_year), "wetup_improvement_days"].iloc[0]),
        "selected_year_peak_improvement_days": float(years.loc[years.water_year.eq(selected_year), "peak_improvement_days"].iloc[0]),
...
def _load_illustrative_series(results_root: Path, basin_id: str, water_year: int):
    ...
    swe = np.load(results_root / "r4_swe_reference_v1" / "swe_ensemble.npz")["swe_median"][idx, test_slice].astype(float)
    ...
    base = (base_npz["wu"][idx, test_slice] + base_npz["wl"][idx, test_slice] + base_npz["wd"][idx, test_slice]).astype(float)
    cn = (cn_npz["wu"][idx, test_slice] + cn_npz["wl"][idx, test_slice] + cn_npz["wd"][idx, test_slice]).astype(float)
    timing = pd.read_csv(results_root / "r4_phase1_soil_official" / "timing_metrics_basin_year.csv", dtype={"basin_id": str})
    timing = timing[(timing["regime"] == CANONICAL_DPL) & (timing["basin_id"] == basin_id) & (timing["water_year"] == water_year)]
...
def generate_figure8(results_root: Path, out_dir: Path) -> Path:
    ...
    (HERE / "figure8_r4_selection_audit.json").write_text(json.dumps(audit, indent=2), encoding="utf-8")
    dates, ref, base, cn, swe, timing = _load_illustrative_series(results_root, basin_id, water_year)
    ...
    df = pd.DataFrame({"date": dates, "ref_z": ref_z, "base_z": base_z, "cn_z": cn_z, "swe": swe})
    ...
    # Q3 Population
    summary = pd.read_csv(official / "timing_metrics_basin_summary.csv", dtype={"basin_id": str})
    paired = pd.read_csv(official / "paired_structural_effects.csv", dtype={"basin_id": str})
    ...
    ax_a.fill_between(df_wy["date"], 0, df_wy["swe"], color="#9ECAE1", alpha=0.25, zorder=2)
    ...
    for key in ["Reference", "Base", "CN"]:
        cfg = STATE_CFG[key]
        col_name = "ref_z" if key == "Reference" else ("base_z" if key == "Base" else "cn_z")
        ax_b.plot(df_wy["date"], df_wy[col_name], color=cfg["color"], ls=cfg["ls"], lw=cfg["lw"], label=cfg["label"], zorder=4)
    ...
    for key in ["Reference", "Base", "CN"]:
        cfg = STATE_CFG[key]
        col_name = "ref_z" if key == "Reference" else ("base_z" if key == "Base" else "cn_z")
        ax_c_top.plot(df_zoom["date"], df_zoom[col_name], color=cfg["color"], ls=cfg["ls"], lw=cfg["lw"], zorder=4)
    ...
    ax_c_bot.text(d_base_w, 1.22, f"{int(row_base['wetup_timing_error_days'])} d", ha="center", va="bottom", fontsize=5.5, color="#2878B5")
    ax_c_bot.text(d_cn_w, 0.78, f"{int(row_cn['wetup_timing_error_days'])} d", ha="center", va="top", fontsize=5.5, color="#D95F02")
    ...
    ax_c_bot.text(d_base_p, 0.22, f"{int(row_base['peak_timing_error_days'])} d", ha="center", va="bottom", fontsize=5.5, color="#2878B5")
    ax_c_bot.text(d_cn_p, -0.22, f"{int(row_cn['peak_timing_error_days'])} d", ha="center", va="top", fontsize=5.5, color="#D95F02")
    ...
    ax_c_top.legend(handles=leg_c, loc="upper left", fontsize=5.6, frameon=True, framealpha=0.92)
    ...
    for reg, reg_name in [("dPL_seed42", "dPL-42"), ("IC_fused", "IC fused")]:
        cfg = REGIME_CFG[reg]
        sub = summary[(summary["regime"] == reg) & (summary["basin_id"].isin(q3_basins))].copy()
        for struct, col, struct_name in [("Base", "#2878B5", "Base"), ("CN", "#D95F02", "CN")]:
            vals = sub[sub["structure"] == struct]["median_wetup_error_days"].dropna().to_numpy()
            sx = np.sort(vals)
            sy = np.linspace(0, 1, len(sx))
            ax_d.step(sx, sy, where="post", color=col, ls=cfg["ls"], lw=1.2, alpha=0.9, label=f"{struct_name} ({reg_name})")
    ...
    for reg, reg_name in [("dPL_seed42", "dPL-42"), ("IC_fused", "IC fused")]:
        ...
        sub = summary[(summary["regime"] == reg) & (summary["basin_id"].isin(q3_basins))].copy()
        for struct, col, struct_name in [("Base", "#2878B5", "Base"), ("CN", "#D95F02", "CN")]:
            vals = sub[sub["structure"] == struct]["median_peak_error_days"].dropna().to_numpy()
            sx = np.sort(vals)
            sy = np.linspace(0, 1, len(sx))
            ax_e.step(sx, sy, where="post", color=col, ls=cfg["ls"], lw=1.2, alpha=0.9, label=f"{struct_name} ({reg_name})")
    ...
    sens_rows = [
        ("Wet-up 7 d", "Peak_Annual_FullWY", "Wetup_07d_Spring", "wetup_abs_error_improvement_days"),
        ("Wet-up 14 d", "Peak_Annual_FullWY", "Wetup_14d_Spring", "wetup_abs_error_improvement_days"),
        ("Wet-up 21 d", "Peak_Annual_FullWY", "Wetup_21d_Spring", "wetup_abs_error_improvement_days"),
        ("Peak full WY", "Peak_Annual_FullWY", "Wetup_14d_Spring", "peak_abs_error_improvement_days"),
        ("Peak Mar–Aug", "Peak_SpringSummer_MarAug", "Wetup_14d_Spring", "peak_abs_error_improvement_days"),
    ]
    y_pos = np.array([4, 3, 2, 0.6, -0.4])

    for y, (label, peak_def, wet_def, value_col) in zip(y_pos, sens_rows):
        for reg in REGIMES:
            row = sens[(sens["regime"] == reg) & (sens["peak_definition"] == peak_def) & (sens["wetup_definition"] == wet_def)].iloc[0]
            ax_f.plot(row[value_col], y, marker=REGIME_CFG[reg]["marker"], color=REGIME_CFG[reg]["color"], ms=4.5, zorder=3)
    ...
    ax_f.set_xlim(-5, 45)
    ax_f.set_title("(f) Timing-definition sensitivity", loc="left", fontweight="bold", fontsize=8.0)
    ...
    leg_f = [
        Line2D([0], [0], marker=REGIME_CFG[r]["marker"], color=REGIME_CFG[r]["color"], ls="none", ms=4.5, label=REGIME_CFG[r]["label"])
        for r in REGIMES
    ]
    ...
    plt.close(fig)
    print(f"Generated Figure 8 (PNG only, 300 dpi):\n  {out}\n  basin={basin_id}, water_year={water_year}")
    return out
