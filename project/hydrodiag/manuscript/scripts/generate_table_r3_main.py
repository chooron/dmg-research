...
Usage: python manuscript/scripts/generate_table_r3_main.py
"""
...
def build_rows(d5, d6, psum, pval, seedmed):
    ...
    g_ic = seedmed.loc[seedmed["paradigm"] == "IC", "G_CN_over_TGD2"].dropna().to_numpy()
    g_dpl = seedmed.loc[seedmed["paradigm"] == "dPL", "G_CN_over_TGD2"].dropna().to_numpy()
    ci_ic_res = pval["V4"]["IC_test"]["G_CN_over_TGD2"]["boot_ci_median"]
    ci_dpl_res = boot_ci(g_dpl, np.median)

    rows = [
        # 1. Limited compensation
        dict(estimand="F_close,test", role="Limited compensation",
             ic=(pc["IC_test"]["median"], pc["IC_test"]["boot_ci_median_display"]),
             dpl=(pc["dPL_test"]["median"], pc["dPL_test"]["boot_ci_median_display"]),
             n_ic=pc["IC_test"]["n_valid"], n_dpl=pc["dPL_test"]["n_valid"],
             decimals=3, source="figure5_summary.panel_c_f_close"),
        # 2. Generalization decay
        dict(estimand="decay_G_base", role="Generalization decay",
             ic=(dd["IC"]["median"], dd["IC"]["boot_ci_median"]),
             dpl=(dd["dPL_agg"]["median"], dd["dPL_agg"]["boot_ci_median_display"]),
             n_ic=dd["IC"]["n_valid"], n_dpl=dd["dPL_agg"]["n"],
             decimals=4, source="figure5_summary.panel_d_decay"),
        # 3. Generic mitigation
        dict(estimand="F_tgd2 (test)", role="Generic mitigation",
             ic=(pb["IC"]["median"], pb["IC"]["boot_ci_median_display"]),
             dpl=(pb["dPL"]["median"], pb["dPL"]["boot_ci_median_display"]),
             n_ic=pb["IC"]["n_valid"], n_dpl=pb["dPL"]["n_valid"],
             decimals=3, source="figure6_summary.panel_b_f_tgd2"),
        # 4. Parameter relief (Delta C_theta)
        dict(estimand="Delta C_theta (R_theta_tgd2)", role="Parameter relief",
             ic=(prth["IC"]["median"], prth["IC"].get("boot_ci_frozen",
                                                     prth["IC"]["boot_ci_median_display"])),
             dpl=(prth["dPL"]["median"], prth["dPL"]["boot_ci_median_display"]),
             n_ic=prth["IC"]["n_valid"], n_dpl=prth["dPL"]["n_valid"],
             decimals=4, source="figure6_summary.panel_c_r_theta"),
        # 5. State relief (Delta C_state)
        dict(estimand="Delta C_state (R_state_tgd2)", role="State relief",
             ic=(prst["IC"]["median"], prst["IC"].get("boot_ci_frozen",
                                                      prst["IC"]["boot_ci_median_display"])),
             dpl=(prst["dPL"]["median"], prst["dPL"]["boot_ci_median_display"]),
             n_ic=prst["IC"]["n_valid"], n_dpl=prst["dPL"]["n_valid"],
             decimals=4, source="figure6_summary.panel_d_r_state"),
        # 6. Residual explicit-structure advantage
        dict(estimand="Delta KGE_CN-TGD2 (test)", role="Residual explicit advantage",
             ic=(pe["IC"]["median"], ci_ic_res),
             dpl=(pe["dPL"]["median"], ci_dpl_res),
             n_ic=int(len(g_ic)), n_dpl=int(len(g_dpl)),
             decimals=4, source="figure6_summary.panel_e_residual_vs_frac_snow + V4 frozen CI"),
        # 7. Snow-active residual
        dict(estimand="Delta RMSE_snow (mm d-1)", role="Residual on snow-active days",
             ic=(pf["IC"]["snow_active"]["median"], pf["IC"]["snow_active"]["boot_ci_median_display"]),
             dpl=(pf["dPL"]["snow_active"]["median"], pf["dPL"]["snow_active"]["boot_ci_median_display"]),
             n_ic=pf["IC"]["snow_active"]["n_valid"], n_dpl=pf["dPL"]["snow_active"]["n_valid"],
             decimals=3, source="figure6_summary.panel_f_process_errors"),
        # 8. Non-snow residual
        dict(estimand="Delta RMSE_non-snow (mm d-1)", role="Residual on non-snow days",
             ic=(pf["IC"]["no_snow_active"]["median"], pf["IC"]["no_snow_active"]["boot_ci_median_display"]),
             dpl=(pf["dPL"]["no_snow_active"]["median"], pf["dPL"]["no_snow_active"]["boot_ci_median_display"]),
             n_ic=pf["IC"]["no_snow_active"]["n_valid"], n_dpl=pf["dPL"]["no_snow_active"]["n_valid"],
             decimals=3, source="figure6_summary.panel_f_process_errors"),
    ]
    return rows
...
def validate(rows, psum, pval, d5, d6):
    ...
    anchors = [
        ...
        (rows[5]["ic"][0], pval["V4"]["IC_test"]["G_CN_over_TGD2"]["median"], "G_CN_over_TGD2 IC"),
    ]
    ...
    dpl_seed_checks = [
        (rows[0]["dpl"][0], d5["panel_c_f_close"]["dPL_test"]["seed_medians"], "F_close dPL"),
        (rows[2]["dpl"][0], d6["panel_b_f_tgd2"]["dPL"]["seed_medians"], "F_tgd2 dPL"),
        (rows[1]["dpl"][0], [pval[f"dPL_{s}_decay_G_base"]["median"] for s in (42, 123, 2026)],
         "decay_G_base dPL"),
        (rows[3]["dpl"][0], d6["panel_c_r_theta"]["dPL"]["seed_medians"], "R_theta dPL"),
        (rows[4]["dpl"][0], d6["panel_d_r_state"]["dPL"]["seed_medians"], "R_state dPL"),
        (rows[5]["dpl"][0], [pval["V4"][f"dPL_{s}_test"]["G_CN_over_TGD2"]["median"] for s in (42, 123, 2026)],
         "G_CN_over_TGD2 dPL"),
    ]
    tol = 1e-3
    for got, seed_meds, label in dpl_seed_checks:
        ...
        if not (lo - tol <= got <= hi + tol):
            errors.append(f"{label}: aggregated {got:.6f} outside seed range [{lo:.6f}, {hi:.6f}]")
    if errors:
        raise SystemExit("Table 5 validation FAILED:\n  " + "\n  ".join(errors))
    print("[check] Table 5 medians match frozen canonical R3 summaries (IC exact 1e-9; "
          "dPL aggregated within frozen per-seed range).")


def build_markdown(rows, n_note):
    header = ("| Estimand | Evidence role | IC median [95% CI] | dPL median [95% CI] | "
              "N (IC/dPL) |\n"
              "| :--- | :--- | :--- | :--- | :---: |\n")
    body = ""
    for r in rows:
        body += ("| {} | {} | {} | {} | {}/{} |\n".format(
            ...
            r["n_ic"], r["n_dpl"]))
    note = ("\n*Note*: {}\n".format(n_note))
    title = ("# Table 5: R3 Synthetic Known-Truth Experiment — Summary of "
...
def main() -> None:
    ...
    for r in rows:
        csv_rows.append({
            "estimand": r["estimand"], "role": r["role"],
            "ic_median": r["ic"][0], "ic_ci_low": r["ic"][1][0], "ic_ci_high": r["ic"][1][1],
            "dpl_median": r["dpl"][0], "dpl_ci_low": r["dpl"][1][0], "dpl_ci_high": r["dpl"][1][1],
            "n_ic": r["n_ic"], "n_dpl": r["n_dpl"], "source": r["source"],
        })
