...
DEFAULT_R13_DIR = Path(
    "/home/jingxin/orca/workspaces/dmg-research/flex-mopex/project/flexmopex/results/root_cause_r13"
)
DEFAULT_OUT_DIR = HERE.parents[1] / "figures"
...
def generate_figure_r13(r13_dir: Path, out_dir: Path) -> tuple[Path, Path]:
    ...
    raw_row = df_rep[
        (df_rep["run"] == "Raw_Attributes_X") & (df_rep["target"] == target_filter)
    ]
    ...
    for run_name, label, color, marker, ls in runs_meta:
        ...
        if not sub.empty:
            ...
            ax_a.fill_between(
                epochs, aucs - stds, aucs + stds, color=color, alpha=0.12, zorder=2
            )
    ...
    ax_a.axhline(
        ...
        linestyle="--",
        linewidth=1.1,
        label=f"Raw Attributes $X$ ({raw_auc:.3f})",
        zorder=1,
    )
    ...
    ax_a.set_title(
        "(a) H-A: Representation Discrimination in Backbone $h$",
        loc="left",
        fontweight="bold",
        fontsize=8.5,
    )
    ...
    ax_a.text(
        ...
        fontsize=7.0,
        bbox=dict(
            boxstyle="round,pad=0.3", facecolor="#E8F4F8", edgecolor=c_blue, alpha=0.9
        ),
    ...
    b1 = ax_b.bar(
        ...
        can_vals,
        width=w,
        label=r"Canonical ($\cos(G_{\mathrm{pos}}, G_{\mathrm{full}})$)",
        color=c_blue,
        alpha=0.9,
    )
    b2 = ax_b.bar(
        x_pos, nobias_vals, width=w, label=r"No-Bias Decomp.", color=c_teal, alpha=0.9
    )
    b3 = ax_b.bar(
        x_pos + w,
        cent_vals,
        width=w,
        label=r"Centered / No-DC Decomp.",
        color=c_orange,
        alpha=0.9,
    )
    ...
    ax_b.set_ylabel(
        r"Gradient Alignment with Full Update $\cos(G_{\mathrm{pos}}, G_{\mathrm{full}})$"
    )
    ax_b.set_title(
        r"(b) H-C: Gradient Alignment & Head Decomposition (ep2)",
        loc="left",
        fontweight="bold",
        fontsize=8.5,
    )
    ...
    ax_b.annotate(
        ...
        bbox=dict(
            boxstyle="round,pad=0.25", facecolor="#FFF0F0", edgecolor=c_red, alpha=0.9
        ),
    )
    ax_b.legend(
        loc="upper left",
    ...
    ax_c.plot(
        ...
        marker="o",
        linewidth=1.5,
        label=r"Learned $w_{\mathrm{int}}$ Median",
        zorder=3,
    )
    ...
    ax_c.set_xticks(eps)
    ax_c.set_title(
        r"(c) H-B: Gate Collapse vs Finite Benefit Retention (N=103)",
        loc="left",
        fontweight="bold",
        fontsize=8.5,
    )
    ...
    ax_c_twin.plot(
        ...
        linestyle="--",
        linewidth=1.4,
        label=r"Finite $\Delta\mathrm{NSE}_{\max}$ Median",
        zorder=3,
    )
    ...
    ax_c.text(
        ...
        fontsize=6.8,
        bbox=dict(
            boxstyle="round,pad=0.3", facecolor="#F5F5F5", edgecolor=c_grey, alpha=0.9
        ),
        verticalalignment="bottom",
    )
    ...
    cos_wint_full = []
    for ep in epochs_d:
        row = df_init[(df_init["epoch"] == ep) & (df_init["process"] == "w_int")].iloc[
            0
        ]
    ...
    ax_d.plot(
        ...
        marker="D",
        linewidth=1.6,
        label=r"$w_{\mathrm{int}}$ $\cos(G_{\mathrm{pos}}, G_{\mathrm{full}})$",
        zorder=4,
    )

    # Cross-head alignments
    cross_snow = [
        init_json[f"ep{ep}"]["cross_process_alignment"]["w_int_vs_w_snow"]
        for ep in epochs_d
    ]
    cross_sub = [
        init_json[f"ep{ep}"]["cross_process_alignment"]["w_int_vs_w_sub"]
        for ep in epochs_d
    ]
    cross_phen = [
        init_json[f"ep{ep}"]["cross_process_alignment"]["w_int_vs_w_phen"]
        for ep in epochs_d
    ]
    ...
    ax_d.plot(
        ...
        linestyle="--",
        linewidth=1.2,
        label=r"Cross-Head: $w_{\mathrm{int}}$ vs $w_{\mathrm{snow}}$",
        zorder=3,
    )
    ax_d.plot(
        epochs_d,
        ...
        linestyle="-.",
        linewidth=1.2,
        label=r"Cross-Head: $w_{\mathrm{int}}$ vs $w_{\mathrm{sub}}$",
        zorder=3,
    )
    ax_d.plot(
        epochs_d,
        ...
        linestyle=":",
        linewidth=1.2,
        label=r"Cross-Head: $w_{\mathrm{int}}$ vs $w_{\mathrm{phen}}$",
        zorder=3,
    )
    ...
    ax_d.set_title(
        r"(d) H-D: Dynamic Polarization & Cross-Head Fluctuation",
        loc="left",
        fontweight="bold",
        fontsize=8.5,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--r13-dir",
        type=Path,
        default=DEFAULT_R13_DIR,
        help="Path to R13 root-cause results directory",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
