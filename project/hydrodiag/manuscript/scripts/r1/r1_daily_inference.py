...
def read_ic_parameters(
    results_root: Path, model_key: str, basin_ids: Iterable[str]
) -> tuple[np.ndarray, list[Path], list[int]]:
    raw_dir = (
        results_root
        / IC_ROOTS[model_key]
        / "raw"
        / {"XAJ": "xaj", "XAJ_TGD2": "xaj_tgd2", "XAJ_CN": "xaj_cn"}[model_key]
    )
    ...
    for path in sorted(raw_dir.glob("*.json")):
        ...
        if np.isfinite(train_kge) and data.get("status") == "complete":
            records.setdefault(basin, []).append(
                (train_kge, int(data["start"]), path, data)
            )
    selected = []
    ...
    parameters = np.asarray(
        [item[3]["parameters"] for item in selected], dtype=np.float32
    )
...
def select_dpl_checkpoints(
    results_root: Path, model_key: str, requested_epoch: int | None = None
) -> tuple[list[tuple[str, Path, Path, str]], dict[str, Any]]:
    root = (
        results_root
        / (
            "dpl_camels_531_lite_v3_tgd2_dpl_audited"
            if model_key == "XAJ_TGD2"
            else "dpl_camels_531_lite_v2"
        )
        / model_key
    )
    ...
    if len(seed_dirs) != 3:
        raise ValueError(
            f"expected three dPL seeds for {model_key}, found {len(seed_dirs)}"
        )
    ...
    if model_key == "XAJ_TGD2":
        ...
        for seed_dir in seed_dirs:
            ...
            if (
                epoch not in set(history["epoch"].astype(int))
                or int(metadata.get("epoch", -1)) != epoch
            ):
            ...
            selected.append(
                (
                    ...
                    checkpoint,
                    config,
                    f"latest common valid periodic checkpoint epoch {epoch}",
                )
            )
        return selected, {
            "root": str(root),
    ...
    for seed_dir in seed_dirs:
        ...
        selected.append(
            (
                ...
                checkpoint,
                config,
                "best_checkpoint.pt is the existing authoritative seed artifact",
            )
        )
    return selected, {
        "root": str(root),
        "rule": "existing best_checkpoint.pt per seed",
        "epoch": None,
    }


def _period_arrays(
    bundle, period: str, basin_indices: np.ndarray | None = None
    ...
    if period == "train":
        forcing = bundle.forcing[
            selector,
            periods.train_forcing_start_index : periods.train_forcing_end_index,
            :,
        ]
        obs = bundle.target_mm_day[
            selector, periods.train.start_index : periods.train.end_index + 1
        ]
        dates = pd.to_datetime(
            bundle.dates[periods.train.start_index : periods.train.end_index + 1]
        )
    elif period == "test":
        forcing = bundle.forcing[
            selector,
            periods.test_forcing_start_index : periods.test_forcing_end_index,
            :,
        ]
        obs = bundle.target_mm_day[
            selector, periods.test.start_index : periods.test.end_index + 1
        ]
        dates = pd.to_datetime(
            bundle.dates[periods.test.start_index : periods.test.end_index + 1]
        )
    else:
...
def _predict_ic(
    ...
    device: str,
    batch_size: int,
    basin_indices: np.ndarray | None = None,
) -> dict[str, np.ndarray]:
    import torch
    from ablation.ic_core.model_adapter import ModelAdapter

    adapter = ModelAdapter(
        model_key, device=device, dtype=torch.float32, variant="lite"
    )
    ...
    for period in ("train", "test"):
        ...
        indices = (
            basin_indices
            if basin_indices is not None
            else np.arange(len(bundle.basin_ids))
        )
        ...
        for start in range(0, len(indices), batch_size):
            ...
            q, _ = adapter.run_model(
                force,
                params,
                temp_mean_train=torch.from_numpy(
                    bundle.temp_mean_train[indices[start:stop]]
                ),
                temp_std_train=torch.from_numpy(
                    bundle.temp_std_train[indices[start:stop]]
                ),
            )
...
def _predict_dpl(
    ...
    indices = (
        basin_indices if basin_indices is not None else np.arange(len(bundle.basin_ids))
    )
    ...
    with torch.no_grad():
        for period in ("train", "test"):
            ...
            predicted = np.full(
                (len(indices), forcing.shape[1]), np.nan, dtype=np.float32
            )
            for start in range(0, len(indices), batch_size):
                stop = min(start + batch_size, len(indices))
                ...
                fc = {
                    ...
                    "temp": torch.from_numpy(forcing[start:stop, :, 1]).to(
                        device=device, dtype=torch.float32
                    ),
                    "pet": torch.from_numpy(forcing[start:stop, :, 2]).to(
                        device=device, dtype=torch.float32
                    ),
                    "temp_mean_train": torch.from_numpy(
                        bundle.temp_mean_train[indices[start:stop]]
...
def _write_run_parquet(
                ...
                frame = pd.DataFrame(
                    {
                        ...
                        "valid_obs": np.isfinite(obs[start:stop]).reshape(-1)
                        & (obs[start:stop].reshape(-1) >= 0),
                        "valid_sim": np.isfinite(qsim[start:stop]).reshape(-1)
                        & (qsim[start:stop].reshape(-1) >= 0),
                        "source_checkpoint_or_parameter_file": str(
                            spec.source_checkpoint
                        )
                        if not parameter_files
                        else np.repeat(
                            [
                                str(parameter_files[index])
    ...
    return {
        ...
        "rows": row_count,
        "sha256": sha256(path),
        "basins": len(bundle.basin_ids),
        "periods": ["train", "test"],
        "date_start": str(periods.warmup.start),
        "date_end": str(periods.test.end),
        "status": "complete",
    }


def _online_metric_rows(
    spec: RunSpec,
    bundle,
    predictions: dict[str, np.ndarray],
    parameter_files: list[Path],
    ...
    for period in ("train", "test"):
        ...
        for local_index, basin_index in enumerate(indices):
            ...
            (
                ...
                valid_days,
                valid_metric,
            ) = standard_kge(simulated, observed)
            mask = (
            ...
            nse_denominator = (
                float(np.sum((observed[mask] - observed[mask].mean()) ** 2))
                if valid_days
                else 0.0
            )
            nse = (
            ...
            pbias = (
                float(100.0 * error.sum() / denominator) if denominator != 0 else np.nan
            )
            rmse = float(np.sqrt(np.mean(error**2))) if valid_days else np.nan
            source_file = (
                parameter_files[basin_index]
                if parameter_files
                else spec.source_checkpoint
            )
            run_id = spec.run_id
            ...
            rows.append(
                {
                    ...
                    "source_checkpoint_or_parameter_file": str(source_file),
                    "source_configuration": str(spec.source_configuration),
                }
            )
    return rows
...
def _online_signature_year_rows(
    spec: RunSpec,
    bundle,
    predictions: dict[str, np.ndarray],
    basin_indices: np.ndarray | None = None,
    ...
    for period in ("train", "test"):
        ...
        for local_index, basin_index in enumerate(indices):
            ...
            for water_year in sorted(set(water_years)):
                ...
                expected_days = (
                    int((year_dates[-1] - year_dates[0]).days + 1)
                    if len(year_dates)
                    else 0
                )
                complete = (
                    len(year_dates) == expected_days
                    and len(set(year_dates)) == expected_days
                    and bool(
                        (
                            np.isfinite(observed)
                            & np.isfinite(simulated)
                ...
                if complete:
                    ...
                    ct_obs = (
                        int(np.argmax(np.cumsum(observed) >= 0.5 * observed_total) + 1)
                        if observed_total > 0
                        else np.nan
                    )
                    ct_sim = (
                        int(
                            np.argmax(np.cumsum(simulated) >= 0.5 * simulated_total) + 1
                        )
                    ...
                    amjj_obs = (
                        float(observed[april_july].sum() / observed_total)
                        if observed_total > 0
                        else np.nan
                    )
                    amjj_sim = (
                        float(simulated[april_july].sum() / simulated_total)
                        if simulated_total > 0
                        else np.nan
                    )
                    row.update(
                        {
                            ...
                            "spo_obs": np.nan,
                            "spo_sim": np.nan,
                            "spo_error_signed": np.nan,
                            "spo_error_absolute": np.nan,
                            "amjj_obs": amjj_obs,
                            "amjj_sim": amjj_sim,
...
def run_daily_export(
    ...
    basin_indices = np.array_split(np.arange(len(bundle.basin_ids)), partition_count)[
        partition_index
    ]
    if device == "cuda":
        import torch
        ...
        if not torch.cuda.is_available():
            raise RuntimeError(
                "CUDA was requested for R1 inference but no CUDA device is available; refusing CPU fallback"
            )
    specs: list[RunSpec] = []
    ...
    if paradigm in ("all", "ic"):
        for model_key in selected_model_keys:
            if model_key == "HBV":
                continue
            parameters, files, starts = read_ic_parameters(
                results_root, model_key, bundle.basin_ids
            )
            specs.append(
                RunSpec(
                    "IC-CMA-ES",
                    ...
                    f"selected_restart",
                    files[0],
                    results_root / IC_ROOTS[model_key] / "manifest.json",
                    "maximum train-period stored KGE; tie=min restart",
                    tuple(files),
                )
            )
    ...
    if paradigm in ("all", "dpl"):
        for model_key in selected_model_keys:
            selected, selection_meta = select_dpl_checkpoints(
                results_root,
                model_key,
                requested_epoch=tgd2_epoch if model_key == "XAJ_TGD2" else None,
            )
            if model_key == "XAJ_TGD2":
                selected_tgd2 = {
                    **selection_meta,
                    "seeds": [
                        {
                            ...
                            "timestamp": pd.Timestamp(
                                checkpoint.stat().st_mtime, unit="s", tz="UTC"
                            ).isoformat(),
                            "config": str(config),
                        }
                        for seed, checkpoint, config, _reason in selected
                    ],
                }
    ...
    for spec in specs:
        parameters, parameter_files, _starts = read_ic_parameters(
            results_root, spec.model_key, bundle.basin_ids
        )
        predictions = _predict_ic(
            bundle, spec.model_key, parameters, device, batch_size, basin_indices
        )
        path = (
            output_root
            / f"r1_daily_simulations_ic_{spec.model_key.lower()}{partition_suffix}.parquet"
        )
        row = _write_run_parquet(
            path, spec, bundle, predictions, parameter_files, basin_indices
        )
        row.update(
            {
                "model": spec.model_label,
                "paradigm": spec.paradigm,
                "seed_or_restart": spec.run_id,
                "source_selection": spec.selection_reason,
            }
        )
        inventory.append(row)
        online_metric_rows.extend(
            _online_metric_rows(
                spec, bundle, predictions, parameter_files, basin_indices
            )
        )
        online_signature_year_rows.extend(
            _online_signature_year_rows(spec, bundle, predictions, basin_indices)
        )
    for model_key, seed, checkpoint, config, reason in dpl_runs:
        ...
        path = (
            output_root
            / f"r1_daily_simulations_dpl_{model_key.lower()}_seed_{seed}{partition_suffix}.parquet"
        )
        ...
        row.update(
            {
                ...
                "checkpoint_epoch": int(
                    __import__("torch")
                    .load(checkpoint, map_location="cpu", weights_only=False)
                    .get("epoch", -1)
                ),
            }
        )
        inventory.append(row)
        online_metric_rows.extend(
            _online_metric_rows(spec, bundle, predictions, [], basin_indices)
        )
        online_signature_year_rows.extend(
            _online_signature_year_rows(spec, bundle, predictions, basin_indices)
        )
    ...
    online_signature_years.to_csv(
        output_root / "r1_online_signature_basin_year.csv", index=False
    )
    audit = {
        ...
        "partition": {
            ...
            "basins_in_partition": int(len(basin_indices)),
            "first_basin": str(bundle.basin_ids[basin_indices[0]]),
            "last_basin": str(bundle.basin_ids[basin_indices[-1]]),
        },
        ...
        "online_statistics_inputs": [
            str(output_root / "r1_online_performance.csv"),
            str(output_root / "r1_online_signature_basin_year.csv"),
        ],
    }
    (output_root / "r1_inference_audit.md").write_text(
        "# R1 Inference Audit\n\n"
    ...
    signature_basin_level, signature_effects = signature_tables_from_years(
        online_signature_years
    )
    audit["precomputed"] = {
