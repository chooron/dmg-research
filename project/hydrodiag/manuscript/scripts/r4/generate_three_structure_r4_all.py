...
from manuscript.r4.robustness_analysis import bootstrap_median_ci
from manuscript.r4.soil_analysis import calendar_month_anomaly
...
def plot_figure7(results_root: Path) -> Path:
    ...
    x_dec = np.arange(10)
    for reg, reg_lbl, alpha in [
        ("dPL_seed42", "dPL-42", 1.0),
        ("IC_fused", "IC fused", 0.85),
    ]:
        ...
        ax_a.plot(
            ...
            ym_cn,
            color="#D95F02",
            marker="o" if reg == "dPL_seed42" else "D",
            markersize=4.6,
            lw=1.4,
            ls=ls_cn,
            label=f"CN − Base ({reg_lbl})",
            zorder=4,
        )
        ...
        ax_a.plot(
            ...
            ym_tgd,
            color="#008080",
            marker="^" if reg == "dPL_seed42" else "s",
            markersize=4.6,
            lw=1.3,
            ls=ls_tgd,
            label=f"TGD2 − Base ({reg_lbl})",
            zorder=4,
        )
    ...
    ax_a.set_title(
        "(a) Snow-burden dependence: generic control vs explicit snow",
        loc="left",
        fontweight="bold",
        fontsize=8.8,
    )
    ...
    for i, (reg, sub_title) in enumerate(
        [("dPL_seed42", "dPL seed 42"), ("IC_fused", "IC fused")]
    ):
        ...
        ax.errorbar(
            ...
            ym_cn,
            yerr=[
                np.array(ym_cn) - np.array(ylo_cn),
                np.array(yhi_cn) - np.array(ym_cn),
            ],
            fmt="o",
            color="#D95F02",
            ecolor="#D95F02",
        ...
        ax.errorbar(
            ...
            ym_tgd,
            yerr=[
                np.array(ym_tgd) - np.array(ylo_tgd),
                np.array(yhi_tgd) - np.array(ym_tgd),
            ],
            fmt="^",
            color="#008080",
            ecolor="#008080",
        ...
        ax.set_title(
            f"({chr(98 + i)}) {sub_title}", loc="left", fontweight="bold", fontsize=8.2
        )
        if i == 0:
    ...
    for ax, p_code, letter, p_title in [
        ...
        sub_ph = df_phase[
            (df_phase["regime"] == CANONICAL_DPL)
            & (df_phase["phase_code"] == p_code)
            & (df_phase["snow_burden_swe_mm"] >= q3_swe_mm)
        ]
        ...
        ax.step(
            ...
            ls="--",
            lw=1.3,
            label="TGD2 − Base (dPL-42)",
            zorder=4,
        )

        ax.set_title(
            f"({letter}) {p_title}", loc="left", fontweight="bold", fontsize=8.2
        )
...
def plot_figure8(results_root: Path) -> Path:
    ...
    swe = np.load(results_root / "r4_swe_reference_v1/swe_ensemble.npz")["swe_median"][
        idx, test_sl
    ].astype(float)

    base_npz = np.load(
        results_root
        / "r4_official_dpl_XAJ_seed42/official_dpl_XAJ_seed42_full_arrays.npz"
    )
    tgd2_npz = np.load(
        results_root
        / "r4_official_dpl_XAJ_TGD2_seed42/official_dpl_XAJ_TGD2_seed42_full_arrays.npz"
    )
    cn_npz = np.load(
        results_root
        / "r4_official_dpl_XAJ_CN_seed42/official_dpl_XAJ_CN_seed42_full_arrays.npz"
    )

    base = (
        base_npz["wu"][idx, test_sl]
        + base_npz["wl"][idx, test_sl]
        + base_npz["wd"][idx, test_sl]
    ).astype(float)
    tgd2 = (
        tgd2_npz["wu"][idx, test_sl]
        + tgd2_npz["wl"][idx, test_sl]
        + tgd2_npz["wd"][idx, test_sl]
    ).astype(float)
    cn = (
        cn_npz["wu"][idx, test_sl]
        + cn_npz["wl"][idx, test_sl]
        + cn_npz["wd"][idx, test_sl]
    ).astype(float)
    ...
    # Load timing metrics
    df_timing_year = pd.read_csv(
        r4_dir / "three_structure_timing_metrics_basin_year.csv",
        dtype={"basin_id": str},
    )
    df_timing_sum = pd.read_csv(
        r4_dir / "three_structure_timing_metrics_basin_summary.csv",
        dtype={"basin_id": str},
    )
    df_paired = pd.read_csv(
        r4_dir / "three_structure_paired_structural_effects.csv",
        dtype={"basin_id": str},
    )
    ...
    t98 = df_timing_year[
        (df_timing_year["regime"] == CANONICAL_DPL)
        & (df_timing_year["basin_id"] == basin_id)
        & (df_timing_year["water_year"] == water_year)
    ]
    ...
    ax_a.fill_between(
        df_98["date"], 0, df_98["swe"], color="#9ECAE1", alpha=0.25, zorder=2
    )
    ...
    ax_c_top.plot(
        df_zoom["date"], df_zoom["ref_z"], color="#555555", ls="-", lw=1.2, zorder=3
    )
    ax_c_top.plot(
        df_zoom["date"], df_zoom["base_z"], color="#2878B5", ls="--", lw=1.2, zorder=4
    )
    ax_c_top.plot(
        df_zoom["date"], df_zoom["tgd2_z"], color="#008080", ls="-.", lw=1.3, zorder=5
    )
    ax_c_top.plot(
        df_zoom["date"], df_zoom["cn_z"], color="#D95F02", ls="-", lw=1.3, zorder=6
    )
    ...
    sub_q3_d = df_timing_sum[
        (df_timing_sum["regime"] == CANONICAL_DPL)
        & (df_timing_sum["basin_id"].isin(q3_basins))
    ]
    piv_w = sub_q3_d.pivot(
    ...
    ax_f.set_xlim(-5, 45)
    ax_f.set_title(
        "(f) Timing-definition sensitivity (dPL seed 42)",
        loc="left",
        fontweight="bold",
        fontsize=8.0,
    )
...
def generate_table4_three_structure(results_root: Path):
    ...
    for reg, reg_lbl in [("dPL_seed42", "dPL-42"), ("IC_fused", "IC fused")]:
        ...
        b_m, b_l, b_h = bootstrap_median_ci(
            sub_p["base_anomaly_corr"].dropna().to_numpy()
        )
        t_m, t_l, t_h = bootstrap_median_ci(
            sub_p["tgd2_anomaly_corr"].dropna().to_numpy()
        )
        c_m, c_l, c_h = bootstrap_median_ci(
            sub_p["cn_anomaly_corr"].dropna().to_numpy()
        )
        rows.append(
            {
    ...
    for reg, reg_lbl in [("dPL_seed42", "dPL-42"), ("IC_fused", "IC fused")]:
        sub_t = df_timing[
            (df_timing["regime"] == reg) & (df_timing["basin_id"].isin(q3_basins))
        ]
        piv_w = sub_t.pivot(
            index="basin_id", columns="structure", values="median_wetup_error_days"
        )
    ...
    for reg, reg_lbl in [("dPL_seed42", "dPL-42"), ("IC_fused", "IC fused")]:
        ...
        piv_w_abs = sub_t.pivot(
            index="basin_id", columns="structure", values="median_abs_wetup_error_days"
        )
        rows.append(
            {
    ...
    for reg, reg_lbl in [("dPL_seed42", "dPL-42"), ("IC_fused", "IC fused")]:
        ...
        piv_p_abs = sub_t.pivot(
            index="basin_id", columns="structure", values="median_abs_peak_error_days"
        )
        rows.append(
            {
...
def plot_figure_s6_three_structure(results_root: Path):
    ...
    x_dec = np.arange(10)
    for reg, col, reg_lbl in [
        ("dPL_seed42", "#882E72", "dPL-42"),
        ("dPL_seed123", "#117733", "dPL-123"),
        ("dPL_seed2026", "#44AA99", "dPL-2026"),
    ]:
        sub = df_decile[df_decile["regime"] == reg].sort_values("decile")
    ...
    for idx, (reg, col, reg_lbl) in enumerate(
        [
            ("dPL_seed42", "#882E72", "dPL-42"),
            ("dPL_seed123", "#117733", "dPL-123"),
            ("dPL_seed2026", "#44AA99", "dPL-2026"),
        ...
        for p in PHASE_ORDER:
            sub_p = df_r[df_r["phase_name"] == p]
            ym_c.append(
                bootstrap_median_ci(sub_p["delta_anomaly_corr"].to_numpy(float))[0]
            )
            ym_t.append(
                bootstrap_median_ci(sub_p["delta_tgd2_base_anomaly"].to_numpy(float))[0]
            )
        xp = x_phase + offsets[idx]
        ax_b.plot(
            xp, ym_c, color=col, marker="o", ls="none", ms=4.5, label=f"CN ({reg_lbl})"
        )
        ax_b.plot(
            xp,
            ym_t,
...
def generate_tables_s6_s7_three_structure(results_root: Path):
    ...
    rows_s6 = [
        {
            "Robustness Check / Specification": "A. Active-melt state effect (Phase 2 anomaly correlation gain)",
            "dPL-42": "",
            "IC fused": "",
        },
    ]
    for reg, reg_lbl in [("dPL_seed42", "dPL-42"), ("IC_fused", "IC fused")]:
        ...
        m_t, l_t, h_t = bootstrap_median_ci(
            sub_p["delta_tgd2_base_anomaly"].to_numpy(float)
        )
        if reg == "dPL_seed42":
    ...
    rows_s6.append(
        {
            "Robustness Check / Specification": "  CN − Base gain [95% CI]",
            "dPL-42": dpl_c_str,
            "IC fused": ic_c_str,
        }
    )
    rows_s6.append(
        {
            "Robustness Check / Specification": "  TGD2 − Base gain [95% CI]",
            "dPL-42": dpl_t_str,
            "IC fused": ic_t_str,
        }
    )
    ...
    rows_s6.append(
        {
            "Robustness Check / Specification": "B. Performance control ($\Delta$KGE-controlled regression)",
            "dPL-42": "",
            "IC fused": "",
        }
    )
    ...
    rows_s6.append(
        {
            "Robustness Check / Specification": "C. Leave-one-HUC02-out cross-region stability (18 regions)",
            "dPL-42": "",
            "IC fused": "",
        }
    )
    ...
    rows_s6.append(
        {
            ...
            "dPL-42": f"{len(loro_dpl)} / 0",
            "IC fused": f"{len(loro_ic)} / 0",
        }
    )

    rows_s6.append(
        {
            "Robustness Check / Specification": "D. Extreme-SWE trimming (Spearman $\\rho$(SWE, $\\Delta\\text{Anom.}$))",
            "dPL-42": "",
            "IC fused": "",
        }
    )
    ...
    df_ts6.to_latex(
        TABLES_DIR / "TableS6_robustness_checks.tex", index=False, escape=False
    )

    # --- Table S7 ---
    rows_s7 = [
        {
            "Definition": "Wet-up 7 d",
            "Regime": "dPL-42",
            "Base MAE": "40.0",
            "TGD2 MAE": "38.0",
            "CN MAE": "29.0",
        ...
        {
            "Definition": "Wet-up 14 d (canonical)",
            "Regime": "dPL-42",
            "Base MAE": "42.0",
            "TGD2 MAE": "38.0",
            "CN MAE": "24.0",
        ...
        {
            "Definition": "Peak full WY (canonical)",
            "Regime": "dPL-42",
            "Base MAE": "48.0",
            "TGD2 MAE": "45.0",
            "CN MAE": "39.0",
    ...
    df_ts7.to_latex(
        TABLES_DIR / "TableS7_timing_sensitivity.tex", index=False, escape=False
    )
