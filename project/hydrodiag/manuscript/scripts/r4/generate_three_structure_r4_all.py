...
from manuscript.scripts.shared.r1_plot_style import apply_clean_spines, setup_publication_style
from manuscript.scripts.r4.common import default_data_root, default_results_root, load_bundle, zfill8
from manuscript.scripts.r4.robustness_analysis import bootstrap_median_ci
from manuscript.scripts.r4.soil_analysis import calendar_month_anomaly

FIGURES_DIR = PROJECT / "manuscript" / "figures"
SUPP_FIGS_DIR = PROJECT / "manuscript" / "figures"
...
def assert_tgd2_provenance(results_root: Path) -> None:
    """Protect final-looking R4 assets until all canonical TGD2 seeds are complete."""
    root = (
        results_root
    ...
    for seed in (42, 123, 2026):
        ...
        if not all((seed_dir / name).exists() for name in TGD2_REQUIRED):
            incomplete.append(str(seed))
    if incomplete:
        raise RuntimeError(
            "TGD2_PENDING: refusing to write R4 three-structure assets; "
            f"incomplete seed(s): {', '.join(incomplete)}. "
            "Complete provenance first or use a separate explicit interim renderer."
        )


def main():
    results_root = default_results_root()
    assert_tgd2_provenance(results_root)
