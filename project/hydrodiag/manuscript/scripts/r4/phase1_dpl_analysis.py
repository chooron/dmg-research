...
from manuscript.scripts.r4 import OFFICIAL_OBSERVATION_TRAINED  # noqa: E402
...
from manuscript.scripts.r4.input_adapters import read_dpl_seed  # noqa: E402
...
def export_models(results_root: Path, data_root: Path, device: str, *, force: bool = False) -> None:
    ...
    for model in ("XAJ", "XAJ_CN"):
        for seed in DPL_SEEDS:
            ...
            manifest = export_run(
                ...
                provenance={"source_run": f"{DPL_RUN}/{model}/seed_{seed}",
                            "seed": seed, "trained_on": "observed discharge (R1/R2)"},
                device=device, batch=64, validate_subset=8, save_npz=True,
            )
            print(f"exported {run_id}: q_finite={manifest['q_finite']} arrays={manifest['arrays']}")
...
def timing_metrics_for_basin(
    ...
    for w in np.unique(wy):
        ...
        dep_err = (g_dep - s_dep) if (np.isfinite(s_dep) and np.isfinite(g_dep)) else np.nan
        rows.append({"peak_timing_error_days": peak_err, "depletion_timing_error_days": dep_err})
    if len(rows) < MIN_VALID_YEARS:
        return {"n_valid_years": len(rows), "peak_timing_error_days": np.nan,
                "depletion_timing_error_days": np.nan}
    return {
        "n_valid_years": len(rows),
        "peak_timing_error_days": float(np.nanmedian([r["peak_timing_error_days"] for r in rows])),
        "depletion_timing_error_days": float(np.nanmedian([r["depletion_timing_error_days"] for r in rows])),
    }


def consistency_for_basin(dates: np.ndarray, g: np.ndarray, swe: np.ndarray) -> dict[str, float]:
    """Daily-scale consistency stats on the test period."""
    ok = np.isfinite(g) & np.isfinite(swe)
    if int(ok.sum()) < MIN_ACTIVE_DAYS:
        return {"n_active_days": 0, "seasonal_corr": np.nan, "anomaly_corr": np.nan,
                "nrmse": np.nan, "bias_mm": np.nan}
    gg, ss = g[ok].astype(np.float64), swe[ok].astype(np.float64)
    active = (gg > 1e-6) | (ss > 1e-6)
    n_active = int(active.sum())
    if n_active < MIN_ACTIVE_DAYS:
        return {"n_active_days": n_active, "seasonal_corr": np.nan, "anomaly_corr": np.nan,
                "nrmse": np.nan, "bias_mm": np.nan}
    ga, sa = gg[active], ss[active]
    seasonal_corr = float(stats.pearsonr(ga, sa)[0]) if ga.std() > 0 and sa.std() > 0 else np.nan
    ...
    return {"n_active_days": n_active, "seasonal_corr": seasonal_corr,
            "anomaly_corr": anomaly_corr, "nrmse": nrmse, "bias_mm": float(np.mean(ga - sa))}
...
def theil_sen(x: np.ndarray, y: np.ndarray) -> dict[str, float]:
    ...
    if ok.sum() < 10 or np.unique(x[ok]).size < 5:
        return {"n": int(ok.sum()), "slope": np.nan, "intercept": np.nan, "rho": np.nan, "p": np.nan}
    rho, p = stats.spearmanr(x[ok], y[ok])
    slope, intercept, _, _ = stats.theilslopes(y[ok], x[ok])
    return {"n": int(ok.sum()), "slope": float(slope), "intercept": float(intercept),
...
def main() -> None:
    ...
    # ---- load runs ---------------------------------------------------------
    q = {model: {s: load_run_arrays(results_root, model, s)["q_full"][:, test_sl]
                 for s in DPL_SEEDS} for model in ("XAJ", "XAJ_CN")}
    g = {s: load_run_arrays(results_root, "XAJ_CN", s)["G"][:, test_sl] for s in DPL_SEEDS}
    from training.dpl.run_dpl_model import compute_kge_fp64

    kge = {model: {s: np.array([compute_kge_fp64(q[model][s][i], obs_test[i])
                                for i in range(len(basin_ids))]) for s in DPL_SEEDS}
           for model in ("XAJ", "XAJ_CN")}
    ...
    for s in DPL_SEEDS:
        ...
        for i, b in enumerate(basin_ids):
            rows_b.append({
                "basin_id": b, "seed": s,
                "kge_cn_test": kge["XAJ_CN"][s][i], "kge_base_test": kge["XAJ"][s][i],
    ...
    for s in DPL_SEEDS:
        ...
        summary_a[f"seed_{s}"] = {
            ...
            "n_with_depletion_timing": int(sub["depletion_timing_error_days"].notna().sum()),
            "median_depletion_timing_error_days": float(sub["depletion_timing_error_days"].median()),
            "frac_basins_peak_err_lt_15d": float((sub["peak_timing_error_days"].abs() <= 15).mean()),
        }
    ...
    for s in DPL_SEEDS:
        ...
        for axis in ("median_annual_max_swe_mm", "median_swe_positive_days", "frac_snow"):
            entry[axis] = theil_sen(sub[axis].to_numpy(), sub["delta_cn_minus_base"].to_numpy())
