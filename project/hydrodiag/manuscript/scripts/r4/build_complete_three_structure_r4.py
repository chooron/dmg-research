...
from manuscript.r4.soil_analysis import (
    BOOTSTRAP_ROUNDS,
    BOOTSTRAP_SEED,
...
def run_full_three_structure_analysis():
    ...
    swe_test = np.load(results_root / "r4_swe_reference_v1" / "swe_ensemble.npz")[
        "swe_median"
    ][:, test_sl].astype(np.float64)
    ...
    regime_configs = [
        ...
        (
            ...
            {
                "Base": results_root
                / "r4_official_dpl_XAJ_seed123/official_dpl_XAJ_seed123_full_arrays.npz",
                "CN": results_root
                / "r4_official_dpl_XAJ_CN_seed123/official_dpl_XAJ_CN_seed123_full_arrays.npz",
                "TGD2": results_root
                / "r4_official_dpl_XAJ_TGD2_seed123/official_dpl_XAJ_TGD2_seed123_full_arrays.npz",
            },
        ),
        (
            "dPL_seed2026",
            {
                ...
                / "r4_official_dpl_XAJ_CN_seed42/official_dpl_XAJ_CN_seed42_full_arrays.npz",
                "TGD2": results_root
                / "r4_official_dpl_XAJ_TGD2_seed2026/official_dpl_XAJ_TGD2_seed2026_full_arrays.npz",
            },
        ),
        (
            "IC_fused",
            {
                ...
                "CN": results_root
                / "r4_ic_fused_XAJ_CN/ic_fused_XAJ_CN_full_arrays.npz",
                "TGD2": results_root
                / "r4_ic_fused_XAJ_TGD2/ic_fused_XAJ_TGD2_full_arrays.npz",
            },
        ),
    ]
    ...
    for reg_name, path_dict in regime_configs:
                ...
                all_paired_rows.append(
                    {
                        ...
                        "delta_cn_base_anomaly": anom_corrs["CN"][i]
                        - anom_corrs["Base"][i],
                        "delta_tgd2_base_anomaly": anom_corrs["TGD2"][i]
                        - anom_corrs["Base"][i],
                        "delta_cn_tgd2_anomaly": anom_corrs["CN"][i]
                        - anom_corrs["TGD2"][i],
                    }
                )
        ...
        for i, b_id in enumerate(basin_ids):
            ...
            for p_name, p_code, p_mask_fn in PHASE_SPECS:
                ...
                base_anom_p = float(
                    np.corrcoef(
                        ref_anom_sub,
                        calendar_month_anomaly(w_dict["Base"][i], months_test)[mask],
                    )[0, 1]
                )
                cn_anom_p = float(
                    np.corrcoef(
                        ref_anom_sub,
                        calendar_month_anomaly(w_dict["CN"][i], months_test)[mask],
                    )[0, 1]
                )
                tgd2_anom_p = float(
                    np.corrcoef(
                        ref_anom_sub,
                        calendar_month_anomaly(w_dict["TGD2"][i], months_test)[mask],
                    )[0, 1]
                )
        ...
        swe_vals = np.array(
            [float(burden_df.loc[b, "median_annual_max_swe_mm"]) for b in basin_ids]
        )
    ...
    for (reg, struct, b_id), group in df_all_timing_year.groupby(
        ["regime", "structure", "basin_id"]
    ):
            ...
            all_timing_summary_rows.append(
                {
                    ...
                    "median_peak_error_days": float(np.median(peak_errs))
                    if len(peak_errs)
                    else np.nan,
                    "median_abs_peak_error_days": float(np.median(np.abs(peak_errs)))
                    if len(peak_errs)
                    else np.nan,
                    "iqr_peak_error_days": float(stats.iqr(peak_errs))
                    if len(peak_errs)
                    else np.nan,
                    "median_wetup_error_days": float(np.median(wetup_errs))
                    if len(wetup_errs)
                    else np.nan,
                    "median_abs_wetup_error_days": float(np.median(np.abs(wetup_errs)))
                    if len(wetup_errs)
                    else np.nan,
                    "iqr_wetup_error_days": float(stats.iqr(wetup_errs))
                    if len(wetup_errs)
                    else np.nan,
                }
            )
    ...
    df_all_basin.to_csv(
        r4_dir / "three_structure_basin_state_consistency.csv", index=False
    )
    df_all_paired.to_csv(
        r4_dir / "three_structure_paired_structural_effects.csv", index=False
    )
    df_all_phase.to_csv(
        r4_dir / "three_structure_process_phase_consistency.csv", index=False
    )
    df_all_decile.to_csv(r4_dir / "three_structure_swe_decile_shape.csv", index=False)
    df_all_timing_year.to_csv(
        r4_dir / "three_structure_timing_metrics_basin_year.csv", index=False
    )
    df_all_timing_summary.to_csv(
        r4_dir / "three_structure_timing_metrics_basin_summary.csv", index=False
    )

    print(
        "\nSUCCESS: All Three-Structure R4 Statistical & Robustness Tables Built!",
        flush=True,
    )
