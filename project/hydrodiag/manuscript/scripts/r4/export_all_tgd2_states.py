...
from manuscript.r4.state_export import continuous_forward, model_instances
...
def export_all():
    ...
    for seed in [42, 123, 2026]:
        dpl_dir = (
            results_root
            / f"dpl_camels_531_lite_v3_tgd2_dpl_audited/XAJ_TGD2/seed_{seed}"
        )
        ...
        for k, v in states.items():
            if np.isnan(v).any():
                print(
                    f"Defensive cleanup of NaNs in state {k} for dPL seed {seed} ({np.isnan(v).sum()} NaNs)"
                )
        ...
        print(
            f"Saved dPL seed {seed}: {npz_path} ({npz_path.stat().st_size / 1024 / 1024:.1f} MB)",
            flush=True,
        )
    ...
    for k, v in states.items():
        if np.isnan(v).any():
            print(
                f"Defensive cleanup of NaNs in state {k} for IC ({np.isnan(v).sum()} NaNs)"
            )
            states[k] = np.nan_to_num(v, nan=0.0)
    q_full = np.nan_to_num(q_full, nan=0.0)
    ...
    print(
        f"Saved IC fused: {ic_npz_path} ({ic_npz_path.stat().st_size / 1024 / 1024:.1f} MB)",
        flush=True,
    )
