...
TABLES_DIR = HERE.parents[2] / "tables"
...
def generate_tables(results_root: Path, out_dir: Path) -> None:
    ...
    for reg, reg_label in [
        ("dPL_seed42", "dPL (Seed 42)"),
        ("dPL_seed123", "dPL (Seed 123)"),
        ("IC_fused", "IC Fused (5x200, Sens.)"),
    ]:
    ...
    for reg, reg_label in [
        ...
        for p_code, p_name in [
            (1, "1. Snow Accumulation"),
            (2, "2. Active Melt / Recharge"),
            (3, "3. Post-Melt Transition"),
            (4, "4. Summer Dry-Down"),
        ]:
    ...
    (out_dir / "TableS4_process_phase_and_robustness.md").write_text(
        md_s4, encoding="utf-8"
    )
    df_tables4.to_latex(
        out_dir / "TableS4_process_phase_and_robustness.tex", index=False
    )

    print(
        f"Generated Manuscript Tables:\n  {out_dir / 'Table4_soil_state_consistency.md'}\n  {out_dir / 'TableS4_process_phase_and_robustness.md'}"
    )
