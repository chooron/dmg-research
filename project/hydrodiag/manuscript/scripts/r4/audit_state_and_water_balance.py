...
def scalar_params(specs: dict) -> dict[str, torch.Tensor]:
    return {
        name: torch.tensor([spec["default"]], dtype=DTYPE)
...
def hbv_et(precip, temp, pet, snowpack, meltwater, sm, p):
    ...
    melt = torch.minimum(
        torch.clamp(p["parCFMAX"] * (temp - p["parTT"]), min=0.0), snowpack
    )
    meltwater = meltwater + melt
    snowpack = snowpack - melt
    refreeze = torch.minimum(
        torch.clamp(p["parCFR"] * p["parCFMAX"] * (p["parTT"] - temp), min=0.0),
        meltwater,
    )
...
def run_hbv(
    ...
    residuals, trace = (
        [],
        {k: [] for k in ("SNOWPACK", "MELTWATER", "SM", "SUZ", "SLZ", "Q", "ET")},
    )
    for pr, pe, te in zip(forcing["precip"], forcing["pet"], forcing["temp"]):
        ...
        q, snow, melt, sm, suz, slz = _hbv_step(
            pr,
            te,
            pe,
            snow,
        ...
        for name, value in zip(
            ("SNOWPACK", "MELTWATER", "SM", "SUZ", "SLZ", "Q", "ET"),
            (snow, melt, sm, suz, slz, q, et),
        ):
...
def run_cemaneige(
    forcing: dict[str, np.ndarray],
) -> tuple[list[float], dict[str, list[float]], np.ndarray]:
...
def run_cemaneige_hyst(
    ...
    residuals, trace, liquid = (
        [],
        {k: [] for k in ("G", "eTG", "sca", "swe_max", "Q_liquid")},
        [],
    )
    for pr, te in zip(precip[0], temp[0]):
        old = float(G)
        ...
        residuals.append(float(pr - q) - (float(G) - old))
        for name, value in zip(
            ("G", "eTG", "sca", "swe_max", "Q_liquid"), (G, eTG, sca, swe_max, q)
        ):
            trace[name].append(float(value))
        liquid.append(float(q))
    return residuals, trace, np.asarray(liquid)
...
def run_gr4j(
    ...
    model = GR4J()
    s_prod, s_route, uh1, uh2 = model._init_states(
        1, torch.device("cpu"), DTYPE, x1=p["x1"], x3=p["x3"]
    )
    uh1_ord = compute_gr4j_uh_ordinates(p["x4"], model.UH1_MAX)[0]
    uh2_ord = compute_gr4j_uh_ordinates(p["x4"], model.UH2_MAX)[1]
    residuals, trace = (
        [],
        {k: [] for k in ("s_prod", "s_route", "uh_pending", "Q", "ET")},
    )

    def storage():
        return tensor_sum(s_prod, s_route, uh1[:, 1:].sum(), uh2[:, 1:].sum())

    for pr, pe in zip(forcing["precip"], forcing["pet"]):
        pr, pe = (torch.tensor([x], dtype=DTYPE) for x in (pr, pe))
        old = storage()
        et = gr4j_et(pr, pe, s_prod, p)
        q, s_prod, s_route, uh1, uh2 = _gr4j_step(
            pr,
            pe,
            s_prod,
            s_route,
        ...
        for name, value in (
            ("s_prod", s_prod),
            ("s_route", s_route),
            ("uh_pending", uh1[:, 1:].sum() + uh2[:, 1:].sum()),
            ("Q", q),
            ("ET", et),
        ):
            trace[name].append(float(value))
...
def run_xaj(
    ...
    model = XAJ()
    wu, wl, wd, s, fr, qi, qg, _buf = model._init_states(
        1,
        torch.device("cpu"),
        DTYPE,
        um=p["xaj_um"],
    ...
    if float(ki + kg) >= 1.0:
        ki, kg = ki * 0.99 / (ki + kg), kg * 0.99 / (ki + kg)
    ords = _gamma_uh_ordinates(
        p["xaj_a"], p["xaj_theta"], XAJ_UH_MAX_LEN, torch.device("cpu"), DTYPE
    )[0].numpy()
    history: list[float] = []
    residuals, trace = (
        [],
    ...
    def storage():
        ...
        linear = p["xaj_ci"] * qi / (1.0 - p["xaj_ci"]) + p["xaj_cg"] * qg / (
            1.0 - p["xaj_cg"]
        )
    ...
    for pr, pe in zip(forcing["precip"], forcing["pet"]):
        ...
        history.append(float(rs_adj))
        history = history[-XAJ_UH_MAX_LEN:]
        routed = sum(
            value * float(ords[len(history) - 1 - idx])
            for idx, value in enumerate(history)
        )
        ...
        values = (
            ...
            wd,
            fr * s,
            p["xaj_ci"] * qi / (1.0 - p["xaj_ci"]),
            p["xaj_cg"] * qg / (1.0 - p["xaj_cg"]),
            torch.tensor([uh_pending(history, ords)]),
            torch.tensor([q]),
            evap_basin,
        )
...
def plot_trace(
    output: Path, model: str, basin: str, trace: dict[str, list[float]]
) -> None:
    fig, ax = plt.subplots(figsize=(12, 5))
    for name, values in trace.items():
        if name not in {"ET", "Q", "Q_liquid"}:
            ax.plot(values, linewidth=0.8, label=name)
    ax.set(
        title=f"{model}: state evolution, basin {basin}",
        xlabel="day",
        ylabel="mm or dimensionless",
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_DIR / "outputs" / "precalibration_integrity_audit",
    )
    args = parser.parse_args()
    args.output_dir.mkdir(parents=True, exist_ok=True)
    data = np.load(ROOT_DIR / "data" / "camels_dataset_petv2.npz", allow_pickle=True)
    dates = np.asarray(
        np.load(ROOT_DIR / "data" / "camels_dates.npy"), dtype="datetime64[D]"
    )
    basin_ids = [
        str(int(x)).zfill(8) for x in np.load(ROOT_DIR / "data" / "gage_id.npy")
    ]
    indices = [basin_ids.index(basin) for basin in SELECTED_BASINS]
    mask = (dates >= np.datetime64("1988-01-01")) & (
        dates <= np.datetime64("1998-12-31")
    )
    forcing_all = data["forcing"][mask]
    reports = {}
    for basin, idx in zip(SELECTED_BASINS, indices):
        forcing = {
            "precip": forcing_all[:, idx, 0].astype(np.float64),
            "temp": forcing_all[:, idx, 1].astype(np.float64),
            "pet": forcing_all[:, idx, 2].astype(np.float64),
        }
        ...
        gr_cn_trace = {
            **{f"cn_{k}": v for k, v in cn_trace.items()},
            **{f"gr4j_{k}": v for k, v in gr_cn_trace.items()},
        }
        xaj_cn_trace = {
            **{f"cn_{k}": v for k, v in cn_trace.items()},
            **{f"xaj_{k}": v for k, v in xaj_cn_trace.items()},
        }
        reports[basin] = {
            ...
            "CemaNeige": balance_summary(
                "CemaNeige", cn_res, cn_trace, forcing["precip"]
            ),
            "CemaNeigeHyst": balance_summary(
                "CemaNeigeHyst", hyst_res, hyst_trace, forcing["precip"]
            ),
            "GR4J_CN": balance_summary(
                "GR4J+CemaNeige", gr_cn_res, gr_cn_trace, forcing["precip"]
            ),
            "XAJ_CN": balance_summary(
                "XAJ+CemaNeige", xaj_cn_res, xaj_cn_trace, forcing["precip"]
            ),
        }
        for model, trace in (
            ("HBV", hbv_trace),
            ...
            ("XAJ", xaj_trace),
            ("CemaNeige", cn_trace),
            ("CemaNeigeHyst", hyst_trace),
            ("GR4J_CN", gr_cn_trace),
            ("XAJ_CN", xaj_cn_trace),
        ):
            plot_trace(
                args.output_dir / f"{basin}_{model}_states.png", model, basin, trace
            )
