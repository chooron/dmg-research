...
def evaluate_all_starts():
    ...
    for model_key in ["XAJ", "XAJ_CN", "XAJ_TGD"]:
        ...
        files = glob.glob(
            os.path.join(out_dir, f"**/seed_101/**/result.json"), recursive=True
        )
        ...
        for f in files:
            try:
                ...
                if (
                    b_id in all_531
                    and s_id in [0, 1, 2]
                    and tr_kge is not None
                    and theta is not None
                ):
                    start_records[s_id][b_id] = {"train_kge": tr_kge, "theta": theta}
            except Exception:
                pass
        ...
        for s_id in [0, 1, 2]:
            ...
            theta_matrix = torch.tensor(
                thetas, dtype=torch.float32, device=cfg["device"]
            ).unsqueeze(1)

            eval_res = runtime.evaluate_candidates(
                theta_matrix, basin_indices=b_indices, split="test"
            )
        ...
        for b_id in all_531:
            tr_vals = [
                start_eval_results[s][b_id]["train_kge"]
                for s in [0, 1, 2]
                if b_id in start_eval_results[s]
            ]
            te_vals = [
                start_eval_results[s][b_id]["test_kge"]
                for s in [0, 1, 2]
                if b_id in start_eval_results[s]
            ]
        ...
        print(
            f"   Train KGE - Median: {valid_df['median_train_kge'].median():.4f} | Mean: {valid_df['median_train_kge'].mean():.4f}"
        )
        print(
            f"   Test  KGE - Median: {valid_df['median_test_kge'].median():.4f} | Mean: {valid_df['median_test_kge'].mean():.4f}\n"
        )
