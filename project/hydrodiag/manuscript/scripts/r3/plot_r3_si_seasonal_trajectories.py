#!/usr/bin/env python3
"""Figure S6 (R3 Supplement): Seasonal process delivery and storage deviation diagnostics.
...
OUT_NAME = "Fig_S6_R3_seasonal_trajectories"
...
def build_figure(summary, out_dir: Path | None = None) -> Path:
    ...
    print(f"Saved SI Figure S6 -> {out_path} ({file_size_mb:.2f} MB)", flush=True)
    return out_path


def main():
    parser = argparse.ArgumentParser(description="Render R3 Supplement Figure S6.")
