...
Usage: python manuscript/scripts/r3/generate_table_r3_si.py
"""
...
OUT_DIRS = (
    ...
    PROJECT / "manuscript" / "stats" / "tables",
)
...
def boot_ci_median(values: np.ndarray, n_boot=None, seed=None, alpha: float = 0.05):
...
def assert_close(got: float, expect: float, tol: float, label: str) -> None:
    ...
    if abs(got - expect) > tol:
        raise SystemExit(
            f"assertion FAILED ({label}): {got:.10f} != frozen {expect:.10f}"
        )
...
def block2_rows(bt, red, resv, proc):
    ...
    for reg in ["IC", "dPL"]:
        ...
        gc[reg] = (
            float(np.median(v)),
            int(len(v)),
            boot_ci_median(v, seed=BOOT_SEED + 4),
        )
    ...
    for reg, fits in [
        ...
        for cond in ["snow_active", "no_snow_active"]:
            ...
            for b in proc_test["basin_id"].unique():
                ...
                for pair in fits:
                    ...
                    cn_r = proc_test[
                        ...
                        & (proc_test["fit"] == fit_cn)
                        & (proc_test["condition"] == cond)
                    ]
                    tg_r = proc_test[
                        (proc_test["basin_id"] == b)
                        & (proc_test["fit"] == fit_tg)
                        & (proc_test["condition"] == cond)
                    ]
            ...
            dproc[reg][cond] = (
                float(np.median(v)),
                int(len(v)),
                boot_ci_median(v, seed=BOOT_SEED + 30),
            )
    ...
    for reg in ["IC", "dPL"]:
        ...
        dg[reg] = (
            float(np.median(v)),
            int(len(v)),
            boot_ci_median(v, seed=BOOT_SEED + 2),
        )
...
def main() -> None:
    ...
    for reg in ["IC", "dPL"]:
        ...
        b1.append(
            {
                "block": 1,
                "estimand": "CN parameter recovery D_theta",
                "regime": reg,
                "n": None,
                "median": dtheta,
                "note": "gate equifinality median",
            }
        )
        for st in ["wu", "wl", "s", "qi", "qg"]:
            ...
            b1.append(
                {
                    "block": 1,
                    "estimand": f"CN state NRMSE [{st}]",
                    "regime": reg,
                    "n": None,
                    "median": float(gss[st][key]["nrmse_median"]),
    ...
    est2 = [
        ...
        (
            "G_CN_over_TGD2_test",
            "Residual CN advantage: G_CN_over_TGD2 (test)",
            "G_CN",
            "panel_e",
        ),
    ...
    seed_specs = [
        (
            "F_close",
            "F_close (test)",
            lambda s: ps[f"dPL_{s}_test"]["F_close"]["median"],
        ),
        ("F_tgd2", "F_tgd2 (test)", lambda s: ps[f"dPL_{s}_test"]["F_tgd2"]["median"]),
        (
            "R_theta",
            "R_theta_tgd2",
            lambda s: pv["V3"][f"dPL_{s}"]["R_theta_tgd2"]["median"],
        ),
        (
            "R_state",
            "R_state_tgd2",
            lambda s: pv["V3"][f"dPL_{s}"]["R_state_tgd2"]["median"],
        ),
        (
            "G_CN",
            "G_CN_over_TGD2 (test)",
            lambda s: pv["V4"][f"dPL_{s}_test"]["G_CN_over_TGD2"]["median"],
        ),
    ...
    assert_close(
        [
            r
            for r in b2
            if r["estimand"] == "Limited compensation: F_close (test)"
            and r["regime"] == "IC"
        ][0]["median"],
        f5c["panel_c_f_close"]["IC_test"]["median"],
        1e-6,
        "F_close IC",
    )
    assert_close(
        ...
        f5c["panel_c_f_close"]["dPL_test"]["median"],
        1e-6,
        "F_close dPL",
    )
    assert_close(
        [
            r
            for r in b2
            if r["estimand"] == "Generalization decay: decay_G_base"
            and r["regime"] == "IC"
        ][0]["median"],
        f5c["panel_d_decay"]["IC"]["median"],
        1e-6,
        "decay IC",
    )
    assert_close(
        ...
        f5c["panel_d_decay"]["dPL_agg"]["median"],
        1e-6,
        "decay dPL",
    )
    assert_close(
        [
            r
            for r in b2
            if r["estimand"] == "Generic mitigation: F_tgd2 (test)"
            and r["regime"] == "IC"
        ][0]["median"],
        f6c["panel_b_f_tgd2"]["IC"]["median"],
        1e-6,
        "F_tgd2 IC",
    )
    assert_close(
    ...
    assert_close(
        [
            r
            for r in b2
            if r["estimand"] == "Parameter relief: R_theta_tgd2" and r["regime"] == "IC"
        ][0]["median"],
        f6c["panel_c_r_theta"]["IC"]["median"],
        1e-6,
        "R_theta IC",
    )
    assert_close(
        [
            r
            for r in b2
            if r["estimand"] == "Parameter relief: R_theta_tgd2"
            and r["regime"] == "dPL"
        ][0]["median"],
        f6c["panel_c_r_theta"]["dPL"]["median"],
        1e-6,
        "R_theta dPL",
    )
    assert_close(
        [
            r
            for r in b2
            if r["estimand"] == "State relief: R_state_tgd2" and r["regime"] == "IC"
        ][0]["median"],
        f6c["panel_d_r_state"]["IC"]["median"],
        1e-6,
        "R_state IC",
    )
    assert_close(
        [
            r
            for r in b2
            if r["estimand"] == "State relief: R_state_tgd2" and r["regime"] == "dPL"
        ][0]["median"],
        f6c["panel_d_r_state"]["dPL"]["median"],
        1e-6,
        "R_state dPL",
    )
    assert_close(
        [
            r
            for r in b2
            if r["estimand"] == "Residual CN advantage: G_CN_over_TGD2 (test)"
            and r["regime"] == "IC"
        ][0]["median"],
        f6c["panel_e_residual_vs_frac_snow"]["IC"]["median"],
        1e-6,
        "G_CN IC",
    )
    assert_close(
        ...
        f6c["panel_e_residual_vs_frac_snow"]["dPL"]["median"],
        1e-6,
        "G_CN dPL",
    )
    for cond in ["snow_active", "no_snow_active"]:
        for reg in ["IC", "dPL"]:
            assert_close(
                [
                    r
                    for r in b2
                    if f"Process residual RMSE gap [{cond}]" in r["estimand"]
                    and r["regime"] == reg
                ][0]["median"],
                f6c["panel_f_process_errors"][reg][cond]["median"],
                1e-6,
                f"dRMSE {reg} {cond}",
            )
    ...
    for r in b3:
        fn = {
            "F_close": lambda s: ps[f"dPL_{s}_test"]["F_close"]["median"],
            ...
            "decay": lambda s: pv[f"dPL_{s}_decay_G_base"]["median"],
        }[r["short"]]
        assert_close(
            r["median"], float(fn(r["seed"])), 1e-9, f"seed {r['short']} s{r['seed']}"
        )

    print(
        f"[check] Block-2 medians match Figure 5/6 summaries (1e-6): OK "
        f"({sum(1 for r in b2 if r['block'] == 2)} rows)"
    )
    print(
        f"[check] Block-3 seed medians match frozen posthoc summaries (1e-9): OK "
        f"({len(b3)} rows)"
    )
    ...
    for r in b1:
        md_parts.append(
            f"| {r['regime']} | {r['estimand']} | {r['median']:.4f} | {r['note']} |\n"
        )
    ...
    for r in b2:
        md_parts.append(
            f"| {r['estimand']} | {r['regime']} | {r['n']} | {fmt_stat(r['median'], r['ci_low'], r['ci_high'])} |\n"
        )
...
\end{table*}""")
    ...
    print(
        f"  md/tex: manuscript/tables/ + manuscript/stats/tables/TableS5_R3_statistics.*"
    )
    print(f"  csv:    {PREP / 'tableS5_si_statistics.csv'}")
    print(
        f"  rows per block: B1={len(b1)} B2={len(b2)} B3={len(b3)} B4={len(b4)} B5={len(b5)}"
    )
