...
def main() -> None:
    ...
    for model_name in MODELS:
        ...
        if (
            model_name.endswith("_TGD")
            and config.get("tgd_structure_version") != TGD_STRUCTURE_VERSION
        ):
            raise RuntimeError(
        ...
        basin_ids, raw_attrs, train_forcing, train_obs, test_forcing, test_obs = (
            load_data(config, indices, None)
        )
        ...
        evaluate_args = (
            ...
            device,
            int(config["window"]["warmup_days"]),
        )
        train_kge, _, _ = evaluate(
            *evaluate_args[:4], train_forcing, train_obs, *evaluate_args[4:]
        )
        test_kge, _, _ = evaluate(
            *evaluate_args[:4], test_forcing, test_obs, *evaluate_args[4:]
        )
        ...
        rows.append(row)
        with (output_dir / "train_test_kge_by_basin.csv").open(
            "w", newline=""
        ) as handle:
            writer = csv.DictWriter(
                handle, fieldnames=["basin_id", "train_kge", "test_kge"]
            )
            writer.writeheader()
            for basin_id, train_value, test_value in zip(
                basin_ids, train_kge, test_kge
            ):
                writer.writerow(
                    {
                        "basin_id": basin_id,
