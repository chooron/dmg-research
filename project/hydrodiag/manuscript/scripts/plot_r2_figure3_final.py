...
REGIME_XTICK_SHORT = [f"S{i + 1}" for i in range(len(REGIMES))]  # sample sizes in caption
...
def regime_series(df_s, paradigm, contrast, metric):
    meds, lows, highs = [], [], []
    for reg in REGIMES:
        row = df_s[(df_s["paradigm"] == paradigm) & (df_s["contrast"] == contrast)
                   & (df_s["stratum"] == reg) & (df_s["metric"] == metric)]
...
def slope_ci(df_r, paradigm, contrast, stratum="Full531"):
    row = df_r[(df_r["paradigm"] == paradigm) & (df_r["contrast"] == contrast)
               & (df_r["stratum"] == stratum) & (df_r["dependent_var"] == "excess")]
    assert len(row) == 1, f"missing slope {paradigm} {contrast} {stratum}"
    return float(row["slope"].iloc[0]), float(row["slope_ci_lower"].iloc[0]), \
...
def panel_ab_scatter(ax, df_b, paradigm, frac_cn, frac_tg, title, lim):
    ...
    for contrast, alpha in [(CN_PRIMARY, 0.34), (CN_TGD2, 0.26)]:
        ...
        ax.scatter(sub["within_pooled"], sub["between_all"], s=8, alpha=alpha,
                   color=st["color"], marker=st["mk"], edgecolors="none",
                   linewidths=0, zorder=3)
    ax.plot([0, lim], [0, lim], color=COLOR_REF, linestyle="--", linewidth=1.0, zorder=1)
    ...
    ax.text(0.98, 0.03,
            ...
            fontsize=7.0, fontweight="normal", linespacing=1.5)
...
def _facet_excess(ax, df_b, df_s, df_r, paradigm):
    ...
    for contrast, sgn in [(CN_PRIMARY, -x_idx), (CN_TGD2, +x_idx)]:
        ...
        ax.scatter(x, y, s=4, alpha=0.11, color=st["color"], marker=st["mk"],
        ...
        ax.plot(x_line, a + slope * x_line, color=st["color"], linestyle=st["ls"],
                linewidth=1.4, zorder=4)
        # slope-uncertainty wedge anchored at the sample centroid
        mx, my = float(x.mean()), float(y.mean())
        ax.fill_between(x_line, my + lo * (x_line - mx), my + hi * (x_line - mx),
                        color=st["color"], alpha=0.15, zorder=1, linewidth=0)
        ...
        bfrac = [float(df_b[(df_b["paradigm"] == paradigm)
                            & (df_b["snow_regime"] == reg)]["frac_snow"].median())
                 for reg in REGIMES]
        bx = np.asarray(bfrac) + sgn
        ax.errorbar(bx, bmeds, yerr=[bmeds - blows, bhighs - bmeds],
                    fmt=st["mk"], color=st["color"], ecolor=st["color"],
                    elinewidth=1.3, capsize=2.5, capthick=1.0, markersize=7.0,
    ...
    for bnd in REGIME_BOUNDS:  # subtle S1–S5 bin boundary guides
        ax.axvline(bnd, color=COLOR_REF, linestyle=":", linewidth=0.6, alpha=0.4,
                   zorder=1)
    ax.set_xlim(0.0, 1.0)
    ...
    for reg, bfrac in zip(REGIMES, [0.025, 0.10, 0.225, 0.40, 0.75]):
        ax.text(bfrac, 0.965, reg, transform=ax.transAxes, ha="center", va="top",
...
def panel_c_hero(ax_ic, ax_dpl, df_b, df_s, df_r):
    ...
    ax_ic.set_title("(c) Snow-dependent structural separation", weight="bold",
                    loc="left", pad=6)
    ...
    for ax, paradigm in [(ax_ic, "IC"), (ax_dpl, "dPL")]:
        ...
        ax.text(0.02, 0.93, f"{paradigm} regime", transform=ax.transAxes,
                ha="left", va="top", fontsize=8.0, fontweight="bold", color="#333333")
        ylims[paradigm] = (ymin, ymax)
    # one shared hero legend (IC facet, below the facet header)
    handles = [
        Line2D([0], [0], marker="s", color=COLOR_BETWEEN, linestyle="-",
               markersize=5.0, label="Base–CN"),
        Line2D([0], [0], marker="^", color=COLOR_TGD2, linestyle="--",
               markersize=5.0, label="Base–TGD2"),
    ]
    ax_ic.legend(handles=handles, loc="upper left", bbox_to_anchor=(0.02, 0.78),
...
def panel_e_slopes(ax, df_r):
    ...
    for yy, paradigm, stratum, label in rows:
        for contrast, sgn in [(CN_PRIMARY, +1.0), (CN_TGD2, -1.0)]:
            ...
            y = yy + sgn * dy
            ax.errorbar(slope, y, xerr=[[slope - lo], [hi - slope]],
                        fmt=st["mk"], color=st["color"], ecolor=st["color"],
                        elinewidth=1.3, capsize=2.5, capthick=1.0,
                        markersize=5.5, markerfacecolor=st["color"],
                        markeredgewidth=1.0, zorder=3)
            ax.text(hi + 0.015, y, f"{slope:+.3f}", ha="left", va="center",
    ...
    handles = [
        Line2D([0], [0], marker="s", color=COLOR_BETWEEN, markersize=5.5,
               linestyle="none", label="Base–CN"),
        Line2D([0], [0], marker="^", color=COLOR_TGD2, markersize=5.5,
               linestyle="none", label="Base–TGD2"),
    ]
    ax.legend(handles=handles, loc="lower left", frameon=True, framealpha=0.90,
              edgecolor="none", fontsize=6.4)
...
def build_figure(df_b, df_s, df_r) -> None:
    ...
    panel_ab_scatter(ax_a, df_b, "IC", f_ic_cn, f_ic_tg,
                     "(a) Structural separation under IC", lim=0.75)
...
def main() -> None:
    ...
    for paradigm, contrast, exp in [
        ("IC", "Base-CN", 0.6309), ("IC", "Base-TGD2", 0.6064),
        ("dPL", "Base-CN", 0.8380), ("dPL", "Base-TGD2", 0.8230),
    ]:
        sub = df_b[(df_b["paradigm"] == paradigm) & (df_b["contrast"] == contrast)]
