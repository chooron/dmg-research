...
def panel_a(ax, seedmed, summary):
    ...
    for reg, period, ls, lw, alpha, zo in styles:
        ...
        x, y = ecdf(d)
        ax.step(x, y, where="post", color=C_CN, linestyle=ls, linewidth=lw,
    ...
    handles = [
        ...
        Line2D([0], [0], color=C_CN, linestyle=(0, (4.0, 2.0)), linewidth=1.6,
               label="dPL"),
        Line2D([0], [0], color=C_CN, linestyle="-", linewidth=1.0, alpha=0.45,
               label="train"),
        Line2D([0], [0], color=C_CN, linestyle="-", linewidth=1.8, label="test"),
    ]
    ax.legend(handles=handles, loc="upper left", bbox_to_anchor=(0.0, 1.0),
              frameon=True, framealpha=0.90, edgecolor="none", fontsize=7.6,
    ...
    ax.text(0.97, 0.95,
            ...
            color=C_TEXT, linespacing=1.3, bbox=TEXT_BG)
...
def _facet_gap_ecdf(ax, seedmed, reg):
    ...
    series = [
        ("Base no-refit", te["kge_base_no_refit"].to_numpy(), C_NOREFIT, (0, (4.0, 2.0)), 1.5),
    ...
    ax.text(0.98, 0.97, "median 0.898 | 0.899 | 0.993" if reg == "IC"
...
def panel_b(ax_ic, ax_dpl, seedmed):
    ax_ic.set_title("(b) Output-level structural gap (test)",
    ...
    handles = [
        Line2D([0], [0], color=C_NOREFIT, linestyle=(0, (4.0, 2.0)),
               linewidth=1.5, label="Base no-refit"),
        Line2D([0], [0], color=C_BASE, linestyle="-", linewidth=1.6,
               label="Base fitted"),
    ...
    ax_ic.legend(handles=handles, loc="lower left", bbox_to_anchor=(0.02, 0.04),
                 frameon=True, framealpha=0.95, edgecolor="none", fontsize=7.8)
...
def _group_summary(ax, xi, vals, entry, reg, period, rng):
    ...
    ax.scatter(jx, win, s=7, alpha=0.28, marker=marker, color=C_BASE,
               facecolors=face, edgecolors=C_BASE, linewidths=0.4, zorder=2)
    ...
    ax.add_patch(plt.Rectangle((xi - bw / 2, q25), bw, q75 - q25,
                               facecolor=C_BASE, edgecolor=C_BASE,
                               alpha=0.35, linewidth=0.8, zorder=3))
    ax.plot([xi - bw / 2, xi + bw / 2], [med, med], color=C_BASE,
...
def panel_c(ax, seedmed, summary):
    ...
    ax.set_title("(c) Gap-closure fraction $F_{close}$",
                 weight="bold", loc="left", pad=5)
        ...
        if reg == "dPL":
            for s, smed in enumerate(entry["seed_medians"]):
                ax.plot(xi + 0.24 + 0.03 * s, smed, marker="D", markersize=2.6,
                        color=C_BASE_IC, markerfacecolor="none", markeredgewidth=0.7,
                        zorder=5)
        ax.text(xi, 0.98, f"{med:.3f}", ha="center", va="top", fontsize=8.0,
    ...
    ax.text(0.01, 1.01, "full closure", ha="left", va="bottom", fontsize=7.2,
            color=C_REF)
    ax.set_xlim(-0.55, 3.75)
    ax.set_ylim(-0.80, 1.30)
    ax.set_xticks(xs)
    n_vals = [str(pc[f"{r}_{p}"]["n_valid"]) for r, p in groups]
    ax.set_xticklabels([f"IC train\nn = {n_vals[0]}", f"IC test\nn = {n_vals[1]}",
                        f"dPL train\nn = {n_vals[2]}", f"dPL test\nn = {n_vals[3]}"],
                       fontsize=8.1)
    ...
    handles = [
        Line2D([0], [0], marker="o", color=C_BASE, linestyle="none",
               markerfacecolor=C_BASE, markersize=4.5, label="train"),
        Line2D([0], [0], marker="o", color=C_BASE, linestyle="none",
               markerfacecolor="white", markeredgecolor=C_BASE, markersize=4.5,
               label="test"),
    ]
    ax.legend(handles=handles, loc="upper right", bbox_to_anchor=(0.995, 0.97),
              frameon=True, framealpha=0.92, edgecolor="none", fontsize=7.2,
...
def panel_d(ax, seedmed, summary):
    ...
    ax.set_title("(d) Train-to-test compensation decay",
                 weight="bold", loc="left", pad=5)
    rng = np.random.default_rng(20260730)
    for xi, reg_name in enumerate(["IC", "dPL"]):
        ...
        ax.errorbar([xi], [med], yerr=[[med - lo], [hi - med]], fmt="none",
                    ecolor=C_TEXT, elinewidth=1.2, capsize=2.6, capthick=1.1,
                    zorder=5)
        ax.text(xi, 0.180, f"+{med:.4f}", ha="center", va="bottom",
                fontsize=8.0, color=C_TEXT, fontweight="bold")
        ax.text(xi, 0.152, f"{agg['frac_gt_0'] * 100:.0f} % > 0",
                ha="center", va="bottom", fontsize=7.0, color="#666666")
    ax.axhline(0.0, color=C_REF, linestyle="--", linewidth=1.0, zorder=1)
    ax.text(1.42, 0.008, "train = test", ha="right", va="bottom", fontsize=7.2,
            color=C_REF)
    ...
    ax.grid(True, axis="y", linestyle=":", alpha=0.25)
    ax.text(0.99, 0.02, "$\\approx$1–4 % of basins beyond window",
            transform=ax.transAxes, ha="right", va="bottom", fontsize=6.8,
            color="#666666")
...
def _plot_merged_excess(ax, seedmed, metric, pef_ic, pef_dpl, show_legend=False):
    ...
    if show_legend:
        handles = [
            Line2D([0], [0], marker="o", color=C_BASE_IC, linestyle="-",
                   linewidth=1.5, markersize=5.0, markerfacecolor=C_BASE_IC,
                   markeredgecolor="white", markeredgewidth=0.5, label="IC (CMA-ES)"),
            Line2D([0], [0], marker="^", color=C_BASE, linestyle="--",
                   linewidth=1.5, markersize=5.5, markerfacecolor=C_BASE,
    ...
    if frac_out > 0:
        ax.text(0.98, 0.17, f"{frac_out * 100:.1f} % of basins beyond axis",
...
def panels_ef(ax_e, ax_f, seedmed, summary):
    ...
    ax_e.set_title("(e) Parameter excess error vs. snow activity",
                   weight="bold", loc="left", pad=5)
    ax_f.set_title("(f) State excess error vs. snow activity",
                   weight="bold", loc="left", pad=5)

    _plot_merged_excess(ax_e, seedmed, "C_theta", pef["C_theta_IC"], pef["C_theta_dPL"],
                        show_legend=True)
    _plot_merged_excess(ax_f, seedmed, "C_state", pef["C_state_IC"], pef["C_state_dPL"],
                        show_legend=False)
...
def build_figure(tidy, seedmed, summary) -> None:
    ...
    gs1 = gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec=gs[0],
                                           width_ratios=[0.28, 0.72], wspace=0.16)
    ax_a = fig.add_subplot(gs1[0, 0]); apply_clean_spines(ax_a)
    gsb = gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec=gs1[0, 1],
                                           wspace=0.14)
    ...
    ax_b1 = fig.add_subplot(gsb[0, 0], sharey=ax_a); apply_clean_spines(ax_b1)
    ax_b2 = fig.add_subplot(gsb[0, 1], sharey=ax_a); apply_clean_spines(ax_b2)
    ...
    gs2 = gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec=gs[1],
                                           width_ratios=[0.50, 0.50], wspace=0.12)
    ax_c = fig.add_subplot(gs2[0, 0]); apply_clean_spines(ax_c)
    ax_d = fig.add_subplot(gs2[0, 1]); apply_clean_spines(ax_d)
    ...
    gs3 = gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec=gs[2],
                                           width_ratios=[0.50, 0.50], wspace=0.12)
    ax_e = fig.add_subplot(gs3[0, 0]); apply_clean_spines(ax_e)
    ax_f = fig.add_subplot(gs3[0, 1]); apply_clean_spines(ax_f)
