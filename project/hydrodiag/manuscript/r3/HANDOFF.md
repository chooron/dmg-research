...
`project/hydrodiag/manuscript/r3/README.md` and `manuscript/r3/docs/kge_audit.md`.
...
- **Worktree (code)**: `/home/jingxin/orca/workspaces/dmg-research/hydrodiag-r3`
  ...
  6 modified files + `manuscript/r3/` + 2 test files + `r3_remote_chain.py` +
...
- Periods: warmup 1980-10-01..1981-09-30, train ..1995-09-30, test ..2010-09-30.
  Metric: standard KGE everywhere (see `manuscript/r3/docs/kge_audit.md`).
...
1. Wait for the pilot's last dPL run to finish (XAJ_TGD2 seed 2026); then
   `manuscript/r3/pilot.py --stage comp-dpl` will complete if needed, else just note
   COMPLETE markers; run `manuscript/r3/analyze_pilot.py` (optional; pilot is
   engineering-only).
2. **Run the 531 gate analysis**: `python manuscript/r3/gate_analysis.py --device cuda`
...
- **Import trap**: `manuscript/r3/common.DEFAULT_PROJECT_ROOT` must be the worktree
  (`Path(__file__).parents[1]`), NOT the main checkout — `load_bundle()`
...
- **KGE**: repo "KGE" is standard KGE everywhere (r/alpha/beta); the paper's
  ...
  recorded in `manuscript/r3/docs/kge_audit.md`, not silently changed.

## 7. Commands

```bash
cd /home/jingxin/orca/workspaces/dmg-research/hydrodiag-r3/project/hydrodiag
python manuscript/r3/gate_analysis.py --device cuda          # 531 gate report
python manuscript/r3/analyze_pilot.py --device cuda          # pilot gate outputs (after pilot finishes)
