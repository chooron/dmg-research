...
- CT/AMJJ water-year signatures: definitions from
  ``manuscript/scripts/r1/r1_statistics.py`` (water year starts Oct 1; CT is
...
# Canonical roots.
# The R3 *code* (models/, ablation/, training/, r3/) lives in this worktree;
# its project root is the parent of the r3 package.  Data and R1/R2 result
# artifacts live in the shared main checkout, referenced by the data and
# results roots below.  Keeping the code root separate from the data/results
# root guarantees the R3 process imports the patched local modules instead
# of the (unpatched) main-checkout copies.
DEFAULT_PROJECT_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_DATA_ROOT = Path("/home/jingxin/code/dmg-research/data")
...
def water_year_series(dates: np.ndarray, flow: np.ndarray) -> pd.DataFrame:
    ...
    Definition reused from ``manuscript/scripts/r1/r1_statistics.py``
