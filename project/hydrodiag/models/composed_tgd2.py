...
class _WithTGD2(BaseHydrologicalModel):
    ...
    def forward(
        ...
        validate_params(
            params, self.parameter_specs, precip.shape[0], device, precip.dtype
        )
        delay_params, runoff_params = self._split_params(params)
        delay_initial = (
            None
            if initial_states is None
            else {k[4:]: v for k, v in initial_states.items() if k.startswith("tgd_")}
        )
        runoff_initial = (
            None
            if initial_states is None
            else {
                k[len(self.runoff_prefix) :]: v
                for k, v in initial_states.items()
                if k.startswith(self.runoff_prefix)
            }
        )
        effective, delay_aux = self._delay(
            forcings, delay_params, delay_initial, return_states
        )
        ...
        if self.compact_output and not return_states:
            return qsim, {}
        aux: dict[str, Any] = {
            **{k: v for k, v in delay_aux.items() if k != "final_states"},
            **{k: v for k, v in runoff_aux.items() if k != "final_states"},
            "effective_precipitation": effective,
        }
        if return_states:
            aux["final_states"] = {
                **{f"tgd_{k}": v for k, v in delay_aux["final_states"].items()},
                **{
                    f"{self.runoff_prefix}{k}": v
                    for k, v in runoff_aux["final_states"].items()
                },
            }
        return qsim, aux
...
class GR4JWithTGD2(_WithTGD2):
    ...
    def __init__(self, nearzero: float = 1e-8):
        ...
        self._delay = TemperatureDependentGenericDelay2()
        self._runoff = GR4J(nearzero=nearzero)

    @property
    def parameter_specs(self):
        return GR4J_TGD2_PARAM_SPECS
...
class SIMHYDWithTGD2(_WithTGD2):
    ...
    @property
    def parameter_specs(self):
        return SIMHYD_TGD2_PARAM_SPECS


class GR4JWithTGD2Lite(GR4JWithTGD2):
    def __init__(self, nearzero: float = 1e-8):
        super().__init__(nearzero)
        self.compact_output = True
        self._delay = TemperatureDependentGenericDelay2(True)
        self._runoff = GR4JLite(nearzero)
...
class SIMHYDWithTGD2Lite(SIMHYDWithTGD2):
    def __init__(self, nearzero: float = 1e-8):
        ...
        self.compact_output = True
        self._delay = TemperatureDependentGenericDelay2(True)
        self._runoff = SIMHYDLite(nearzero)
