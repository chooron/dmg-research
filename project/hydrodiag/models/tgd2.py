...
def tgd2_step(
    ...
    tau_warm: torch.Tensor,
    delta_tau_cold: torch.Tensor,
...
class TemperatureDependentGenericDelay2(BaseHydrologicalModel):
    ...
    def forward(
        ...
        if initial_states is None:
            ...
            if (
                storage is None
                or storage.shape != (batch,)
                or storage.device != device
                or storage.dtype != precip.dtype
            ):
                raise ValueError(
                    "Initial TGD2 state 'storage' must be [batch] on the forcing device/dtype"
                )
        effective = torch.empty_like(precip)
        diagnostics = not self.compact_output or return_states
        if diagnostics:
            storage_trace, tau_trace, retention_trace = (
                torch.empty_like(precip) for _ in range(3)
            )
