...
def build_windows(
    ...
    while forecast_start + prediction_days <= calibration_days:
        windows.append(
            (
                forecast_start,
                forecast_start + prediction_days,
                forecast_start + warmup_days,
            )
        )
        forecast_start += stride_days
    if not windows:
        raise ValueError(
            "No complete prediction windows fit in the calibration period."
        )
    return windows


def kge_per_basin(
    qsim: torch.Tensor, qobs: torch.Tensor, eps: float = 1e-6
) -> torch.Tensor:
...
def evaluate_validation(
    ...
    with torch.no_grad():
        for start in range(0, n_basins, batch_size):
            ...
            fc = {
                key: torch.from_numpy(value[index].copy()).to(
    ...
    return (
        kges,
        parameters,
        lower.cpu().numpy() + parameters * parameter_range.cpu().numpy(),
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--config", type=Path, default=PROJECT_DIR / "configs/dpl_hbv_kgeq_365d_v1.json"
    )
    parser.add_argument("--epochs", type=int, help="Override config training.epochs.")
    parser.add_argument(
        "--max-basins", type=int, help="Use only the first N basins for a smoke test."
    )
    parser.add_argument(
        "--output-dir", help="Override config output_dir (useful for smoke tests)."
    )
    ...
    lower = torch.tensor(
        [HBV_PARAM_SPECS[name]["lower"] for name in names],
        device=device,
        dtype=torch.float32,
    )
    upper = torch.tensor(
        [HBV_PARAM_SPECS[name]["upper"] for name in names],
        device=device,
        dtype=torch.float32,
    )
    ...
    print(
        f"DPL HBV/KGE(Q): basins={n_basins}, attrs={attributes.shape[1]}, params={len(names)}",
        flush=True,
    )
    print(
        f"Windows={len(windows)} × warmup={win_cfg['warmup_days']} + prediction={win_cfg['prediction_days']}; "
        f"calibration tail excluded={tail_days} days",
        flush=True,
    )
    network_shape = "→".join(["35", *map(str, hidden_sizes), str(len(names))])
    print(
        f"Network={network_shape}, "
        f"AdamW lr={train_cfg['lr']}, epochs={train_cfg['epochs']}",
        flush=True,
    )
    ...
    for epoch in range(1, train_cfg["epochs"] + 1):
        ...
        for forecast_start, forecast_stop, _ in shuffled_windows:
            ...
            for batch_start in range(0, n_basins, train_cfg["batch_size"]):
                batch_index = basin_order[
                    batch_start : batch_start + train_cfg["batch_size"]
                ]
                ...
                fc = {
                    key: torch.from_numpy(
                        values[batch_index, forcing_start:forcing_stop].copy()
                    ).to(device)
        ...
        if (
            ...
            val_kge, _, _ = evaluate_validation(
                ...
                device,
                names,
                lower.to(torch.float64),
                parameter_range.to(torch.float64),
                win_cfg["warmup_days"],
            )
        ...
        history.append(row)
        print(
            "epoch={epoch:03d} train_loss={loss:.5f} batches={batches} lr={lr:.2e}{val}".format(
                epoch=epoch,
                loss=row["train_loss"],
    ...
    np.savez_compressed(
        output_dir / "best_parameters_normalized.npz", params=parameters_norm
    )
    np.savez_compressed(
        output_dir / "best_parameters_physical.npz", params=parameters_phys
    )
    with open(output_dir / "basin_final_summary.csv", "w", newline="") as handle:
        ...
        for index, basin_id in enumerate(basin_ids):
            writer.writerow(
                {"basin_id": basin_id, "basin_index": index, "val_kge": val_kge[index]}
            )
    report = (
