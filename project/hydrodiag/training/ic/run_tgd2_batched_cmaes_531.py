...
class UnitRandom:
    def __init__(
        ...
        self.generators = [
            np.random.default_rng(seed_for(model, basin, start))
    ...
    def sample(self) -> torch.Tensor:
        values = [
            item.standard_normal((self.population, self.dimension))
...
def evaluate_all(
    ...
    values = torch.empty(
        (units, population), dtype=torch.float64, device=runtime.device
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--basin-ids", help="Comma-separated canonical basin IDs for smoke runs."
    )
    parser.add_argument(
        "--target-npz",
        type=Path,
    ...
    output = (
        args.output
        or PROJECT / "results" / f"{args.model.lower()}_cmaes_531_batched_v1"
    )

    config = json.loads(
        (PROJECT / "ablation/configs/ic_foundation_531_v1.json").read_text()
    )
    ...
    if args.basin_ids:
        requested = tuple(
            item.strip().zfill(8) for item in args.basin_ids.split(",") if item.strip()
        )
    ...
    manifest = {
        ...
        "protocol": {
            ...
            "canonical_cn_psol_annual": bool(
                config.get("canonical_cn_psol_annual", False)
            ),
    ...
    if checkpoint_path.exists():
        ...
        expected = (
            args.model,
            structure_version_for(args.model),
            basin_ids,
            args.starts,
            population,
            args.generations,
        ...
        actual = (
            saved.get("model"),
            saved.get("structure_version"),
            tuple(saved.get("basin_ids", ())),
            saved.get("starts"),
            saved.get("population"),
            saved.get("generations"),
        )
        if actual != expected:
        ...
        centers = np.stack(
            [
                np.random.default_rng(seed_for(args.model, basin, start)).uniform(
                    0.01, 0.99, dimension
                )
                for basin in basin_ids
        ...
        while solver.state.generation < args.generations:
            if STOP:
                checkpoint(
                    ...
                    chunk_basins=args.chunk_basins,
                    history=history,
                    started=started,
                )
                return
            ...
            if (
                solver.state.generation % args.checkpoint_interval == 0
                or solver.state.generation == args.generations
            ):
                checkpoint(
                    checkpoint_path,
                    model=args.model,
    ...
    train = evaluate_all(
        runtime, selected, basin_indices, args.starts, args.chunk_basins, "train"
    )[:, 0]
    validation = evaluate_all(
        runtime, selected, basin_indices, args.starts, args.chunk_basins, "test"
    )[:, 0]
    physical = (
        normalized_to_physical(args.model, best, clip=True).detach().cpu().numpy()
    )
    ...
    for basin_position, basin in enumerate(basin_ids):
        for start in range(args.starts):
            ...
            record = {
                ...
                "basin_id": basin,
                "start": start,
                "seed": seed_for(args.model, basin, start),
                "parameter_names": list(specs),
                "theta_normalized": best[unit].detach().cpu().tolist(),
                "parameters": physical[unit].tolist(),
                "objective_definition": "KGE(Q), maximize, train only",
                "train_metrics": {"kge": float(train[unit])},
                "test_metrics": {"kge": float(validation[unit])},
                "best_train_objective": float(best_values[unit]),
                "generations": args.generations,
                "population": population,
                "candidate_evaluations": args.generations * population,
                "checkpoint": str(checkpoint_path),
                "completed_at": utcnow(),
            }
            atomic_json(
                output / "raw" / args.model.lower() / f"{basin}_start{start:02d}.json",
                record,
            )
    atomic_torch(
        output / "summaries" / "per_start_train_best_history.pt",
        {
            "basin_ids": basin_ids,
            "starts": args.starts,
            "history": history,
    ...
    atomic_json(
        output / "DONE.json",
        {
            "status": "complete",
            "records": units,
            "candidate_evaluations": units * population * args.generations,
            "elapsed_seconds": time.perf_counter() - started,
            "chunk_basins": args.chunk_basins,
        },
    )
