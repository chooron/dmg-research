...
def gate_a_audit(config: dict) -> dict[str, tuple[int, int] | int]:
    ...
    return {
        ...
        "eval": evaluation,
        "warmup": warmup,
        "wd": warmup[1] - warmup[0] + 1,
    }


def prepare_data(
    config: dict, time_indices: dict
) -> tuple[dict, dict, np.ndarray, np.ndarray, list[str], np.ndarray]:
    ...
    frac_snow = np.array(
        [attributes[full_index[basin_id], 3] for basin_id in basin_ids]
    )
    ...
    forcing_train = {
        key: forcing[cal_start - warmup_days : cal_end + 1, :, axis]
        .transpose()
        .copy()
        .astype(np.float32)
    ...
    observations_train = (
        target[cal_start : cal_end + 1, :, 0].transpose().copy().astype(np.float32)
    )
    evaluation_start = cal_end - warmup_days + 1
    evaluation_end = time_indices["eval"][1]
    forcing_eval = {
        key: forcing[evaluation_start : evaluation_end + 1, :, axis]
        .transpose()
        .copy()
        .astype(np.float32)
        for key, axis in forcing_axis.items()
    ...
    observations_eval = (
        target[time_indices["eval"][0] : evaluation_end + 1, :, 0]
        .transpose()
        .copy()
        .astype(np.float32)
    )
