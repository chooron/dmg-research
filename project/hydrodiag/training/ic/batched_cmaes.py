...
class BatchedCMAES:
    ...
    def __init__(
        ...
        *,
        device: torch.device,
        stdev_init: float = 0.20,
        active: bool = True,
    ) -> None:
        if units < 1 or dimension < 1 or population < 4:
            raise ValueError(
                "units and dimension must be positive; population must be at least four"
            )
        ...
        raw = torch.log(
            torch.tensor((population + 1) / 2, dtype=torch.float64, device=device)
        )
        raw = raw - torch.log(
            torch.arange(population, dtype=torch.float64, device=device) + 1
        )
        ...
        self.damp_sigma = (
            1
            + 2 * max(0.0, float(torch.sqrt((self.mu_eff - 1) / (dimension + 1)) - 1))
            + self.c_sigma
        )
        self.c_c = (4 + self.mu_eff / dimension) / (
            dimension + 4 + 2 * self.mu_eff / dimension
        )
        self.c_1 = (
            min(1.0, population / 6.0) * 2 / ((dimension + 1.3) ** 2 + self.mu_eff)
        )
        self.c_mu = min(
            1 - self.c_1,
            2
            * (
                ...
                / ((dimension + 2) ** 2 + self.mu_eff)
            ),
        )
        self.discount_sigma = torch.sqrt(
        ...
        self.expected_norm = math.sqrt(dimension) * (
            1 - 1 / (4 * dimension) + 1 / (21 * dimension**2)
        )
        self.decompose_frequency = max(
            1, int(math.floor(1 / (10 * dimension * float(self.c_1 + self.c_mu))))
        )
        eye = (
        ...
        self.state = BatchedCMAState(
            ...
            covariance=eye.clone(),
            factor=eye,
            sigma=torch.full((units,), stdev_init, dtype=torch.float64, device=device),
            path_sigma=torch.zeros(
                (units, dimension), dtype=torch.float64, device=device
            ),
            path_covariance=torch.zeros(
                (units, dimension), dtype=torch.float64, device=device
            ),
            best_fitness=torch.full(
                (units,), -torch.inf, dtype=torch.float64, device=device
            ),
    ...
    def ask(self, standard_normals: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        candidates = (
            self.state.mean[:, None, :] + self.state.sigma[:, None, None] * shaped
        )
        return z, candidates

    def tell(
        self, z: torch.Tensor, candidates: torch.Tensor, fitness: torch.Tensor
    ) -> None:
        if fitness.shape != (self.units, self.population):
            raise ValueError("fitness shape mismatch")
        f = torch.nan_to_num(
            fitness.to(self.device, dtype=torch.float64),
            nan=-999.0,
            posinf=-999.0,
            neginf=-999.0,
        )
        shaped = (candidates - self.state.mean[:, None, :]) / self.state.sigma[
            :, None, None
        ].clamp_min(1e-20)
        ...
        ranks.scatter_(
            1,
            order,
            torch.arange(self.population, device=self.device).expand(self.units, -1),
        )
        ...
        self.state.sigma.mul_(
            torch.exp(
                (self.state.path_sigma.norm(dim=1) / self.expected_norm - 1)
                * self.c_sigma
                / self.damp_sigma
            )
        )
        ...
        h_sigma = (
            (
                self.state.path_sigma.norm(dim=1).square()
                / denominator
                / self.dimension
                - 1
            )
            < 1 + 4 / (self.dimension + 1)
        ).to(torch.float64)
        self.state.path_covariance.mul_(1 - self.c_c).add_(
            h_sigma[:, None] * self.discount_covariance * mean_step
        )
        covariance_weights = assigned
        if self.active:
            covariance_weights = torch.where(
                covariance_weights > 0,
                covariance_weights,
                self.dimension
                * covariance_weights
                / z.norm(dim=2).square().clamp_min(1e-23),
            )
        c1a = self.c_1 * (1 - (1 - h_sigma.square()) * self.c_c * (2 - self.c_c))
        scaled_path = (
            torch.sqrt(self.c_1 / (c1a + 1e-23))[:, None] * self.state.path_covariance
        )
        rank_one = c1a[:, None, None] * (
            scaled_path[:, :, None] * scaled_path[:, None, :] - self.state.covariance
        )
        rank_mu = self.c_mu * (
            (
                covariance_weights[:, :, None, None]
                * shaped[:, :, :, None]
                * shaped[:, :, None, :]
            ).sum(1)
            - self.weights.sum() * self.state.covariance
        )
        self.state.covariance.add_(rank_one + rank_mu)
        self.state.covariance.copy_(
            0.5 * (self.state.covariance + self.state.covariance.transpose(1, 2))
        )
        self.state.generation += 1
        if self.state.generation % self.decompose_frequency == 0:
            jitter = (
                torch.eye(self.dimension, dtype=torch.float64, device=self.device)[None]
                * 1e-12
            )
            try:
                self.state.factor.copy_(
                    torch.linalg.cholesky(self.state.covariance + jitter)
                )
            except RuntimeError:
                bad = torch.linalg.eigvalsh(self.state.covariance)[:, 0] <= 1e-12
                self.state.covariance[bad] = torch.eye(
        ...
        self.state.best_candidate[improved] = candidates[
            torch.arange(self.units, device=self.device), index
        ][improved]
    ...
    def load_state_dict(self, payload: dict[str, Any]) -> None:
        if (payload["units"], payload["dimension"], payload["population"]) != (
            self.units,
            self.dimension,
            self.population,
        ):
