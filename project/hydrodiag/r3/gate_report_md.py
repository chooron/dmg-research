...
def main() -> None:
    ...
    L.append("Analysis is **CN-only**. No Base/TGD2 results were used; no "
    ...
    L.append(f"- **IC-CMA-ES** (best train-KGE restart): train KGE median "
    ...
    L.append(f"- **dPL** (median of 3 seeds): train KGE median {fmt(qdp['median_kge_train'])}, "
    ...
    L.append(f"- dPL per seed test medians: "
             + ", ".join(f"seed {s}={fmt(v)}" for s, v in qdp["per_seed_median_test_kge"].items()) + ".")
    L.append(f"- theta* round-trip (recorded forward vs frozen q_star): max abs diff "
             f"{report['round_trip_theta_star_max_abs_diff']:.2e}.\n")
    L.append("Verdict: **yes** — correct CN recovers Q* near its oracle ceiling "
             "under both regimes.\n")

    L.append("## 2. Shared XAJ parameter recovery (normalized errors)\n")
    L.append("`e = (theta_hat - theta_star)/(upper - lower)`; recovery profile per "
             "parameter, IC best-restart and dPL median-of-seeds:\n")
    L.append("| param | regime | med e | med \|e\| | q25/q75 \|e\| | q90 \|e\| | r(θ̂,θ\*) | slope | frac. bound | ρ(\|e\|,snow) |")
    L.append("|---|---|---|---|---|---|---|---|---|---|")
    for p in COMMON_XAJ:
        for regime, run in (("IC", "best-restart"), ("dPL", "median-seeds")):
            r = param[(param["parameter"] == p) & (param["run"] == run)].iloc[0]
            L.append(f"| {p} | {regime} | {fmt(r['median_signed_e'])} | {fmt(r['median_abs_e'])} "
    ...
    for p in ("cn_ctg", "cn_kf"):
        for regime, run in (("IC", "best-restart"), ("dPL", "median-seeds")):
            r = param[(param["parameter"] == p) & (param["run"] == run)].iloc[0]
            L.append(f"| {p} | {regime} | {fmt(r['median_abs_e'])} | {fmt(r['q90_abs_e'])} "
    ...
    L.append("|---|---|---|")
    for p in COMMON_XAJ:
        L.append(f"| {p} | {fmt(disp_sum.get(p, float('nan')), 4)} | "
    ...
    for paradigm in ("IC", "dPL"):
        ...
        L.append(f"- **{paradigm}**: D_theta (median over 15 shared params of |e|) "
        ...
        L.append(f"  D_theta among basins with KGE≥0.999: {fmt(e['D_theta_among_kge_ge_0p999'])} "
    ...
    for paradigm, exs in report["equifinality"]["examples_high_kge_poor_dtheta"].items():
        L.append(f"- {paradigm}: " + "; ".join(
            f"{x['basin_id']} (KGE {fmt(x['kge_train'])}, D_theta {fmt(x['D_theta'])})"
            for x in exs) + "")
    L.append("Verdict: **yes** — equifinality is present even for correct CN; "
             "near-ceiling discharge KGE does not imply parameter recovery.\n")

    L.append("## 5. Common internal state recovery\n")
    L.append("RMSE / NRMSE / correlation / bias distributions (median over basins), "
    ...
    for var in ("wu", "wl", "wd", "s", "fr", "qi", "qg"):
        for fit in ("IC", "dPL_median"):
            s = state[var][fit]
            L.append(f"| {var} | {fit} | {fmt(s['rmse_median'], 3)} | {fmt(s['nrmse_median'], 3)} "
    ...
    for col in ("ic_deficit_train", "ic_deficit_test", "dpl_deficit_test"):
        L.append(f"- Spearman({col}, frac_snow) = {fmt(sd.get(f'spearman_{col}_vs_frac_snow'), 3)}")
    for paradigm in ("IC", "dPL"):
        L.append(f"- {paradigm}: Spearman(KGE_train, frac_snow) = "
