...
def load_ic_all_starts(run_dir: Path, basin_ids: list[str]) -> dict[str, dict[int, np.ndarray]]:
    ...
    for basin in basin_ids:
        if basin not in out or len(out[basin]) != 10:
            raise ValueError(f"basin {basin}: expected 10 IC restarts, got {len(out.get(basin, {}))}")
    return out
...
def load_dpl_estimates(run_dir: Path, basin_ids: list[str]) -> dict[str, dict]:
    ...
    kge_by_basin = dict(zip(summary["basin_id"].astype(str).str.zfill(8),
                            summary["val_kge"]))
    out: dict[str, dict] = {}
...
def oracle_kge_cn(bundle, q_star, theta_star, device) -> dict[str, np.ndarray]:
    ...
    for split in ("train", "test"):
        fit, _ = runtime.evaluate_candidates_tensor(
            ...
            basin_indices=list(range(len(bundle.basin_ids))), split=split,
        )
...
def validate_and_align(bundle, truth_dir, ic_dir, dpl_dirs, q_star) -> dict:
    ...
    for key, arr in (("q", np.load(truth_dir / "q_star.npz")["target_mm_day"]),
    ...
    report["checks"]["ic_raw_records"] = {
        "basins": len(ic_ids), "restarts_per_basin": sorted(set(map(len, ic_starts.values()))),
    }
    ...
    report["headline_kge"] = {
        ...
        "dpl_val_kge_median": {
            str(s): float(pd.read_csv(dpl_dirs[s] / "basin_final_summary.csv")["val_kge"].median())
            for s in SEEDS
        },
    }
...
def main() -> None:
    ...
    x_star = {k: np.asarray(np.load(truth_dir / "x_star.npz")[k], dtype=np.float64)
              for k in STATE_KEYS}
    snow_star = {k: np.asarray(np.load(truth_dir / "snow_star.npz")[k], dtype=np.float64)
                 for k in SNOW_KEYS}
    ...
    z_dpl = {s: np.stack([(dpl_est[s][b]["theta_hat"] - lower) / p_range for b in basin_ids])
             for s in SEEDS}
    ...
    def state_metrics(sim: np.ndarray, truth: np.ndarray) -> dict:
        ...
        return {"rmse": rmse, "nrmse": nrmse, "corr": corr,
    ...
    for left in range(0, n, batch):
        ...
        fc["cn_psol_annual"] = torch.from_numpy(psol[left:right]).to(device, dtype=dtype)
        p = {name: torch.from_numpy(theta_star[left:right, i]).to(device, dtype=dtype)
             for i, name in enumerate(names)}
        qsim, _stores, _fs = recorded_cn_forward(model, fc, p, device, dtype)
        rt_max = max(rt_max, float(np.abs(
            qsim.detach().cpu().numpy().astype(np.float64) - q_star[left:right]).max()))
    ...
    for fit_name, z in fits.items():
        ...
        for left in range(0, n, batch):
            ...
            fc = build_forcing_dict(bundle.forcing[left:right].astype(np.float32), device, dtype)
            fc["cn_psol_annual"] = torch.from_numpy(psol[left:right]).to(device, dtype=dtype)
            p = {name: torch.from_numpy(theta_hat[left:right, i]).to(device, dtype=dtype)
                 for i, name in enumerate(names)}
                ...
                for k in range(left, right):
                    ...
                    for period, (si, ei) in (("train", pi["train"]), ("test", pi["test"])):
                        for var in STATE_KEYS + SNOW_KEYS:
                            ...
                            truth = (x_star if var in STATE_KEYS else snow_star)[var][k, si:ei + 1]
                            row = {"basin_id": basin_ids[k], "paradigm": fit_name.split("_")[0],
                                   "run": fit_name, "variable": var, "period": period}
    ...
    pd.DataFrame(metric_rows).to_csv(output_dir / "gate_discharge_metrics.csv", index=False)
    if not args.skip_states:
        pd.DataFrame(state_rows).to_csv(output_dir / "gate_state_metrics_basin.csv", index=False)
    ...
    for name in analysis_params:
        ...
        for paradigm, z_run, run_label in (
            ...
            ae = np.abs(e)
            valid = np.isfinite(e)
            pearson = float(np.corrcoef(z_run[valid], star_j[valid])[0, 1]) if valid.sum() > 2 else float("nan")
            spearman = float(pd.Series(z_run[valid]).corr(pd.Series(star_j[valid]), method="spearman"))
            slope, intercept = np.polyfit(star_j[valid], z_run[valid], 1) if valid.sum() > 2 else (float("nan"), float("nan"))
            phys = lower[j] + z_run * p_range[j]
            lo_b, up_b = lower[j], upper[j]
            param_rows.append({
                "parameter": name,
                "parameter_group": "cn_only" if name in ("cn_ctg", "cn_kf") else "shared",
                "paradigm": paradigm, "run": run_label,
                ...
                "spearman_signed_e_vs_frac_snow": float(
                    pd.Series(e[valid]).corr(pd.Series(frac[valid]), method="spearman")),
                "spearman_abs_e_vs_frac_snow": float(
                    pd.Series(ae[valid]).corr(pd.Series(frac[valid]), method="spearman")),
            })
    ...
    for j, name in enumerate(COMMON_XAJ):
        ...
        for paradigm, z_run, run_label in (
            ...
            for k, basin in enumerate(basin_ids):
                rec_rows.append({
                    "basin_id": basin, "paradigm": paradigm, "run": run_label,
                    "parameter": name, "z_hat": float(z_run[k]), "z_star": float(star_j[k]),
                    "delta_z": float(z_run[k] - star_j[k]),
                    "abs_delta_z": float(abs(z_run[k] - star_j[k])),
                })
    pd.DataFrame(rec_rows).to_csv(output_dir / "parameters_recoverability.csv", index=False)

    # IC restart dispersion per parameter (all 10 starts)
    disp_rows = []
    for k, basin in enumerate(basin_ids):
        for j, name in enumerate(COMMON_XAJ):
            z_starts = np.array([(ic_starts_all[basin][s] - lower) / p_range
                                 for s in ic_starts_all[basin]])
            zj = z_starts[:, shared_idx[j]]
            disp_rows.append({
                "basin_id": basin, "parameter": name,
                "z_std_across_starts": float(zj.std()), "z_range_across_starts": float(zj.max() - zj.min()),
            })
    ...
    for k, basin in enumerate(basin_ids):
        for j, name in enumerate(COMMON_XAJ):
            ...
            seed_rows.append({
                ...
                "z_min_across_seeds": float(zj.min()), "z_max_across_seeds": float(zj.max()),
            })
    ...
    for paradigm, run_label in (("IC", "IC"), ("dPL", "dPL_median")):
        sub = metric_df[(metric_df["paradigm"] == paradigm) & (metric_df["run"] == run_label)]
        for k, basin in enumerate(basin_ids):
            ...
            r = row.iloc[0]
            eq_rows.append({
                "basin_id": basin, "paradigm": paradigm,
                "kge_train": float(r["kge_train"]), "kge_test": float(r["kge_test"]),
    ...
    for paradigm in ("IC", "dPL"):
        ...
        eq_summary[paradigm] = {
            ...
            "spearman_kge_vs_D_theta": float(pd.Series(kge).corr(pd.Series(dt), method="spearman")),
            "D_theta_among_kge_ge_0p999": float(np.median(dt[kge >= 0.999])),
            "D_theta_among_kge_ge_0p99": float(np.median(dt[kge >= 0.99])),
            "D_theta_among_kge_ge_q90": float(np.median(dt[kge >= np.quantile(kge, 0.90)])),
    ...
    deficit_rows = []
    for k, basin in enumerate(basin_ids):
        ic_row = metric_df[(metric_df["basin_id"] == basin) & (metric_df["paradigm"] == "IC")]
        dpl_row = metric_df[(metric_df["basin_id"] == basin) & (metric_df["paradigm"] == "dPL") &
                            (metric_df["run"] == "dPL_median")]
        deficit_rows.append({
            "basin_id": basin,
            "frac_snow": float(frac_arr[k]),
            "ic_deficit_train": float(oracle["train"][k] - ic_row.iloc[0]["kge_train"]),
            "ic_deficit_test": float(oracle["test"][k] - ic_row.iloc[0]["kge_test"]),
    ...
    for paradigm, run_label in (("IC", "best-restart"), ("dPL", "median-seeds")):
        sub = param_df[(param_df["paradigm"] == paradigm) & (param_df["run"] == run_label)]
        snow_diag[f"param_abs_e_vs_frac_snow_{paradigm}"] = {
            r["parameter"]: float(r["spearman_abs_e_vs_frac_snow"]) for _, r in sub.iterrows()
        }
    ...
    if not args.skip_states:
        ...
        for var in STATE_KEYS + SNOW_KEYS:
            for fit_name in fits:
                ...
                sub = sub[(sub["run"] == fit_name) & (sub["variable"] == var) &
                          (sub["period"] == "train")]
                merged = sub.merge(pd.DataFrame({"basin_id": basin_ids, "frac_snow": frac_arr}),
                                   on="basin_id")
        ...
        snow_diag["state_nrmse_vs_frac_snow"] = {
            var: {fit: state_summary[var][fit]["spearman_nrmse_vs_frac_snow"]
                  for fit in fits}
    ...
    report = {
        ...
        "q_recovery_ic": {
            ...
            "median_oracle_gap_train": float(np.median(oracle["train"] - ic_metric["kge_train"])),
            "median_oracle_gap_test": float(np.median(oracle["test"] - ic_metric["kge_test"])),
        },
        "q_recovery_dpl": {
            "median_kge_train": float(dpl_metric["kge_train"].median()),
            "median_kge_test": float(dpl_metric[dpl_metric["run"] == "dPL_median"]["kge_test"].median()),
            "median_eval_ceiling": float(np.median(dpl_ceiling)),
            "median_gap_test": float(np.median(dpl_ceiling - dpl_metric[dpl_metric["run"] == "dPL_median"]["kge_test"])),
            "per_seed_median_test_kge": {
                str(s): float(dpl_metric[dpl_metric["run"] == f"dPL_seed{s}"]["kge_test"].median())
                for s in SEEDS
            },
        },
    ...
    manifest = {
        ...
        "parameter_contract": {
            "order": names,
            "bounds": {x: {"lower": specs[x]["lower"], "upper": specs[x]["upper"]}
                       for x in names},
