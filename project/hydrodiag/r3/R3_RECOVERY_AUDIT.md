...
| Local results root | `/home/jingxin/code/dmg-research/project/hydrodiag/results` **does not exist** — the entire local R1/R2/R3 results tree is gone (the `outputs -> results/archive/legacy_outputs_20260730` symlink is dangling). R3 code's `r3/common.py::DEFAULT_RESULTS_ROOT` points here. |
...
`r3/protocol_misspec_v1.json` exists (committed + on remote), frozen 2026-08-14
...
`r3/misspec_analysis.py` and match the task's historical description.
...
3. **Recorded-forward state export**: `r3/recorded_forward.py` replays the
...
### 2.6 KGE formula
`r3/docs/kge_audit.md` documents that all repository KGE implementations
...
| Component | Status |
|---|---|
| `r3/` package (common, truth_generator, recorded_forward, generate_truth, run_base_no_refit, pilot, analyze_pilot, oracle_identity, oracle_dpl_audit, run_gate_531, launch_d2_parallel, gate_analysis, gate_report_md, misspec_analysis, protocol_misspec_v1.json, README, HANDOFF, docs/kge_audit.md) | PRESENT + current (committed) |
...
| Gate analysis outputs (13 files: gate_report.json/.md, gate_manifest.json, gate_input_validation.json, parameters_recoverability.csv, parameter_recovery_summary.csv, ic_restart_parameter_dispersion.csv, dpl_seed_parameter_spread.csv, gate_discharge_metrics.csv, kge_vs_parameter_recovery.csv, kge_deficit_vs_frac_snow.csv, gate_state_metrics_basin.csv, gate_state_summary.json) | `results/r3_gate_v1/` | — | **LOST**; regenerated by `r3/gate_analysis.py` after the gates are re-run (CPU, cheap) |
...
- `r3/common.py::DEFAULT_RESULTS_ROOT` and `DEFAULT_DATA_ROOT` hard-code the
...
2. **Fetch from remote (read-only)** to the local results root:
   `r3_synthetic_truth_v1/` (~250 MB) and `r3/results/r3_misspec_dpl_{xaj,xaj_tgd2}_seed_*` (6 dirs). Verify by re-running the 4 currently skipped/blocked R3 tests (they pass once the truth NPZ is local). Channel: AutoDL web console (downlink is unreliable over ssh; base64-over-ssh worked for the 485 KB tar, so small files can be pulled that way).
3. **Rebuild cheap artifacts locally**: `r3/run_base_no_refit.py --device cuda`; `r3/oracle_identity.py`, `r3/oracle_dpl_audit.py` (531-basin forwards).
4. **Regenerate the correct-CN gate** (remote or local GPU): `r3/run_gate_531.py` D1 (`--model XAJ_CN --starts 10 --generations 300 --target-npz q_star`) and D2 (`launch_d2_parallel.py`, seeds 42/123/2026). Expected cost ≈ 1 h (IC) + ≈ 4 h (dPL × 3 in parallel) on the remote 3080 Ti.
5. **Re-run gate analysis**: `r3/gate_analysis.py --device cuda` → `r3_gate_v1/` (gate_report.json + CSVs), then `r3/gate_report_md.py`. Re-verify the frozen-tier rationale (tiers themselves are preserved in `protocol_misspec_v1.json` and must NOT be re-selected).
...
7. **Final analysis**: `r3/misspec_analysis.py` (refuses to run on incomplete inputs; implements the frozen protocol; Base-no-refit as raw reference; seed-matched dPL; paired delta_KGE / delta_abs_e / delta_e / delta_E with frac_snow associations; primary/secondary tier labels fixed).
