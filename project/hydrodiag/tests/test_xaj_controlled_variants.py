...
def _params(specs, batch=1, dtype=torch.float64, requires_grad=False):
    return {
        name: torch.tensor(
            [spec["default"]] * batch, dtype=dtype, requires_grad=requires_grad
        )
...
def _forcings(kind="mixed", n=32, dtype=torch.float64):
    ...
    if kind == "zero":
        ...
        precip, pet, temp = (
            torch.zeros(n, dtype=dtype),
            torch.full((n,), 1.0, dtype=dtype),
            torch.full((n,), 4.0, dtype=dtype),
        )
        precip[2] = 35.0
    elif kind == "snow":
        ...
        temp = torch.where(
            t < n // 2, torch.full_like(t, -3.0), torch.full_like(t, 4.0)
        )
    else:
...
def _initial(dtype=torch.float64):
    ...
    return {
        ...
        "xaj_wu": torch.tensor([8.0], dtype=dtype),
        "xaj_wl": torch.tensor([25.0], dtype=dtype),
        "xaj_wd": torch.tensor([15.0], dtype=dtype),
        "xaj_s": torch.tensor([4.0], dtype=dtype),
        "xaj_fr": torch.tensor([0.25], dtype=dtype),
        "xaj_qi": torch.tensor([0.0], dtype=dtype),
        "xaj_qg": torch.tensor([0.0], dtype=dtype),
        "xaj_qb": torch.tensor([0.0], dtype=dtype),
    }
...
def test_existing_xaj_and_xaj_cn_regression():
    ...
    forcing = {
        "precip": torch.tensor([[0.0, 3.0, 9.0, 0.0, 2.0]], dtype=torch.float64),
        "pet": torch.tensor([[1.0, 2.0, 1.0, 3.0, 2.0]], dtype=torch.float64),
        "temp": torch.tensor([[-2.0, 1.0, 4.0, 3.0, -1.0]], dtype=torch.float64),
    }
    expected = {
        XAJ: [
            0.37669,
    ...
    for model_cls, specs in (
        ...
        assert torch.allclose(
            qsim[0],
            torch.tensor(expected[model_cls], dtype=torch.float64),
            atol=1e-12,
            rtol=1e-12,
        )
...
def _mass_residuals(aux, params, initial, two_source):
    ...
    assert (
        torch.max(torch.abs(raw_precip + snow_prev - effective - snow)).item() < cn_tol
    )
    ...
    if two_source:
        ...
        route0 = params["xaj_ci"] * initial["xaj_qi"] / (
            1.0 - params["xaj_ci"]
        ) + params["xaj_cg"] * initial["xaj_qg"] / (1.0 - params["xaj_cg"])
    ...
    state0 = (
        ...
        + initial["xaj_wl"]
        + initial["xaj_wd"]
        + initial["xaj_fr"] * initial["xaj_s"]
        + route0
    )
    delta = torch.cat(
        (state[:, :1] - state0.unsqueeze(1), state[:, 1:] - state[:, :-1]), dim=1
    )
    return effective - aux["evap_total"] - out - delta


@pytest.mark.parametrize(
    "model_cls,specs,two_source",
    [
...
def test_xaj_rwpe_evaporation_invariants():
    ...
    _, aux = XAJRWPEWithCemaNeige().to(dtype=torch.float64)(
        forcing, params, initial_states=_initial()
    )
    pet_adj = forcing["pet"] * params["xaj_k"].unsqueeze(1)
    assert (
        torch.all(aux["eu"] >= 0)
        and torch.all(aux["el"] >= 0)
        and torch.all(aux["ed"] >= 0)
    )
...
def test_xaj_rwpe_tau_e_boundary_formula_is_finite_and_conservative(tau_e):
    ...
    el, ed, er, z_root, stress = _rootzone_moisture_stress_evaporation(
        ...
        torch.tensor([0.0, 20.0, 50.0], dtype=dtype),
        torch.full((3,), 50.0, dtype=dtype),
        torch.full((3,), 50.0, dtype=dtype),
        torch.full((3,), tau_e, dtype=dtype),
        1e-8,
    )
...
def test_xaj_variants_eager_and_compiled_forward_backward_agree(model_cls, specs):
    ...
    eager_q, _ = model_cls(compile_step=False).to(dtype=torch.float64)(
        forcing, eager_params, initial_states=_initial()
    )
    compiled_q, _ = model_cls().to(dtype=torch.float64)(
        forcing, compiled_params, initial_states=_initial()
    )
    assert torch.allclose(eager_q, compiled_q, atol=1e-12, rtol=1e-12)
    eager_q.square().mean().backward()
    compiled_q.square().mean().backward()
    for name in specs:
        eager_grad, compiled_grad = eager_params[name].grad, compiled_params[name].grad
        if eager_grad is not None:
            assert torch.isfinite(eager_grad).all()
        if compiled_grad is not None:
            assert torch.isfinite(compiled_grad).all()
    core = {"cn_kf", "xaj_k"} | (
        {"xaj_kb", "xaj_cb"} if "xaj_kb" in specs else {"xaj_tau_e"}
    )
    for name in core:
        assert (
            eager_params[name].grad is not None
            and compiled_params[name].grad is not None
        )
...
def test_xaj_variants_one_step_and_long_sequence_mass_balance(
    model_cls, specs, two_source, kind
):
    ...
    for dtype, tolerance in ((torch.float64, 5e-7), (torch.float32, 2e-3)):
        ...
        forcing = {
            key: value.to(dtype=dtype) for key, value in _forcings(kind, 80).items()
        }
...
def test_xaj_variants_parameter_boundaries_and_gradients(model_cls, specs):
    ...
    for name, spec in specs.items():
        params[name] = torch.tensor(
            [spec["lower"], (spec["lower"] + spec["upper"]) / 2, spec["upper"]],
            dtype=torch.float64,
            requires_grad=True,
        )
    forcing = {
        key: value.expand(3, -1) for key, value in _forcings("mixed", 20).items()
    }
    qsim, aux = model_cls().to(dtype=torch.float64)(
        forcing, params, initial_states=None, return_states=True
    )
...
def _finite_difference(model_cls, specs, name, epsilon):
    ...
    loss = (
        model_cls()
        .to(dtype=torch.float64)(forcing, analytic, initial_states=initial)[0]
        .sum()
    )
    loss.backward()
    ...
    plus = _params(specs)
    minus = _params(specs)
    plus[name] = plus[name] + epsilon
    minus[name] = minus[name] - epsilon
    fp = (
        model_cls()
        .to(dtype=torch.float64)(forcing, plus, initial_states=initial)[0]
        .sum()
        .item()
    )
    fm = (
        model_cls()
        .to(dtype=torch.float64)(forcing, minus, initial_states=initial)[0]
        .sum()
        .item()
    )
...
@pytest.mark.parametrize(
    "name,eps", [("xaj_kb", 1e-5), ("xaj_cb", 1e-5), ("xaj_b", 1e-5), ("cn_kf", 1e-5)]
)
...
def test_xaj_variants_registry_lite_and_checkpoint_roundtrip(
    full_cls, lite_cls, specs, key
):
    assert (
        MODEL_REGISTRY[key][0] is full_cls and LITE_MODEL_REGISTRY[key][0] is lite_cls
    )
    ...
    clone = full_cls().to(dtype=torch.float64)
    clone.load_state_dict(restored["state_dict"])
    q1, _ = clone(forcing, params)
    assert (
        restored["schema"] == model.checkpoint_schema and restored["model_key"] == key
    )
