...
from models.gr4j import GR4J_UH1_MAX, GR4J_UH2_MAX, _gr4j_step
...
def make_cn_gr4j_fused_inputs(batch, device, dtype):
    return (
        ...
        (
            torch.rand(batch, device=device, dtype=dtype) * 300.0,
            torch.rand(batch, device=device, dtype=dtype) * 300.0,
            torch.rand(batch, GR4J_UH1_MAX, device=device, dtype=dtype),
            torch.rand(batch, GR4J_UH2_MAX, device=device, dtype=dtype),
        ),
        (
            torch.rand(batch, device=device, dtype=dtype) * 0.5,
            torch.rand(batch, device=device, dtype=dtype) * 5.0 + 1.0,
        ...
        (
            torch.rand(batch, GR4J_UH1_MAX, device=device, dtype=dtype),
            torch.rand(batch, GR4J_UH2_MAX, device=device, dtype=dtype),
            torch.rand(batch, device=device, dtype=dtype) * 500.0 + 100.0,
            torch.randn(batch, device=device, dtype=dtype) * 2.0,
            torch.rand(batch, device=device, dtype=dtype) * 1000.0 + 100.0,
        ),
        NEARZERO,
    )
...
def make_cn_simhyd_fused_inputs(batch, device, dtype):
    ...
    kf = torch.rand(batch, device=device, dtype=dtype) * 5.0 + 1.0
    g_thresh = torch.rand(batch, device=device, dtype=dtype) * 500.0
    precip_t, pet_t, soil, gw, insc, coeff, sq, smsc, sub, crak, k, etmul, nz = (
        make_simhyd_inputs(batch, device, dtype)
    )
    return (
        precip_t,
        temp_t,
...
def run_compile_test(step_fn, make_inputs, name, device, dtype):
    ...
    assert eager_out[0].shape == (BATCH,), (
        f"[{name}] Eager output shape {eager_out[0].shape} != expected ({BATCH},)"
    )
    assert compiled_out[0].shape == (BATCH,), (
        f"[{name}] Compiled output shape {compiled_out[0].shape} != expected ({BATCH},)"
    )

    # Verify finite
    assert torch.isfinite(eager_out[0]).all(), f"[{name}] Eager output contains NaN/Inf"
    assert torch.isfinite(compiled_out[0]).all(), (
        f"[{name}] Compiled output contains NaN/Inf"
    )
...
class TestStepCompile:
    ...
    def test_xaj_compact_step_compile(self, device_str, dtype):
        ...
        assert run_compile_test(
            _xaj_step_compact, make_xaj_compact_inputs, "XAJ_Compact", device, dtype
        )
    ...
    def test_simhyd_step_compile(self, device_str, dtype):
        device = torch.device(device_str)
        assert run_compile_test(
            _simhyd_step, make_simhyd_inputs, "SIMHYD", device, dtype
        )
    ...
    def test_simhyd_compact_step_compile(self, device_str, dtype):
        device = torch.device(device_str)
        assert run_compile_test(
            _simhyd_step_compact, make_simhyd_inputs, "SIMHYD_Compact", device, dtype
        )
    ...
    def test_cemaneige_step_compile(self, device_str, dtype):
        device = torch.device(device_str)
        assert run_compile_test(
            _cemaneige_step, make_cemaneige_inputs, "CemaNeige", device, dtype
        )
    ...
    def test_precip_delay_step_compile(self, device_str, dtype):
        device = torch.device(device_str)
        assert run_compile_test(
            _precip_delay_step, make_precip_delay_inputs, "PrecipDelay", device, dtype
        )
