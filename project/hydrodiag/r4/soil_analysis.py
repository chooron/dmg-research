...
def calendar_month_anomaly(series: np.ndarray, months: np.ndarray) -> np.ndarray:
    """Subtract the calendar-month climatological mean from a daily series.

    Missing values remain missing and all-missing calendar months are handled
    without raising a RuntimeWarning or contaminating the whole series.
    """
    values = np.asarray(series, dtype=np.float64)
    anom = np.full(values.shape, np.nan, dtype=np.float64)
    for m in range(1, 13):
        idx = months == m
        valid = idx & np.isfinite(values)
        if np.any(valid):
            anom[valid] = values[valid] - np.mean(values[valid])
    return anom


def pairwise_pearson(x: np.ndarray, y: np.ndarray, min_points: int = 30) -> float:
    """Return Pearson correlation on finite paired values, or NaN if undefined."""
...
def monthly_aggregate(
    series: np.ndarray, dates: np.ndarray
...
def zscore_nrmse(x: np.ndarray, y: np.ndarray) -> float:
    """Normalized trajectory error on standardized, paired finite values."""
    xv = np.asarray(x, dtype=np.float64)
    yv = np.asarray(y, dtype=np.float64)
    valid = np.isfinite(xv) & np.isfinite(yv)
    if int(valid.sum()) < 30:
    ...
    zx = (xv - np.mean(xv)) / sx
    zy = (yv - np.mean(yv)) / sy
    return float(np.sqrt(np.mean((zx - zy) ** 2)))
...
def compute_basin_year_timing(
    ...
    for w in np.unique(wy):
        ...
        # Snow activity filter for the year.  Some dry/no-data slices can be
        # entirely NaN after a long continuous forward; skip them explicitly
        # instead of calling nanmax/nanargmax on an empty slice.
        finite_swe = np.isfinite(swe_w)
        finite_wm = np.isfinite(wm_w)
        finite_sm = np.isfinite(sm_w)
        if not (finite_swe.any() and finite_wm.any() and finite_sm.any()):
            continue
        annual_swe_max = float(np.max(swe_w[finite_swe]))
        annual_wm_max = float(np.max(wm_w[finite_wm]))
        is_snow_year = (annual_swe_max >= MIN_ANNUAL_SWE_PEAK_MM) and (
            annual_wm_max >= MIN_ANNUAL_W_PEAK_MM
        )

        # 1. Soil-water peak timing (doy of maximum)
        peak_idx_ref = int(np.where(finite_sm, sm_w, -np.inf).argmax())
        peak_idx_mod = int(np.where(finite_wm, wm_w, -np.inf).argmax())
        peak_doy_ref = float(dw[peak_idx_ref])
        peak_doy_mod = float(dw[peak_idx_mod])
        peak_timing_error = (
            float(peak_doy_mod - peak_doy_ref) if is_snow_year else np.nan
        )
        ...
        if spring_mask.sum() >= 60 and is_snow_year:
            ...
            # Restrict search to spring and finite candidate windows.
            valid_ref = spring_mask & np.isfinite(diff_ref)
            valid_mod = spring_mask & np.isfinite(diff_mod)
            if valid_ref.any() and valid_mod.any():
                wetup_idx_ref = int(np.where(valid_ref, diff_ref, -np.inf).argmax())
                wetup_idx_mod = int(np.where(valid_mod, diff_mod, -np.inf).argmax())
...
def run_soil_consistency_analysis() -> Dict[str, Any]:
    ...
    print(
        f"Loaded Caravan soil reference: {n_basins} basins x {len(test_dates)} test days (1995-10-01 .. 2010-09-30)"
    )
    ...
    for regime_name, prefix, seed, tag in regimes:
        ...
        for i, b in enumerate(basin_ids):
            ...
            r_anom_base_sm289 = float(
                stats.pearsonr(wb_anom, calendar_month_anomaly(ref_sm289, months_test))[
                    0
                ]
            )
            ...
            r_anom_cn_sm289 = float(
                stats.pearsonr(wc_anom, calendar_month_anomaly(ref_sm289, months_test))[
                    0
                ]
            )
            ...
            paired_rows.append(
                {
                    ...
                    "delta_nrmse": nrmse_cn
                    - nrmse_base,  # Negative delta = CN has lower error
            ...
            timing_years_base = compute_basin_year_timing(
                b, test_dates, wb, ref_sm100, ref_swe
            )
            timing_years_cn = compute_basin_year_timing(
                b, test_dates, wc, ref_sm100, ref_swe
            )

            for rec in timing_years_base:
                timing_year_rows.append(
                    {"regime": regime_name, "structure": "Base", **rec}
                )
            for rec in timing_years_cn:
                timing_year_rows.append(
                    {"regime": regime_name, "structure": "CN", **rec}
                )
            ...
            def summarize_timing(t_rows):
                snow_rows = [
                    r
                    for r in t_rows
                    if r["is_snow_year"] and np.isfinite(r["peak_timing_error_days"])
                ]
                n_v = len(snow_rows)
                ...
                w_errs = np.array(
                    [
                        r["wetup_timing_error_days"]
                        for r in snow_rows
                        if np.isfinite(r["wetup_timing_error_days"])
                    ]
                )
                ...
                return {
                    ...
                    "iqr_peak_error_days": float(
                        np.nanpercentile(p_errs, 75) - np.nanpercentile(p_errs, 25)
                    ),
                    "median_wetup_error_days": float(np.nanmedian(w_errs))
                    if len(w_errs) >= MIN_VALID_YEARS_PER_BASIN
                    else np.nan,
                    "mean_wetup_error_days": float(np.nanmean(w_errs))
                    if len(w_errs) >= MIN_VALID_YEARS_PER_BASIN
                    else np.nan,
                    "median_abs_wetup_error_days": float(np.nanmedian(np.abs(w_errs)))
                    if len(w_errs) >= MIN_VALID_YEARS_PER_BASIN
                    else np.nan,
                    "iqr_wetup_error_days": float(
                        np.nanpercentile(w_errs, 75) - np.nanpercentile(w_errs, 25)
                    )
                    if len(w_errs) >= MIN_VALID_YEARS_PER_BASIN
                    else np.nan,
                }
            ...
            timing_summary_rows.append(
                {
                    ...
                    "structure": "Base",
                    "basin_id": b,
                    **summarize_timing(timing_years_base),
                }
            )
            timing_summary_rows.append(
                {
                    ...
                    "structure": "CN",
                    "basin_id": b,
                    **summarize_timing(timing_years_cn),
                }
            )
    ...
    for regime_name in ["dPL_seed42", "dPL_seed123", "IC_fused"]:
        ...
        for target_col in [
            ...
            reg_results[f"{target_col}_vs_swe_burden_snow_active_only"] = (
                theil_sen_bootstrap(
                    sub_p.loc[active_mask, "snow_burden_swe_mm"].to_numpy(),
                    sub_p.loc[active_mask, target_col].to_numpy(),
                )
            )
        ...
        q_bins = pd.qcut(
            sub_p["snow_burden_swe_mm"], 4, labels=q_labels, duplicates="drop"
        )
        sub_p = sub_p.copy()
        ...
        for qv in q_labels:
            ...
            c_base_q = sub_c[
                (sub_c["structure"] == "Base") & (sub_c["basin_id"].isin(b_ids_q))
            ]
            c_cn_q = sub_c[
                (sub_c["structure"] == "CN") & (sub_c["basin_id"].isin(b_ids_q))
            ]
            ...
            q_entry = {
                "n": n_q,
                "swe_burden_median_mm": float(
                    np.nanmedian(sub_p.loc[q_idx, "snow_burden_swe_mm"])
                ),
                "base_median_anomaly_corr": float(
                    np.nanmedian(c_base_q["monthly_anomaly_corr"])
                ),
                "cn_median_anomaly_corr": float(
                    np.nanmedian(c_cn_q["monthly_anomaly_corr"])
                ),
                "delta_anomaly_corr_median": float(np.nanmedian(delta_anom_q)),
                "delta_anomaly_corr_iqr": float(
                    np.nanpercentile(delta_anom_q, 75)
                    - np.nanpercentile(delta_anom_q, 25)
                ),
                "base_median_7d_corr": float(
                    np.nanmedian(c_base_q["smoothed_7d_corr"])
                ),
        ...
        report_summary["regimes"][regime_name] = {
            "overall_all_531_basins": {
                "base_median_anomaly_corr": float(
                    np.nanmedian(c_base["monthly_anomaly_corr"])
                ),
                "cn_median_anomaly_corr": float(
                    np.nanmedian(c_cn["monthly_anomaly_corr"])
                ),
                "delta_anomaly_corr_median": float(
                    np.nanmedian(sub_p["delta_anomaly_corr"])
                ),
                ...
                "base_median_raw_daily_corr": float(
                    np.nanmedian(c_base["raw_daily_corr"])
                ),
                "cn_median_raw_daily_corr": float(np.nanmedian(c_cn["raw_daily_corr"])),
                "delta_raw_daily_corr_median": float(
                    np.nanmedian(sub_p["delta_raw_daily_corr"])
                ),
                "base_median_monthly_corr": float(
                    np.nanmedian(c_base["monthly_mean_corr"])
                ),
                "cn_median_monthly_corr": float(
                    np.nanmedian(c_cn["monthly_mean_corr"])
                ),
                "delta_monthly_corr_median": float(
                    np.nanmedian(sub_p["delta_monthly_corr"])
                ),
            ...
            "snow_active_subset_352_basins": {
                "base_median_anomaly_corr": float(
                    np.nanmedian(c_base_act["monthly_anomaly_corr"])
                ),
                "cn_median_anomaly_corr": float(
                    np.nanmedian(c_cn_act["monthly_anomaly_corr"])
                ),
                "delta_anomaly_corr_median": float(
                    np.nanmedian(
                        sub_p.loc[sub_p["is_snow_active"], "delta_anomaly_corr"]
                    )
                ),
                "base_median_7d_corr": float(
                    np.nanmedian(c_base_act["smoothed_7d_corr"])
                ),
                "cn_median_7d_corr": float(np.nanmedian(c_cn_act["smoothed_7d_corr"])),
                "delta_7d_corr_median": float(
                    np.nanmedian(sub_p.loc[sub_p["is_snow_active"], "delta_7d_corr"])
                ),
                "base_median_raw_daily_corr": float(
                    np.nanmedian(c_base_act["raw_daily_corr"])
                ),
                "cn_median_raw_daily_corr": float(
                    np.nanmedian(c_cn_act["raw_daily_corr"])
                ),
                "delta_raw_daily_corr_median": float(
                    np.nanmedian(
                        sub_p.loc[sub_p["is_snow_active"], "delta_raw_daily_corr"]
                    )
                ),
                "base_median_monthly_corr": float(
                    np.nanmedian(c_base_act["monthly_mean_corr"])
                ),
                "cn_median_monthly_corr": float(
                    np.nanmedian(c_cn_act["monthly_mean_corr"])
                ),
                "delta_monthly_corr_median": float(
                    np.nanmedian(
                        sub_p.loc[sub_p["is_snow_active"], "delta_monthly_corr"]
                    )
                ),
            },
            "timing_errors": {
                "base_median_peak_error_days": float(
                    np.nanmedian(t_base["median_peak_error_days"])
                ),
                "base_median_abs_peak_error_days": float(
                    np.nanmedian(t_base["median_abs_peak_error_days"])
                ),
                "cn_median_peak_error_days": float(
                    np.nanmedian(t_cn["median_peak_error_days"])
                ),
                "cn_median_abs_peak_error_days": float(
                    np.nanmedian(t_cn["median_abs_peak_error_days"])
                ),
                "base_median_wetup_error_days": float(
                    np.nanmedian(t_base["median_wetup_error_days"])
                ),
                "base_median_abs_wetup_error_days": float(
                    np.nanmedian(t_base["median_abs_wetup_error_days"])
                ),
                "cn_median_wetup_error_days": float(
                    np.nanmedian(t_cn["median_wetup_error_days"])
                ),
                "cn_median_abs_wetup_error_days": float(
                    np.nanmedian(t_cn["median_abs_wetup_error_days"])
                ),
            },
    ...
    report_path.write_text(
        json.dumps(report_summary, indent=2, sort_keys=True), encoding="utf-8"
    )
