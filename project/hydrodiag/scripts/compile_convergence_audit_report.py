...
for bin_label in fixed_labels:
    ...
    print(f"  XAJ_TGD:  Spread Med = {iqr_str(tgd_sp)} | Spread < 0.01: {tgd_sub01:.1f}%\n")
...
print(f"  KGE Spread Med: {df_audit_base.loc[high_ids, 'kge_spread'].median():.6f} | Spread < 0.01: {(df_audit_base.loc[high_ids, 'kge_spread'] < 0.01).mean()*100:.1f}%")
print(f"  Plateau Gen Med: {df_audit_base.loc[high_ids, 'avg_plateau_gen'].median():.1f} / 300")
print(f"  Param Dist Med: {df_audit_base.loc[high_ids, 'param_dist_mean'].median():.4f}")

print("\nXAJ_CN High Snow:")
print(f"  KGE Spread Med: {df_audit_cn.loc[high_ids, 'kge_spread'].median():.6f} | Spread < 0.01: {(df_audit_cn.loc[high_ids, 'kge_spread'] < 0.01).mean()*100:.1f}%")
print(f"  Plateau Gen Med: {df_audit_cn.loc[high_ids, 'avg_plateau_gen'].median():.1f} / 300")
print(f"  Param Dist Med: {df_audit_cn.loc[high_ids, 'param_dist_mean'].median():.4f}")

print("\nXAJ_TGD High Snow:")
print(f"  KGE Spread Med: {df_audit_tgd.loc[high_ids, 'kge_spread'].median():.6f} | Spread < 0.01: {(df_audit_tgd.loc[high_ids, 'kge_spread'] < 0.01).mean()*100:.1f}%")
print(f"  Plateau Gen Med: {df_audit_tgd.loc[high_ids, 'avg_plateau_gen'].median():.1f} / 300")
