...
from scripts.freeze_native_subsurface_scale import (
...
def _summary(values: np.ndarray) -> dict[str, float | int]:
    ...
    return {
        ...
        "p75": float(q[3]), "p95": float(q[4]), "max": float(values.max()),
        "iqr": float(q[3] - q[1]),
    }
...
def _bin_ids(values: np.ndarray, basin_ids: list[str], mask: np.ndarray) -> dict[str, list[str]]:
    selected_ids = np.asarray(basin_ids, dtype=object)[mask]
    selected_values = values[mask]
    return {
        "<1%": [str(b) for b, v in zip(selected_ids, selected_values) if v < 0.01],
        "1-5%": [str(b) for b, v in zip(selected_ids, selected_values) if 0.01 <= v < 0.05],
        "5-20%": [str(b) for b, v in zip(selected_ids, selected_values) if 0.05 <= v < 0.20],
...
def _select_records(raw_directory: str | Path, basin_ids: list[str]) -> tuple[dict[str, list[dict[str, Any]]], dict[str, dict[str, Any]]]:
    ...
    if any(len(records) != 10 for records in by_basin.values()):
        raise RuntimeError("Endpoint audit requires exactly 10 existing starts per basin")
    best: dict[str, dict[str, Any]] = {}
    for basin_id, records in by_basin.items():
        best[basin_id] = max(records, key=lambda r: (float(r["train_objective"]), -int(r["start"])))
    return by_basin, best


def _objective_gap_audit(by_basin: dict[str, list[dict[str, Any]]], best: dict[str, dict[str, Any]]) -> dict[str, Any]:
    gaps: list[float] = []
    endpoint_fraction: list[float] = []
    no_finite: list[str] = []
    endpoint_best: list[str] = []
    for basin_id, records in by_basin.items():
        endpoint = [r for r in records if float(r["parameters"][11]) == 1.0 or float(r["parameters"][12]) == 1.0]
        endpoint_fraction.append(len(endpoint) / len(records))
        if float(best[basin_id]["parameters"][11]) == 1.0 or float(best[basin_id]["parameters"][12]) == 1.0:
            endpoint_best.append(basin_id)
        finite = [r for r in records if float(r["parameters"][11]) < 1.0 and float(r["parameters"][12]) < 1.0]
        if not finite:
            no_finite.append(basin_id)
        else:
            best_finite = max(finite, key=lambda r: (float(r["train_objective"]), -int(r["start"])))
            gaps.append(float(best[basin_id]["train_objective"]) - float(best_finite["train_objective"]))
    ...
    return {
        ...
        "delta_le_0_001_fraction": float((gaps_np <= 0.001).mean()) if gaps_np.size else None,
        "delta_le_0_005_fraction": float((gaps_np <= 0.005).mean()) if gaps_np.size else None,
        "delta_le_0_01_fraction": float((gaps_np <= 0.01).mean()) if gaps_np.size else None,
        "endpoint_start_fraction": _summary(np.asarray(endpoint_fraction)),
    }


def _best_forward_activity(parameters: np.ndarray, forcing: np.ndarray) -> dict[str, np.ndarray]:
    ...
    with torch.no_grad():
        for timestep in range(nsteps):
            out = model._step(
                precip[:, timestep], pet[:, timestep], wu, wl, wd, s, fr, qi, qg,
                k, b, im, um, lm, dm, c, sm, ex, ki, kg, ci, cg, 1e-8,
            )
            (_q, rs_adj, qi, qg, _evap, wu, wl, wd, s, fr,
             _rs, ri, rg, _eu, _el, _ed) = out
            vs += rs_adj
            vi += ri * one_minus_im
            vg += rg * one_minus_im
    vss = vi + vg
    ...
    return {
        "vi": vi.numpy(), "vg": vg.numpy(), "vss": vss.numpy(),
        "vtotal": vtotal.numpy(), "share_i_subsurface": (vi / safe_ss).numpy(),
...
def _activity_audit(best_parameters: np.ndarray, basin_ids: list[str], activity: dict[str, np.ndarray]) -> dict[str, Any]:
    ci_endpoint = best_parameters[:, 11] == 1.0
    cg_endpoint = best_parameters[:, 12] == 1.0
    result: dict[str, Any] = {}
    for label, mask, sub_share, total_share in (
        ("CI_endpoint_interflow", ci_endpoint, activity["share_i_subsurface"], activity["share_i_total"]),
        ("CG_endpoint_groundwater", cg_endpoint, activity["share_g_subsurface"], activity["share_g_total"]),
    ):
        result[label] = {
    ...
    result["all_best_activity"] = {
        "vi": _summary(activity["vi"]), "vg": _summary(activity["vg"]),
        "vss": _summary(activity["vss"]), "vtotal": _summary(activity["vtotal"]),
...
def _group_comparison(parameters: np.ndarray, best: dict[str, dict[str, Any]], basin_ids: list[str], activity: dict[str, np.ndarray]) -> dict[str, Any]:
    ...
    objective = np.asarray([float(best[basin_id]["train_objective"]) for basin_id in basin_ids], dtype=np.float64)
    data = {
        "native_train_objective": objective,
        "kss": kss, "share_i_subsurface": activity["share_i_subsurface"],
    ...
    groups: dict[str, Any] = {"endpoint_basin_count": int(endpoint.sum()), "finite_basin_count": int(finite.sum())}
    for name, values in data.items():
        if name.endswith("_finite_only"):
            valid = parameters[:, 11] < 1.0 if name == "ci_finite_only" else parameters[:, 12] < 1.0
            values = values[valid]
            ...
            finite_mask = finite
        groups[name] = {"endpoint": _summary(values[endpoint_mask]), "finite": _summary(values[finite_mask])}
        ep_med = np.median(values[endpoint_mask]); fi_med = np.median(values[finite_mask])
        pooled_iqr = max(groups[name]["endpoint"].get("iqr", 0.0), groups[name]["finite"].get("iqr", 0.0), 1e-30)
        groups[name]["standardized_median_difference"] = float((ep_med - fi_med) / pooled_iqr)
    groups["basin_ids_endpoint"] = [basin_id for basin_id, flag in zip(basin_ids, endpoint) if flag]
    groups["basin_ids_finite"] = [basin_id for basin_id, flag in zip(basin_ids, finite) if flag]
    return groups
...
def run_audit(
    *, dataset_path: str | Path, gage_ids_path: str | Path,
    basin_list_path: str | Path, native_raw_directory: str | Path,
) -> dict[str, Any]:
    ...
    best_parameters = np.asarray([best[basin_id]["parameters"] for basin_id in basin_ids], dtype=np.float64)
    all_counts = _endpoint_counts([record for records in by_basin.values() for record in records])
    ...
    return {
        "semantics": {
            "native_qi0": 0.1, "native_qg0": 0.1,
            "native_state_names": ["wu", "wl", "wd", "s", "fr", "qi", "qg", "rs_uh_buffer"],
        ...
        "best_endpoint_basin_ids": [basin_id for basin_id, flag in zip(basin_ids, endpoint) if flag],
        "best_ci_endpoint_basin_ids": [basin_id for basin_id, value in zip(basin_ids, best_parameters[:, 11]) if value == 1.0],
        "best_cg_endpoint_basin_ids": [basin_id for basin_id, value in zip(basin_ids, best_parameters[:, 12]) if value == 1.0],
        ...
        "finite_vs_endpoint_descriptive": _group_comparison(best_parameters, best, basin_ids, activity),
        "z0_candidate": {
        ...
        "source": {
            ...
            "n_basins": len(basin_ids), "n_timesteps": int(forcing.shape[1]),
        },
    }
...
def main() -> None:
    ...
    parser.add_argument("--dataset", type=Path, default=Path("/home/jingxin/code/dmg-research/data/camels_dataset"))
    parser.add_argument("--gage-ids", type=Path, default=Path("/home/jingxin/code/dmg-research/data/gage_id.npy"))
    parser.add_argument("--basin-list", type=Path, default=Path("/home/jingxin/code/dmg-research/data/531sub_id.txt"))
    parser.add_argument("--native-raw", type=Path, default=Path("results/xaj_base_cmaes_531_batched_paired_v2/raw/xaj"))
    args = parser.parse_args()
    summary = run_audit(
        dataset_path=args.dataset, gage_ids_path=args.gage_ids,
        basin_list_path=args.basin_list, native_raw_directory=args.native_raw,
    )
