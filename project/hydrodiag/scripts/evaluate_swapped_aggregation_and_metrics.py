...
ic_xaj = pd.read_csv(OUTPUTS_DIR / "XAJ_ic_median_and_best_of_3.csv").set_index("basin_id")
ic_xaj_cn = pd.read_csv(OUTPUTS_DIR / "XAJ_CN_ic_median_and_best_of_3.csv").set_index("basin_id")
ic_xaj_tgd = pd.read_csv(OUTPUTS_DIR / "XAJ_TGD_ic_median_and_best_of_3.csv").set_index("basin_id")
...
def get_dpl_seeds_data(model_name):
    ...
    return pd.DataFrame({
        ...
        "tr_best": tr_best, "te_best": te_best
    })
...
combinations = [
    ("Combination 1: Original Asymmetric (IC Best-of-3 vs dPL Median-of-3)", "tr_best", "tr_med", "te_best", "te_med"),
    ("Combination 2: Aligned Median-of-3 (IC Median-of-3 vs dPL Median-of-3)", "tr_med", "tr_med", "te_med", "te_med"),
    ("Combination 3: Swapped Asymmetric (IC Median-of-3 vs dPL Best-of-3)", "tr_med", "tr_best", "te_med", "te_best"),
    ("Combination 4: Aligned Best-of-3 (IC Best-of-3 vs dPL Best-of-3)", "tr_best", "tr_best", "te_best", "te_best"),
]

for title, ic_tr_suf, dpl_tr_suf, ic_te_suf, dpl_te_suf in combinations:
    print(f"\n--- {title} ---")
    print(f"{'Model':10s} | {'IC Train Med':12s} | {'dPL Train Med':13s} | {'IC Test Med':12s} | {'dPL Test Med':13s} | {'Test Winner':12s}")
    print("-" * 80)
    for m_short, m_key in [("XAJ Base", "xaj"), ("XAJ_CN", "cn"), ("XAJ_TGD", "tgd")]:
        ...
        winner = "dPL Wins" if dpl_te > ic_te else ("IC Wins" if ic_te > dpl_te else "Tie")
        diff_str = f"({dpl_te - ic_te:+.4f})"
        print(f"{m_short:10s} | {ic_tr:12.4f} | {dpl_tr:13.4f} | {ic_te:12.4f} | {dpl_te:13.4f} | {winner:8s} {diff_str}")
