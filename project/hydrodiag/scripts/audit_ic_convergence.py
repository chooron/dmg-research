...
def audit_convergence():
    ...
    for model_key in models:
        ...
        result_files = glob.glob(
            os.path.join(out_dir, "**/seed_101/**/result.json"), recursive=True
        )
        ...
        for b_id in all_531:
            ...
            if len(valid_s) == 3:
                ...
                plateaus = [
                    starts[s]["plateau_gen"]
                    for s in [0, 1, 2]
                    if starts[s]["plateau_gen"] is not None
                ]
        ...
        print(
            f"Saved {len(df_sum)} basin convergence audit records for {model_key} to {out_csv}"
        )
        ...
        print(
            f"   Start KGE Spread Med: {df_sum['kge_spread'].median():.6f} | IQR: {q75 - q25:.6f} | Max: {df_sum['kge_spread'].max():.6f}"
        )
        print(
            f"   Basins with Start KGE Spread < 0.01: {(df_sum['kge_spread'] < 0.01).sum()} / {len(df_sum)} ({(df_sum['kge_spread'] < 0.01).mean() * 100:.1f}%)"
        )
        print(
            f"   Basins with Start KGE Spread < 0.001: {(df_sum['kge_spread'] < 0.001).sum()} / {len(df_sum)} ({(df_sum['kge_spread'] < 0.001).mean() * 100:.1f}%)"
        )
        print(
            f"   Mean Param Vector Distance Med: {df_sum['param_dist_mean'].median():.4f}"
        )
        print(
            f"   Avg Plateau Generation Med: {df_sum['avg_plateau_gen'].median():.1f} / 300 generations\n"
        )
