...
def lyne_hollick_bfi(discharge: np.ndarray, alpha: float = ALPHA) -> float:
    ...
    for _ in range(3):
        ...
        for index in range(1, len(q)):
            forward[index] = alpha * forward[index - 1] + (1.0 + alpha) * 0.5 * (q[index] - q[index - 1])
            forward[index] = min(max(forward[index], 0.0), q[index])
        reverse = forward[::-1].copy()
        for index in range(1, len(reverse)):
            reverse[index] = alpha * reverse[index - 1] + (1.0 + alpha) * 0.5 * (forward[::-1][index] - forward[::-1][index - 1])
...
def _greedy_maximin(ids: list[str], points: np.ndarray, n: int) -> list[int]:
    ...
    first = max(remaining, key=lambda i: (float(np.linalg.norm(points[i] - centroid)), -int(ids[i])))
    chosen = [first]
...
def build_manifest(config_path: str | Path, output: str | Path) -> dict:
    ...
    bfi = np.asarray([lyne_hollick_bfi(q) for q in bundle.target_mm_day], dtype=np.float64)
...
def main() -> None:
    ...
    parser.add_argument("--config", default="ablation/configs/ic_foundation_531_v1.json")
    parser.add_argument("--output", required=True)
    args = parser.parse_args()
    manifest = build_manifest(args.config, args.output)
    print(json.dumps({k: manifest[k] for k in ("n_selected", "n_eligible", "n_excluded", "strata")}, indent=2))
