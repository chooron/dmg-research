...
class GAAdapter(OptimizerAdapter):
    ...
    def tell(self, fitness: np.ndarray) -> None:
        ...
        if max_fit > self.best_fitness:
            ...
            self.best_candidate = self.latest_batch.values[max_idx].clone().cpu().numpy().astype(self.np_dtype)
    ...
    def state_dict(self) -> dict[str, Any]:
        state = {
            ...
            'population': self.ga.population.values.cpu().numpy().tolist() if self.ga.population is not None else None,
            'evals': self.ga.population.evals.cpu().numpy().tolist() if (self.ga.population is not None and self.ga.population.evals is not None) else None,
    ...
    def load_state_dict(self, state: dict[str, Any]) -> None:
        ...
        if state['population'] is not None:
            self.ga.population.set_values(torch.tensor(state['population'], dtype=self.dtype, device=self.device))
        if state['evals'] is not None:
            self.ga.population.set_evals(torch.tensor(state['evals'], dtype=torch.float64, device=self.device))
