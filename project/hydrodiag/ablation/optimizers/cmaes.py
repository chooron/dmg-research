...
class CMAESAdapter(OptimizerAdapter):
    ...
    def ask(self) -> np.ndarray:
        self.latest_zs, self.latest_ys, self.latest_xs = self.cmaes.sample_distribution()
    ...
    def tell(self, fitness: np.ndarray) -> None:
        ...
        if max_fit > self.best_fitness:
            ...
            self.best_candidate = self.latest_xs[max_idx].clone().cpu().numpy().astype(self.np_dtype)
        ...
        ranks[indices] = torch.arange(self.cmaes.popsize, dtype=indices.dtype, device=indices.device)
        assigned_weights = self.cmaes.weights[ranks]
        
        zs, ys = self.latest_zs, self.latest_ys
        local_m_displacement, shaped_m_displacement = self.cmaes.update_m(zs, ys, assigned_weights)
        ...
        if self.cmaes.stdev_min is not None or self.cmaes.stdev_max is not None:
            self.cmaes.C = _limit_stdev(self.cmaes.sigma, self.cmaes.C, self.cmaes.stdev_min, self.cmaes.stdev_max)
        ...
        if (self.cmaes._steps_count + 1) % self.cmaes.decompose_C_freq == 0:
            try:
                ...
                eps = 1e-8 * torch.eye(self.cmaes.solution_length, dtype=self.cmaes.C.dtype, device=self.cmaes.C.device)
                self.cmaes.C = self.cmaes.C + eps
                try:
                    self.cmaes.decompose_C()
                except Exception:
                    # Reset C to identity if jitter fails
                    self.cmaes.C = torch.eye(self.cmaes.solution_length, dtype=self.cmaes.C.dtype, device=self.cmaes.C.device)
                    self.cmaes.decompose_C()
    ...
    def load_state_dict(self, state: dict[str, Any]) -> None:
        ...
        self.cmaes.m.copy_(torch.tensor(state['m'], dtype=self.dtype, device=self.device))
        self.cmaes.C.copy_(torch.tensor(state['C'], dtype=self.dtype, device=self.device))
        self.cmaes.sigma.copy_(torch.tensor(state['sigma'], dtype=self.dtype, device=self.device))
        ...
        self.cmaes.p_c = torch.tensor(state['p_c'], dtype=self.dtype, device=self.device)
        self.cmaes.p_sigma = torch.tensor(state['p_sigma'], dtype=self.dtype, device=self.device)
        self.cmaes.decompose_C()
