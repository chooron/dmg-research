...
@register("CEM")
class CEMAdapter(OptimizerAdapter):
    def __init__(self):
        self.cem = None
    ...
    def initialize(
        ...
        parenthood_ratio = config.get('parenthood_ratio', 0.2)
        self.cem = CEM(
            self.problem, 
            popsize=population, 
    ...
    def ask(self) -> np.ndarray:
        self.latest_samples = self.cem._distribution.sample(
    ...
    def tell(self, fitness: np.ndarray) -> None:
        ...
        grads = self.cem._distribution.compute_gradients(
            self.latest_samples, 
            fit_t, 
            objective_sense='max',
            ranking_method=self.cem._ranking_method
        )
        self.cem._update_distribution(grads)
        self.generation += 1

    def get_center(self) -> np.ndarray:
        if hasattr(self.cem._distribution, 'mu'):
            return self.cem._distribution.mu.cpu().numpy().astype(self.np_dtype)
        elif hasattr(self.cem._distribution, 'center'):
            return self.cem._distribution.center.cpu().numpy().astype(self.np_dtype)
    ...
    def state_dict(self) -> dict[str, Any]:
        ...
        if hasattr(self.cem._distribution, 'mu'):
            state['mu'] = self.cem._distribution.mu.cpu().numpy().tolist()
        if hasattr(self.cem._distribution, 'sigma'):
            state['sigma'] = self.cem._distribution.sigma.cpu().numpy().tolist()
        return state
    ...
    def load_state_dict(self, state: dict[str, Any]) -> None:
        ...
        if 'mu' in state and hasattr(self.cem._distribution, 'mu'):
            self.cem._distribution.mu.copy_(torch.tensor(state['mu'], dtype=self.dtype, device=self.device))
        if 'sigma' in state and hasattr(self.cem._distribution, 'sigma'):
            self.cem._distribution.sigma.copy_(torch.tensor(state['sigma'], dtype=self.dtype, device=self.device))
