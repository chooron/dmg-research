...
def run_phase2_ablation():
    ...
    output_root = os.path.join(
        "outputs", "ic_ablation", "phase2_stdev_screening", "v1", "tasks"
    )
    ...
    completed_marker = os.path.join(
        "outputs",
        "ic_ablation",
        "phase2_stdev_screening",
        "v1",
        "PHASE2_STDEV_COMPLETED.txt",
    )
    ...
    print(
        f"Models: GR4J(Pop=24, dim=4), SIMHYD(Pop=40, dim=10), XAJ(Pop=90, dim=15)",
        flush=True,
    )
    ...
    base_config["max_concurrent_groups"] = (
        8  # 8 CONCURRENT GPU WORKERS FOR HIGH PARALLELISM!
    )
    ...
    for stdev_val in [0.05, 0.10, 0.15, 0.25, 0.35, 0.50]:
        ...
        print(
            f"\n--- Running stdev_init = {stdev_val:.2f} ({len(stdev_tasks)} tasks) ---",
            flush=True,
        )
    ...
    print(
        f"\n=== Phase 2 stdev_init Ablation Completed in {elapsed_hours:.2f} hours ===",
        flush=True,
    )
