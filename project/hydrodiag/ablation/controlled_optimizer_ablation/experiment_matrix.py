...
from .protocol import SEEDS, Phase1Task
...
def generate_lhs_center_for_basin_model(
    basin_id: str, model_key: str, start_idx: int, dim: int
) -> List[float]:
...
def generate_phase1_matrix(config: dict) -> List[Phase1Task]:
    ...
    if len(basins) < config["n_basins"]:
        raise ValueError(
            f"Found only {len(basins)} basins for split {split}, but {config['n_basins']} requested."
        )
                ...
                for start_idx in range(config["starts"]):
                    if has_npz_centers and basin_id in lhs_basin_ids:
                        ...
                        center = generate_lhs_center_for_basin_model(
                            basin_id, model_key, start_idx, dim
                        )
