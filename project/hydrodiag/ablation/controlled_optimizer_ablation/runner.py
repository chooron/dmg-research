...
def run_batched_tasks_for_group_process(args):
    ...
    if not pending_tasks:
        print(
            f"Group ({opt_name}, pop {population}, start {start_idx}, seed {seed}): All {len(tasks_for_group)} tasks already completed."
        )
        return
    ...
    for t in pending_tasks:
        if t.population != task_pop:
            raise ValueError(
                f"Fail-loud: Population mismatch within group {t.population} vs {task_pop}"
            )
        if t.generations != task_gen:
            raise ValueError(
                f"Fail-loud: Generations mismatch within group {t.generations} vs {task_gen}"
            )
        if t.stdev_init != task_stdev:
            raise ValueError(
                f"Fail-loud: stdev_init mismatch within group {t.stdev_init} vs {task_stdev}"
            )
    ...
    print(
        f"Running Group ({opt_name}, pop {task_pop}, gen {task_gen}, stdev {task_stdev}, start {start_idx}, seed {seed}): {len(pending_tasks)} pending basins on GPU..."
    )
    ...
    input_length = (
        bundle.periods.train_forcing_end_index
        - bundle.periods.train_forcing_start_index
    )
    ...
    if (
        ...
        or train_length <= 0
        or input_length <= 0
        or objective_length <= 0
    ):
        raise ValueError(
            f"Fail-loud: Invalid period lengths warmup={warmup_length}, train={train_length}, input={input_length}, obj={objective_length}"
        )
    ...
    for gen in range(generations):
        ...
        try:
            ...
            eval_result = rt.evaluate_candidates(
                cands_np, basin_indices=basin_indices, split="train"
            )
            ...
            if "out of memory" in err_str.lower():
                print(
                    f"[CUDA OOM ERROR DETECTED] in Group ({opt_name}, pop{population}, start{start_idx}, seed{seed}): {err_str}",
                    flush=True,
                )
                with open(
                    "/autodl-fs/data/dmg_hydro_structure_diagnosis/oom_error.log", "a"
                ) as f_oom:
                    f_oom.write(
                        f"[{time.ctime()}] CUDA OOM in group ({opt_name}, pop{population}, start{start_idx}, seed{seed}): {err_str}\n"
                    )
        ...
        if (gen + 1) % 50 == 0 or gen == 0:
            ...
            print(
                f"  [{opt_name} pop{population} start{start_idx} seed{seed}] gen {gen + 1}/{generations}: median_kge={med_kge:.4f}, step_time={elapsed_gen:.2f}s",
                flush=True,
            )
    ...
    thetas_3d = torch.tensor(best_thetas, dtype=torch.float32, device=device).unsqueeze(
        1
    )
    test_eval_res = rt.evaluate_candidates(
        thetas_3d, basin_indices=basin_indices, split="test"
    )
...
def run_tasks(tasks: List[Phase1Task], config: dict):
    ...
    print(
        f"Total task groups: {len(groups)} (each group runs 32 basins in a single GPU batched matrix call per generation)."
    )

    max_concurrent_groups = config.get("max_concurrent_groups", 6)
    print(
        f"Running up to {max_concurrent_groups} task groups concurrently using ProcessPoolExecutor (spawn context) on GPU..."
    )
    ...
    with concurrent.futures.ProcessPoolExecutor(
        max_workers=max_concurrent_groups, mp_context=mp_ctx
    ) as executor:
        futures = [
            executor.submit(run_batched_tasks_for_group_process, args)
