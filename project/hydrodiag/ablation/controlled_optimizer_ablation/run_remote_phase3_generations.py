...
def run_phase3_ablation():
    ...
    output_root = os.path.join(
        "outputs", "ic_ablation", "phase3_generations_screening", "v1", "tasks"
    )
    ...
    print(
        f"=== Starting Phase 3 Max-Generation (Gen=600) Convergence Trajectory Ablation ===",
        flush=True,
    )
    print(
        f"Total Phase 3 tasks: {len(all_tasks)} (Single Gen=600 run per start, 0 redundant re-runs!)",
        flush=True,
    )
    ...
    print(
        f"\n=== Phase 3 Generations Ablation Completed in {elapsed_hours:.2f} hours ===",
        flush=True,
    )
    ...
    completed_marker = os.path.join(
        "outputs",
        "ic_ablation",
        "phase3_generations_screening",
        "v1",
        "PHASE3_GENERATIONS_COMPLETED.txt",
    )
