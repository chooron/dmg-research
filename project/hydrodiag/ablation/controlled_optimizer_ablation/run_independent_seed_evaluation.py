...
from ablation.controlled_optimizer_ablation.experiment_matrix import (
    ...
    generate_lhs_center_for_basin_model,
)
...
MODEL_OPTIMAL_POPS = {"GR4J": 24, "SIMHYD": 40, "XAJ": 90}

# Fixed optimal stdev_init* from Phase 2 per model
MODEL_OPTIMAL_STDEV = {"GR4J": 0.10, "SIMHYD": 0.05, "XAJ": 0.05}

# Fixed optimal generations G* from Phase 3 per model
MODEL_OPTIMAL_GENS = {"GR4J": 200, "SIMHYD": 200, "XAJ": 300}


def generate_independent_eval_tasks(
    output_root: str, basin_ids: list[str]
) -> list[Phase1Task]:
    tasks = []
            ...
            for b_id in basin_ids:
                for seed in independent_seeds:
                    for s_idx in range(starts):
                        center = generate_lhs_center_for_basin_model(
                            b_id, model_key, s_idx, dim
                        )
                        t_dir = os.path.join(
                            output_root,
                            model_key,
...
def run_independent_eval():
    ...
    basin_manifest_path = os.path.join(
        "ablation", "manifests", "ic_ablation_96_basins_v1.json"
    )
    ...
    print(
        f"=== Starting Independent Seed Rerun Evaluation (Seeds 404, 505, 606) ===",
        flush=True,
    )
    print(
        f"Total tasks: {len(all_tasks)} (32 basins x 3 models x 2 optimizers x 3 seeds x 3 starts)",
        flush=True,
    )
    ...
    base_config["max_concurrent_groups"] = (
        8  # Safe concurrency to guarantee 5GB VRAM safety buffer & 0 CUDA OOM!
    )
    ...
    print(
        f"\n=== Independent Seed Rerun Evaluation Completed in {elapsed_hours:.2f} hours ===",
        flush=True,
    )

    completed_marker = os.path.join(
        "/root/outputs/independent_seed_evaluation/v1", "INDEPENDENT_EVAL_COMPLETED.txt"
    )
