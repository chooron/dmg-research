...
def _bounds(
    ...
    *,
    ...
    names = tuple(specs)
    lower = torch.tensor(
        [specs[name]["lower"] for name in names], dtype=dtype, device=device
    )
    upper = torch.tensor(
        [specs[name]["upper"] for name in names], dtype=dtype, device=device
    )
...
def validate_parameter_shape(model_key: str, theta: Any) -> None:
    ...
    if not shape or shape[-1] != expected:
        raise ValueError(
            f"{model_key} expects trailing parameter dimension {expected}, got {shape}"
        )
...
def normalized_to_physical(model_key: str, theta_01: Any, *, clip: bool = True):
    ...
    if isinstance(theta_01, torch.Tensor):
        names, lower, upper = _bounds(
            model_key, dtype=theta_01.dtype, device=theta_01.device
        )
        ...
        for index, name in enumerate(names):
            if name in LOG_SCALED_PARAMETERS:
                physical[..., index] = torch.exp(
                    ...
                    + value[..., index]
                    * (torch.log(upper[index]) - torch.log(lower[index]))
                )
        return physical
    ...
    lower = np.asarray(
        [get_parameter_spec(model_key)[name]["lower"] for name in names],
        dtype=np.float64,
    )
    upper = np.asarray(
        [get_parameter_spec(model_key)[name]["upper"] for name in names],
        dtype=np.float64,
    )
    physical = lower + value * (upper - lower)
    for index, name in enumerate(names):
        if name in LOG_SCALED_PARAMETERS:
            physical[..., index] = np.exp(
                np.log(lower[index])
                + value[..., index] * (np.log(upper[index]) - np.log(lower[index]))
            )
    return physical
...
def physical_to_normalized(model_key: str, physical: Any, *, clip: bool = False):
    ...
    if isinstance(physical, torch.Tensor):
        names, lower, upper = _bounds(
            model_key, dtype=physical.dtype, device=physical.device
        )
        value = physical.clone()
        for index, name in enumerate(names):
            if name in LOG_SCALED_PARAMETERS:
                value[..., index] = (
                    torch.log(value[..., index]) - torch.log(lower[index])
                ) / (torch.log(upper[index]) - torch.log(lower[index]))
            else:
                value[..., index] = (value[..., index] - lower[index]) / (
    ...
    for index, name in enumerate(names):
        if name in LOG_SCALED_PARAMETERS:
            normalized[..., index] = (
                np.log(value[..., index]) - np.log(lower[index])
            ) / (np.log(upper[index]) - np.log(lower[index]))
        else:
            normalized[..., index] = (value[..., index] - lower[index]) / (
                upper[index] - lower[index]
            )
...
def parameter_summary(model_key: str) -> dict[str, Any]:
    ...
    return {
        ...
        "log_scaled_parameters": sorted(
            name for name in specs if name in LOG_SCALED_PARAMETERS
        ),
        "bounds": {
            name: {"lower": spec["lower"], "upper": spec["upper"]}
