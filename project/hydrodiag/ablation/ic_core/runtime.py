...
class ICObjectiveRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self.objective = KGEObjective(
            min_samples=int(config.get("objective", {}).get("min_samples", 30))
        )
        self.basin_batch_size = int(config["batching"].get("basin_batch_size", 4))
        self.cache_device_data = bool(
            config.get("batching", {}).get("cache_device_data", False)
        )
        ...
        if bool(config.get("canonical_cn_psol_annual", False)) and model_key.endswith(
            "_CN"
        ):
            ...
            temp = forcing_t[:, :, 1]
            with torch.no_grad():
                self._cn_psol_annual = (
                    _estimate_psol_annual(precip, temp)
                    .numpy()
    ...
    def _split_arrays(self, split: str) -> tuple[np.ndarray, np.ndarray, int]:
        if split == "train":
            ...
            return (
                self.bundle.forcing[
                    :, p.train_forcing_start_index : p.train_forcing_end_index, :
                ],
                self.bundle.target_mm_day[
                    :, p.train.start_index : p.train.end_index + 1
                ],
                p.warmup.days,
            )
        if split == "test":
            ...
            return (
                self.bundle.forcing[
                    :, p.test_forcing_start_index : p.test_forcing_end_index, :
                ],
    ...
    def _normalize_candidates(
        self, theta_01: Any, basin_indices: Sequence[int] | None
        ...
        basin_array = (
            default_basins
            if basin_indices is None
            else np.asarray(list(basin_indices), dtype=np.int64)
        )
        if len(basin_array) != theta.shape[0]:
            raise ValueError(
                "basin_indices length must match candidate basin dimension"
            )
    ...
    def _split_tensors(
        self, split: str, basin_array: np.ndarray
        ...
        if self.cache_device_data:
            ...
            if cached is None:
                ...
                cached = (
                    torch.from_numpy(forcing).to(
                        device=self.device, dtype=torch.float32
                    ),
                    torch.from_numpy(target).to(
        ...
        forcing, target, warmup = self._split_arrays(split)
        return (
            torch.from_numpy(forcing[basin_array]).to(
                device=self.device, dtype=torch.float32
            ),
            torch.from_numpy(target[basin_array]).to(
                device=self.device, dtype=torch.float64
            ),
            warmup,
        )
    ...
    def evaluate_candidates_tensor(
        ...
        *,
        ...
        forcing_tensor, target_tensor, warmup_days = self._split_tensors(
            split, basin_array
        )
        physical_flat = physical.reshape(n_basins * population, d).to(
            device=self.device, dtype=torch.float32
        )
        temp_mean_train = torch.from_numpy(self.bundle.temp_mean_train[basin_array]).to(
            device=self.device, dtype=torch.float32
        )
        temp_std_train = torch.from_numpy(self.bundle.temp_std_train[basin_array]).to(
            device=self.device, dtype=torch.float32
        )
        ...
        q_full, _ = self.model_adapter.run_model(
            forcing_expanded,
            physical_flat,
            forcing_names=self.bundle.forcing_names,
            temp_mean_train=temp_mean_train,
            temp_std_train=temp_std_train,
            cn_psol_annual=(
                torch.from_numpy(self._cn_psol_annual[basin_array]).to(
                    device=self.device, dtype=torch.float32
                )
                if self._cn_psol_annual is not None
                else None
            ),
        )
    ...
    def evaluate_candidates(
        ...
        *,
        ...
        theta, basin_array = self._normalize_candidates(theta_01, basin_indices)
        n_basins, population, d = theta.shape
        fitness_tensor, diagnostics = self.evaluate_candidates_tensor(
            theta, basin_indices=basin_array, split=split
        )
        ...
        return CandidateEvaluation(
            ...
            forcing_shape=(
                n_basins * population,
                split_forcing.shape[1],
                len(self.bundle.forcing_names),
            ),
