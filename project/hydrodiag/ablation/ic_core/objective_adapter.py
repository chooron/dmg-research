...
class KGEObjective:
    ...
    def evaluate(
        self, simulation: torch.Tensor, target_mm_day: torch.Tensor
        ...
        if target_mm_day.ndim == 2:
            target_mm_day = target_mm_day.unsqueeze(1).expand(
                -1, simulation.shape[1], -1
            )
        if simulation.shape != target_mm_day.shape:
            raise ValueError(
                f"simulation/target shape mismatch: {simulation.shape} vs {target_mm_day.shape}"
            )
        ...
        fitness = compute_kge_fp64_matrix_gpu(
            sim64, target64, min_samples=self.min_samples
        )
        mask = (
            torch.isfinite(sim64)
            & torch.isfinite(target64)
            & (sim64 >= 0.0)
            & (target64 >= 0.0)
        )
        valid_count = mask.sum(dim=1).reshape(simulation.shape[:2])
        valid = torch.isfinite(fitness).reshape(simulation.shape[:2]) & (
            fitness.reshape(simulation.shape[:2]) > self.invalid_fitness
        )
    ...
    def evaluate_numpy(
        self, simulation: np.ndarray, target_mm_day: np.ndarray
