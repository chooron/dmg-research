...
def run_step5_and_6_and_7():
    ...
    with (
        open(out_kge_csv, "w", newline="") as fkge,
        ...
        kge_writer.writerow(
            [
                "basin_id",
                "seed",
                "saved_best_train_kge",
                "gpu_recomputed",
                "cpu_independent_fp64",
            ]
        )
        ...
        for basin_id in target_basins:
            ...
            with open(Path(task_dir) / "result.json") as f:
                ...
                best_norm = np.array(
                    res_data["best_theta_normalized"], dtype=np.float32
                )
                saved_kge = res_data["best_train_kge"]

            cand_tensor = (
                torch.tensor(best_norm, device=runtime.device).unsqueeze(0).unsqueeze(0)
            )

            # evaluate_candidates takes (theta_01, basin_indices, split)
            evals = runtime.evaluate_candidates(
                cand_tensor, basin_indices=[b_idx], split="train"
            )
            gpu_kge = evals.fitness[0, 0].item()

            physical = (
                normalized_to_physical(runtime.model_key, cand_tensor, clip=True)
                .squeeze(0)
                .squeeze(0)
            )  # (D)
            ...
            forcing_tensor = torch.from_numpy(forcing_sub).to(
                device=runtime.device, dtype=torch.float32
            )
            physical_tensor = (
                torch.from_numpy(physical.cpu().numpy())
                .unsqueeze(0)
            ...
            sim_tensor, _ = runtime.model_adapter.run_model(
                ...
                temp_mean_train=torch.from_numpy(bundle.temp_mean_train[[b_idx]]).to(
                    device=runtime.device, dtype=torch.float32
                ),
                temp_std_train=torch.from_numpy(bundle.temp_std_train[[b_idx]]).to(
                    device=runtime.device, dtype=torch.float32
                ),
            )
            ...
            if basin_id in discrim_basins:
                ...
                c_rand1 = np.random.uniform(0, 1, size=best_norm.shape).astype(
                    np.float32
                )
                c_pert1 = np.clip(
                    best_norm + np.random.normal(0, 0.01, size=best_norm.shape), 0, 1
                ).astype(np.float32)
                ...
                for c_name, c_val in cands_to_test.items():
                    c_tensor = (
                        torch.tensor(c_val, device=runtime.device)
                        .unsqueeze(0)
                        .unsqueeze(0)
                    )
                    ev = runtime.evaluate_candidates(
                        c_tensor, basin_indices=[b_idx], split="train"
                    )
