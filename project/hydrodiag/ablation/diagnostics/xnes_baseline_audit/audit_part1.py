...
audit_out_dir = os.path.join(
    base_dir, "outputs/ic_ablation/stage1_screening/v1/xnes_audit"
)
data_dir = os.path.join(
    base_dir, "data", "camels_subset"
)  # guess based on typical hydro setups, need to find actual data if different
...
def step1_raw_precision_audit():
    ...
    with open(out_csv, "w", newline="") as f:
        ...
        for res_f in sorted(result_files):
            ...
            model_basin = parts[-2]
            basin_id = (
                model_basin.split("_")[1] if "_" in model_basin else model_basin
            )  # Assuming format model_basin
...
def step4_param_diversity():
    ...
    with open(out_csv, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["basin_id", "seed", "best_theta_norm_hash", "best_theta_phys_hash"]
        )
        for res_f in sorted(result_files):
            task_dir = os.path.dirname(res_f)
            parts = task_dir.split("/")
            ...
            try:
                with open(params_f, "r") as fp:
                    ...
                    n_hash = hashlib.sha256(
                        json.dumps(p.get("best_theta_normalized", [])).encode()
                    ).hexdigest()
                    p_hash = hashlib.sha256(
                        json.dumps(p.get("best_theta_physical", [])).encode()
                    ).hexdigest()
