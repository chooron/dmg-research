...
def _synthetic_checks() -> list[dict[str, object]]:
    ...
    add(
        "one_cfs_one_km2",
        np.isclose(
            convert_ft3s_to_mm_day(np.array([[1.0]]), np.array([1.0]))[0, 0],
            FT3S_TO_MMDAY_FACTOR,
        ),
        repr(FT3S_TO_MMDAY_FACTOR),
    )
    add(
        "100_cfs_1000_km2",
        np.isclose(
            convert_ft3s_to_mm_day(np.array([[100.0]]), np.array([1000.0]))[0, 0],
            0.24465755455488,
            rtol=1e-12,
        ),
        "expected 0.24465755455488",
    )
    add(
        "zero_preserved",
        convert_ft3s_to_mm_day(np.array([[0.0]]), np.array([10.0]))[0, 0] == 0.0,
        "valid zero remains zero",
    )
    add(
        "nan_preserved",
        np.isnan(convert_ft3s_to_mm_day(np.array([[np.nan]]), np.array([10.0]))[0, 0]),
        "NaN remains NaN",
    )
    try:
    ...
    a32 = convert_ft3s_to_mm_day(
        np.ones((2, 3), dtype=np.float32), np.array([10.0, 20.0], dtype=np.float32)
    )
    a64 = convert_ft3s_to_mm_day(
        np.ones((2, 3), dtype=np.float64), np.array([10.0, 20.0], dtype=np.float64)
    )
    add(
        "float32_float64_consistency",
        np.allclose(a32, a64, rtol=1e-6),
        "broadcasted basin factors agree",
    )
    add(
        "vectorized_broadcast",
        np.allclose(a64[0], a64[1] * 2.0),
        "area broadcast over time",
    )
    negative = convert_ft3s_to_mm_day(np.array([[-1.0]]), np.array([10.0]))
    add(
        "negative_not_clipped",
        np.isnan(negative[0, 0]),
        "negative raw value remains invalid",
    )
    return checks


def _old_npz_comparison(
    bundle, config: dict[str, object]
    ...
    if old_ids_path.exists():
        with old_ids_path.open() as handle:
            old_labels = [
                str(value.strip()).zfill(8) for value in handle if value.strip()
            ]
    if len(old_labels) != old_forcing.shape[1]:
        return [], {
            "status": "BLOCKED",
            "reason": "legacy NPZ column count does not match its basin label file",
        }
    ...
    for basin_id in overlap:
        ...
        relative = error[np.abs(new_values[valid]) > 1e-12] / np.abs(
            new_values[valid][np.abs(new_values[valid]) > 1e-12]
        )
        rows.append(
            {
                "basin_id": basin_id,
    ...
    all_abs = np.asarray(
        [row["median_absolute_error"] for row in rows], dtype=np.float64
    )
    all_rel = np.asarray(
        [row["median_relative_error"] for row in rows], dtype=np.float64
    )
    summary = {
        "status": "PASS",
        ...
        if old_labels
        else "UNAVAILABLE_PARENT_WORKTREE",
        ...
        "max_absolute_error": float(
            np.nanmax([row["max_absolute_error"] for row in rows])
        ),
        "mapping_is_old_label_order": old_id_by_position == old_labels
        if old_labels
        else "UNVERIFIED",
...
def main() -> None:
    ...
    atomic_write_csv(
        output_root / "flow_unit_conversion_checks.csv",
        ["check", "status", "detail"],
        checks,
    )
    old_rows, old_summary = _old_npz_comparison(bundle, config)
    atomic_write_csv(
        output_root / "old_npz_overlap_comparison.csv",
        [
            "basin_id",
            "n_valid",
