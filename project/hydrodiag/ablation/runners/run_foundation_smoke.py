...
def _candidate_matrix(model_key: str, population: int) -> np.ndarray:
    ...
    theta += (
        offsets[:, None] * np.linspace(-1.0, 1.0, dimension, dtype=np.float64)[None, :]
    )
    return np.clip(theta, 0.05, 0.95)


def _record_evaluation(
    bundle, model_key: str, evaluation, basin_indices, theta_shape, device
) -> dict[str, object]:
    return {
        "status": "pass",
...
def main() -> None:
    ...
    parser.add_argument(
        "--device",
        default="cpu",
        help="smoke device; CPU is the default to avoid full GPU residency",
    )
    ...
    atomic_write_json(
        output_root / "smoke_result_schema.json",
        {
            "status": "pass",
    ...
    single = single_runtime.evaluate_candidates(
        single_theta, basin_indices=[0], split="train"
    )
    atomic_write_json(
        output_root / "smoke_single_basin.json",
        _record_evaluation(bundle, "XAJ", single, [0], single_theta.shape, args.device),
    )

    multi_theta = _candidate_matrix("XAJ", 8)
    multi_candidate = single_runtime.evaluate_candidates(
        multi_theta, basin_indices=[0], split="train"
    )
    atomic_write_json(
        output_root / "smoke_multi_candidate.json",
        _record_evaluation(
            bundle, "XAJ", multi_candidate, [0], multi_theta.shape, args.device
        ),
    )

    multi_basin_theta = np.stack([multi_theta for _ in range(4)], axis=0)
    multi_basin = single_runtime.evaluate_candidates(
        multi_basin_theta, basin_indices=[0, 1, 2, 3], split="train"
    )
    atomic_write_json(
        output_root / "smoke_multi_basin.json",
        _record_evaluation(
            bundle,
            "XAJ",
            multi_basin,
            [0, 1, 2, 3],
            multi_basin_theta.shape,
            args.device,
        ),
    )
    ...
    for model_key in cross_models:
        ...
        try:
            ...
            evaluation = runtime.evaluate_candidates(
                theta, basin_indices=[0], split="train"
            )
            record = _record_evaluation(
                bundle, model_key, evaluation, [0], theta.shape, args.device
            )
    ...
    atomic_write_json(
        output_root / "smoke_cross_model.json",
        {
            "status": "pass"
            if all(row["status"] == "pass" for row in cross_model_records)
            else "fail",
    ...
    atomic_write_json(
        output_root / "smoke_checkpoint.json",
        {
            "status": "pass",
