...
from .parameter_specs import (
    ...
    XAJ_DE_PARAM_SPECS, XAJ_GE_PARAM_SPECS,
...
def _prepare_xaj_rwpe_parameters(params: dict[str, torch.Tensor]) -> tuple[torch.Tensor, ...]:
...
def _prepare_xaj_2s_parameters(params: dict[str, torch.Tensor]) -> tuple[torch.Tensor, ...]:
    ...
    """
    return (
        params["xaj_k"], params["xaj_b"], params["xaj_im"],
        params["xaj_um"], params["xaj_lm"], params["xaj_dm"],
        params["xaj_c"], params["xaj_sm"], params["xaj_ex"],
        params["xaj_kb"], params["xaj_cb"], params["xaj_a"],
        params["xaj_theta"],
    )
...
def _xaj_2s_step(
    ...
    fr: torch.Tensor, qb: torch.Tensor,
    k: torch.Tensor, b: torch.Tensor, im: torch.Tensor, um: torch.Tensor,
    lm: torch.Tensor, dm: torch.Tensor, c: torch.Tensor, sm: torch.Tensor,
    ex: torch.Tensor, kb: torch.Tensor, cb: torch.Tensor, nearzero: float,
) -> tuple[torch.Tensor, ...]:
    """XAJ step with RI/RG and QI/QG replaced by one merged slow branch."""
    zero = torch.zeros_like(qb)
    wm = um + lm + dm
    out = _xaj_step_impl(
        precip_t, pet_t, wu, wl, wd, s, fr, qb, zero,
        k, b, im, um, lm, dm, c, sm, ex, kb, zero, cb, zero,
        nearzero, wm, wm * (1.0 + b), sm * (1.0 + ex),
        1.0 - im, 1.0 - kb, True,
    )
    (
        q_out, rs_adj, qb_next, _unused_qg, evap_total,
        wu, wl, wd, s_next, fr, rs, rb, _unused_rg, eu, el, ed,
    ) = out
    return q_out, rs_adj, qb_next, evap_total, wu, wl, wd, s_next, fr, rs, rb, eu, el, ed


def _cemaneige_xaj_2s_fused_step(
    precip_t: torch.Tensor, temp_t: torch.Tensor, pet_t: torch.Tensor,
    ...
    xaj_params: tuple[torch.Tensor, ...], nearzero: float,
) -> tuple[torch.Tensor, ...]:
    ...
    return (effective, G, eTG, sca, rain, melt,
...
class _XAJCNVariant(BaseHydrologicalModel):
    ...
    def _split_initial(
        self, initial_states: Optional[dict[str, torch.Tensor]],
    ) -> tuple[Optional[dict[str, torch.Tensor]], Optional[dict[str, torch.Tensor]]]:
        if initial_states is None:
            return None, None
        return (
            {key[3:]: value for key, value in initial_states.items() if key.startswith("cn_")},
            {key[4:]: value for key, value in initial_states.items() if key.startswith("xaj_")},
        )

    def _cn_setup(
        self, precip: torch.Tensor, temp: torch.Tensor, batch: int,
        device: torch.device, dtype: torch.dtype,
        params: dict[str, torch.Tensor], initial_states: Optional[dict[str, torch.Tensor]],
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        cn_initial, _ = self._split_initial(initial_states)
        G, eTG = _init_basic_states(batch, device, dtype, cn_initial)
        return (
            params["cn_ctg"], params["cn_kf"],
            0.9 * _estimate_psol_annual(precip, temp), G, eTG,
        )

    @staticmethod
    def _diagnostic_aux(
        *, qsim: torch.Tensor, effective: list[torch.Tensor], snow: list[torch.Tensor],
        thermal: list[torch.Tensor], sca: list[torch.Tensor], rain: list[torch.Tensor],
        melt: list[torch.Tensor], model_name: str, evaporation_scheme: str,
    ) -> dict[str, Any]:
        return {
            "q_out": qsim,
...
class XAJ2SWithCemaNeige(_XAJCNVariant):
    ...
    def __init__(self, nearzero: float = 1e-8, compact_output: bool = False, compile_step: bool = True):
        super().__init__()
        self.nearzero = nearzero
        self.compact_output = compact_output
        self._fused_step = torch.compile(_cemaneige_xaj_2s_fused_step, fullgraph=True) if compile_step else _cemaneige_xaj_2s_fused_step
    ...
    def forward(self, forcings, params, initial_states=None, return_states=False):
        ...
        k, b, im, um, lm, dm, c, sm, ex, kb, cb, a_uh, theta_uh = _prepare_xaj_2s_parameters(params)
        ...
        if not compact:
            effective: list[torch.Tensor] = []; snow: list[torch.Tensor] = []; thermal: list[torch.Tensor] = []
            sca: list[torch.Tensor] = []; rain: list[torch.Tensor] = []; melt: list[torch.Tensor] = []
            evap: list[torch.Tensor] = []; wu_values: list[torch.Tensor] = []; wl_values: list[torch.Tensor] = []
            wd_values: list[torch.Tensor] = []; s_values: list[torch.Tensor] = []; fr_values: list[torch.Tensor] = []
            rs_raw: list[torch.Tensor] = []; rb_values: list[torch.Tensor] = []; eu_values: list[torch.Tensor] = []
            el_values: list[torch.Tensor] = []; ed_values: list[torch.Tensor] = []
        for t in range(nsteps):
            out = self._fused_step(
                precip[:, t], temp[:, t], pet[:, t], (G, eTG), (wu, wl, wd, s, fr, qb),
                (ctg, kf, g_thresh), (k, b, im, um, lm, dm, c, sm, ex, kb, cb), self.nearzero,
            )
            effective_t, G, eTG, sca_t, rain_t, melt_t, _instant, rs_adj_t, qb, evap_t, wu, wl, wd, s, fr, rs_t, rb_t, eu_t, el_t, ed_t = out
            rs_values.append(rs_adj_t); qb_values.append(qb)
            if not compact:
                effective.append(effective_t); snow.append(G); thermal.append(eTG); sca.append(sca_t); rain.append(rain_t); melt.append(melt_t)
                evap.append(evap_t); wu_values.append(wu); wl_values.append(wl); wd_values.append(wd); s_values.append(s); fr_values.append(fr)
                rs_raw.append(rs_t); rb_values.append(rb_t); eu_values.append(eu_t); el_values.append(el_t); ed_values.append(ed_t)
        rs_store = torch.stack(rs_values, dim=1)
        qb_store = torch.stack(qb_values, dim=1)
        rs_routed, rs_uh_buffer = _route_xaj_surface_runoff(rs_store, rs_uh_buffer, a_uh, theta_uh, device, dtype)
        qsim = rs_routed + qb_store
        if compact:
            return qsim, {}
        aux = self._diagnostic_aux(qsim=qsim, effective=effective, snow=snow, thermal=thermal, sca=sca, rain=rain, melt=melt,
                                   model_name=self.model_name, evaporation_scheme="standard_sequential")
        aux.update({
            "rs_adj": rs_store, "rs_routed": rs_routed, "qb": qb_store, "evap_total": torch.stack(evap, dim=1),
            "wu": torch.stack(wu_values, dim=1), "wl": torch.stack(wl_values, dim=1), "wd": torch.stack(wd_values, dim=1),
            "s_next": torch.stack(s_values, dim=1), "fr": torch.stack(fr_values, dim=1),
            "rs": torch.stack(rs_raw, dim=1), "rb": torch.stack(rb_values, dim=1),
            "eu": torch.stack(eu_values, dim=1), "el": torch.stack(el_values, dim=1), "ed": torch.stack(ed_values, dim=1),
        })
        if return_states:
            aux["final_states"] = {"cn_G": G, "cn_eTG": eTG, "xaj_wu": wu, "xaj_wl": wl, "xaj_wd": wd,
                                   "xaj_s": s, "xaj_fr": fr, "xaj_qb": qb, "xaj_rs_uh_buffer": rs_uh_buffer}
        return qsim, aux
...
class XAJRWPEWithCemaNeige(_XAJCNVariant):
    ...
    def __init__(self, nearzero: float = 1e-8, compact_output: bool = False, compile_step: bool = True):
        ...
        self.nearzero = nearzero
        self.compact_output = compact_output
        self._fused_step = torch.compile(_cemaneige_xaj_rwpe_fused_step, fullgraph=True) if compile_step else _cemaneige_xaj_rwpe_fused_step
        self._compact_step = torch.compile(_xaj_rwpe_step_compact, fullgraph=True) if compile_step else _xaj_rwpe_step_compact
    ...
    def forward(self, forcings, params, initial_states=None, return_states=False):
        ...
        dtype = precip.dtype
        validate_params(params, self.parameter_specs, batch, device, dtype)
        ctg, kf, g_thresh, G, eTG = self._cn_setup(precip, temp, batch, device, dtype, params, initial_states)
        _, xaj_initial = self._split_initial(initial_states)
        xaj_params = _prepare_xaj_rwpe_parameters(params)
        k, b, im, um, lm, dm, tau_e, sm, ex, ki, kg, ci, cg, a_uh, theta_uh = xaj_params
        wu, wl, wd, s, fr, qi, qg, rs_uh_buffer = XAJ._init_states(self, batch, device, dtype, xaj_initial, um, lm, dm, sm)
        compact = self.compact_output and not return_states
        rs_values: list[torch.Tensor] = []; qi_values: list[torch.Tensor] = []; qg_values: list[torch.Tensor] = []
        if not compact:
            effective: list[torch.Tensor] = []; snow: list[torch.Tensor] = []; thermal: list[torch.Tensor] = []
            sca: list[torch.Tensor] = []; rain: list[torch.Tensor] = []; melt: list[torch.Tensor] = []
            evap: list[torch.Tensor] = []; wu_values: list[torch.Tensor] = []; wl_values: list[torch.Tensor] = []
            wd_values: list[torch.Tensor] = []; s_values: list[torch.Tensor] = []; fr_values: list[torch.Tensor] = []
            rs_raw: list[torch.Tensor] = []; ri_values: list[torch.Tensor] = []; rg_values: list[torch.Tensor] = []
            eu_values: list[torch.Tensor] = []; el_values: list[torch.Tensor] = []; ed_values: list[torch.Tensor] = []
            er_values: list[torch.Tensor] = []; z_root_values: list[torch.Tensor] = []; stress_values: list[torch.Tensor] = []
        for t in range(nsteps):
            if compact:
                ...
                rs_adj_t, qi, qg, wu, wl, wd, s, fr = self._compact_step(
                    effective_t, pet[:, t], wu, wl, wd, s, fr, qi, qg, *xaj_params[:-2], self.nearzero,
                    wm, wm * (1.0 + b), sm * (1.0 + ex), 1.0 - im, 1.0 - ki - kg,
                )
            else:
                wl_before, wd_before = wl, wd
                out = self._fused_step(
                    precip[:, t], temp[:, t], pet[:, t], (G, eTG), (wu, wl, wd, s, fr, qi, qg),
                    (ctg, kf, g_thresh), xaj_params[:-2], self.nearzero,
                )
                effective_t, G, eTG, sca_t, rain_t, melt_t, _instant, rs_adj_t, qi, qg, evap_t, wu, wl, wd, s, fr, rs_t, ri_t, rg_t, eu_t, el_t, ed_t = out
                remaining_pet_t = torch.clamp(torch.clamp(pet[:, t] * k, min=0.0) - eu_t, min=0.0)
                _el_check, _ed_check, er_t, z_root_t, stress_t = _rootzone_moisture_stress_evaporation(
                    remaining_pet_t, wl_before, wd_before, lm, dm, tau_e, self.nearzero,
                )
            rs_values.append(rs_adj_t); qi_values.append(qi); qg_values.append(qg)
            if not compact:
                effective.append(effective_t); snow.append(G); thermal.append(eTG); sca.append(sca_t); rain.append(rain_t); melt.append(melt_t)
                evap.append(evap_t); wu_values.append(wu); wl_values.append(wl); wd_values.append(wd); s_values.append(s); fr_values.append(fr)
                rs_raw.append(rs_t); ri_values.append(ri_t); rg_values.append(rg_t); eu_values.append(eu_t); el_values.append(el_t); ed_values.append(ed_t)
                er_values.append(er_t); z_root_values.append(z_root_t); stress_values.append(stress_t)
        rs_store = torch.stack(rs_values, dim=1); qi_store = torch.stack(qi_values, dim=1); qg_store = torch.stack(qg_values, dim=1)
        rs_routed, rs_uh_buffer = _route_xaj_surface_runoff(rs_store, rs_uh_buffer, a_uh, theta_uh, device, dtype)
        qsim = rs_routed + qi_store + qg_store
        if compact:
            return qsim, {}
        aux = self._diagnostic_aux(qsim=qsim, effective=effective, snow=snow, thermal=thermal, sca=sca, rain=rain, melt=melt,
                                   model_name=self.model_name, evaporation_scheme="aggregated_rootzone_moisture_stress")
        aux.update({
            "rs_adj": rs_store, "rs_routed": rs_routed, "qi": qi_store, "qg": qg_store, "evap_total": torch.stack(evap, dim=1),
            "wu": torch.stack(wu_values, dim=1), "wl": torch.stack(wl_values, dim=1), "wd": torch.stack(wd_values, dim=1),
            "s_next": torch.stack(s_values, dim=1), "fr": torch.stack(fr_values, dim=1),
            "rs": torch.stack(rs_raw, dim=1), "ri": torch.stack(ri_values, dim=1), "rg": torch.stack(rg_values, dim=1),
            "eu": torch.stack(eu_values, dim=1), "el": torch.stack(el_values, dim=1), "ed": torch.stack(ed_values, dim=1),
            "er": torch.stack(er_values, dim=1), "z_root": torch.stack(z_root_values, dim=1),
            "root_stress": torch.stack(stress_values, dim=1), "tau_e": tau_e,
        })
        if return_states:
            aux["final_states"] = {"cn_G": G, "cn_eTG": eTG, "xaj_wu": wu, "xaj_wl": wl, "xaj_wd": wd,
                                   "xaj_s": s, "xaj_fr": fr, "xaj_qi": qi, "xaj_qg": qg, "xaj_rs_uh_buffer": rs_uh_buffer}
        return qsim, aux
...
class XAJ2SWithCemaNeigeLite(XAJ2SWithCemaNeige):
    ...
    def __init__(self, nearzero: float = 1e-8, compile_step: bool = True):
        super().__init__(nearzero=nearzero, compact_output=True, compile_step=compile_step)
...
def _xaj_structure_step_full(
    precip_t: torch.Tensor, pet_t: torch.Tensor,
    wu: torch.Tensor, wl: torch.Tensor, wd: torch.Tensor,
    s: torch.Tensor, fr: torch.Tensor, q1: torch.Tensor, q2: torch.Tensor,
    k: torch.Tensor, b: torch.Tensor, im: torch.Tensor,
    um: torch.Tensor, lm: torch.Tensor, dm: torch.Tensor, c: torch.Tensor,
    sm: torch.Tensor, ex: torch.Tensor, p1: torch.Tensor, p2: torch.Tensor,
    ci: torch.Tensor, cg: torch.Tensor, a_uh: torch.Tensor,
    theta_uh: torch.Tensor, nearzero: float, wm: torch.Tensor,
    wmm: torch.Tensor, ms: torch.Tensor, one_minus_im: torch.Tensor,
    one_minus_p1_p2: torch.Tensor, variant: int,
    gamma: torch.Tensor, tau_0: torch.Tensor, beta: torch.Tensor,
    z0: torch.Tensor,
) -> tuple[torch.Tensor, ...]:
    ...
    return _xaj_step_impl(
        precip_t, pet_t, wu, wl, wd, s, fr, q1, q2,
        k, b, im, um, lm, dm, c, sm, ex, p1, p2, ci, cg,
        nearzero, wm, wmm, ms, one_minus_im, one_minus_p1_p2, True,
        evaporation_scheme, None, response_scheme, gamma, tau_0, beta, z0,
    )
...
def _xaj_structure_step_compact(
    ...
    return _xaj_step_impl(
        precip_t, pet_t, wu, wl, wd, s, fr, q1, q2,
        k, b, im, um, lm, dm, c, sm, ex, p1, p2, ci, cg,
        nearzero, wm, wmm, ms, one_minus_im, one_minus_p1_p2, False,
        evaporation_scheme, None, response_scheme, gamma, tau_0, beta, z0,
    )
...
class _XAJStructureVariant(BaseHydrologicalModel):
    ...
    def __init__(self, nearzero: float = 1e-8, *, compact_output: bool = False,
                 compile_step: bool = True, z0: float | torch.Tensor = DEFAULT_Z0):
        super().__init__()
        self.nearzero = nearzero
        self.compact_output = compact_output
        ...
        self._step = torch.compile(_xaj_structure_step_full, fullgraph=True) if compile_step else _xaj_structure_step_full
        self._compact_step = torch.compile(_xaj_structure_step_compact, fullgraph=True) if compile_step else _xaj_structure_step_compact

    @property
    def parameter_specs(self) -> dict[str, dict[str, Any]]:
        return self.parameter_spec

    def _prepare_parameters(self, params: dict[str, torch.Tensor], batch: int) -> tuple[torch.Tensor, ...]:
        if self.response_variant:
            ...
            return (
                k, b, im, um, lm, dm, c, sm, ex,
                kss, zero, torch.ones_like(kss), torch.ones_like(kss),
                params["xaj_a"], params["xaj_theta"],
                tau_0, beta,
            )
        ...
        k, b, im, um, lm, dm, _c, sm, ex, ki, kg, ci, cg, a, theta = _prepare_xaj_parameters(with_c)
        gamma = params["xaj_gamma"] if self.generic_variant else torch.ones_like(k)
        return k, b, im, um, lm, dm, _c, sm, ex, ki, kg, ci, cg, a, theta, torch.ones_like(k), gamma
    ...
    def _init_variant_states(
        ...
        um: torch.Tensor, lm: torch.Tensor, dm: torch.Tensor, sm: torch.Tensor,
    ) -> tuple[torch.Tensor, ...]:
    ...
    def forward(self, forcings, params, initial_states=None, return_states=False):
        ...
        (k, b, im, um, lm, dm, c, sm, ex, p1, p2, ci, cg,
         a_uh, theta_uh, tau_0, beta) = prepared
        gamma = params["xaj_gamma"] if self.generic_variant and not self.response_variant else torch.ones_like(k)
        ...
        if not compact:
            ...
            eu_values: list[torch.Tensor] = []; el_values: list[torch.Tensor] = []; ed_values: list[torch.Tensor] = []
            wu_values: list[torch.Tensor] = []; wl_values: list[torch.Tensor] = []; wd_values: list[torch.Tensor] = []
            s_values: list[torch.Tensor] = []; fr_values: list[torch.Tensor] = []
            response_input_values: list[torch.Tensor] = []; response_storage_values: list[torch.Tensor] = []
            response_available_values: list[torch.Tensor] = []
            qi_values: list[torch.Tensor] = []; qg_values: list[torch.Tensor] = []
        for t in range(nsteps):
            out = (self._compact_step if compact else self._step)(
                precip[:, t], pet[:, t], wu, wl, wd, s, fr, q1, q2,
                k, b, im, um, lm, dm, c, sm, ex, p1, p2, ci, cg,
                a_uh, theta_uh, self.nearzero, wm, wm * (1.0 + b),
                sm * (1.0 + ex), 1.0 - im, 1.0 - p1 - p2,
                self.variant, gamma, tau_0, beta, z0,
            )
            if compact and self.response_variant:
                ...
                z_old = q1
                (_q, rs_t, qss_t, evap_t, wu, wl, wd, s, fr,
                 rs_raw_t, r_ss_t, _rg_t, eu_t, el_t, ed_t,
                 z_new, _r_ss_diag) = out
                rs_values.append(rs_t); baseflow_values.append(qss_t)
                q_values.append(_q); rs_raw_values.append(rs_raw_t); evap_values.append(evap_t)
                eu_values.append(eu_t); el_values.append(el_t); ed_values.append(ed_t)
                wu_values.append(wu); wl_values.append(wl); wd_values.append(wd); s_values.append(s); fr_values.append(fr)
                response_input_values.append(r_ss_t); response_storage_values.append(z_new)
                response_available_values.append(torch.clamp(z_old, min=0.0) + torch.clamp(r_ss_t, min=0.0))
                qi_values.append(qss_t); qg_values.append(torch.zeros_like(qss_t))
                q1, q2 = z_new, torch.zeros_like(z_new)
            else:
                (_q, rs_t, qi_t, qg_t, evap_t, wu, wl, wd, s, fr,
                 rs_raw_t, ri_t, rg_t, eu_t, el_t, ed_t) = out
                rs_values.append(rs_t); baseflow_values.append(qi_t + qg_t)
                q_values.append(_q); rs_raw_values.append(rs_raw_t); evap_values.append(evap_t)
                eu_values.append(eu_t); el_values.append(el_t); ed_values.append(ed_t)
                wu_values.append(wu); wl_values.append(wl); wd_values.append(wd); s_values.append(s); fr_values.append(fr)
                qi_values.append(qi_t); qg_values.append(qg_t)
        ...
        if self.use_hydrodl2_uh:
            rs_routed, rs_uh_buffer = _route_xaj_surface_runoff_hydrodl2(rs_store, rs_uh_buffer, a_uh, theta_uh)
        else:
            rs_routed, rs_uh_buffer = _route_xaj_surface_runoff(rs_store, rs_uh_buffer, a_uh, theta_uh, device, dtype)
        ...
        aux = {
            "q_out": qsim, "evap": torch.stack(evap_values, dim=1),
            "evap_total": torch.stack(evap_values, dim=1),
            "rs_instant": rs_store, "rs_routed": rs_routed,
            "rs": torch.stack(rs_raw_values, dim=1),
            "wu": torch.stack(wu_values, dim=1), "wl": torch.stack(wl_values, dim=1),
            "wd": torch.stack(wd_values, dim=1), "s_next": torch.stack(s_values, dim=1),
            "fr": torch.stack(fr_values, dim=1), "eu": eu_store,
            "el": el_store, "ed": ed_store,
            ...
            "evaporation_scheme": "parallel_linear" if self.variant == 0 else "parallel_power" if self.variant == 1 else "native_sequential",
        }
        if self.response_variant:
            ...
            conditioning = summarize_response_conditioning(
                aux["z_available"], aux["z"], z0.unsqueeze(1),
            )
        ...
        if return_states:
            final = {"wu": wu, "wl": wl, "wd": wd, "s": s, "fr": fr, "rs_uh_buffer": rs_uh_buffer}
            if self.response_variant:
                final["z"] = q1
            else:
                final["qi"] = q1; final["qg"] = q2
...
class XAJDELite(XAJDE):
    def __init__(self, nearzero: float = 1e-8, compile_step: bool = True, z0: float | torch.Tensor = DEFAULT_Z0):
        super().__init__(nearzero, compact_output=True, compile_step=compile_step, z0=z0)
