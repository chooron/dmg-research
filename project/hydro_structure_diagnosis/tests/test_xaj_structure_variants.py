...
from models import (
    ...
    DE, GE, DR, GR,
    normalized_to_beta, normalized_to_gamma, normalized_to_kss, normalized_to_tau0,
...
E_CASES = ((XAJDE, XAJDELite, XAJ_DE_PARAM_SPECS), (XAJGE, XAJGELite, XAJ_GE_PARAM_SPECS))
R_CASES = ((XAJDR, XAJDRLite, XAJ_DR_PARAM_SPECS), (XAJGR, XAJGRLite, XAJ_GR_PARAM_SPECS))
...
def _forcing(steps: int = 64, dtype: torch.dtype = torch.float64):
    ...
    return {"precip": precip.unsqueeze(0), "pet": pet.unsqueeze(0), "temp": temp.unsqueeze(0)}


def _params(specs, dtype=torch.float64, batch=1, requires_grad=False):
    return {
        name: torch.full((batch,), float(spec["default"]), dtype=dtype, requires_grad=requires_grad)
...
def test_controlled_variant_eager_compiled_full_lite_and_gradients(full_cls, lite_cls, specs):
    forcing = _forcing(48)
    params = _active_params(specs, requires_grad=True)
    initial = _initial(full_cls, specs)
    eager, aux = full_cls(compile_step=False)(forcing, params, initial_states=initial, return_states=True)
    compiled, _ = full_cls()(forcing, _active_params(specs), initial_states=initial, return_states=True)
    ...
    active_name = next((n for n in ("xaj_gamma", "xaj_beta", "xaj_tau0", "xaj_kss") if n in params), None)
...
def _balance_residual(model_cls, specs, steps=96):
    ...
    if "R" in model_cls.__name__:
        ...
        initial_storage = (
            initial_soil + initial_free
            + params["xaj_ci"] * initial["qi"] / (1.0 - params["xaj_ci"])
            + params["xaj_cg"] * initial["qg"] / (1.0 - params["xaj_cg"])
        )
    delta = torch.cat((storage[:, :1] - initial_storage.unsqueeze(1), storage[:, 1:] - storage[:, :-1]), dim=1)
...
def test_controlled_variant_water_balance_and_long_sequence(full_cls, lite_cls, specs):
    ...
    for key, cap in (("wu", "xaj_um"), ("wl", "xaj_lm"), ("wd", "xaj_dm"), ("s_next", "xaj_sm")):
    ...
    q, long_aux = full_cls(compile_step=False)(long_forcing, p, initial_states=_initial(full_cls, specs), return_states=True)
    assert torch.isfinite(q).all()
    assert all(torch.isfinite(v).all() for v in long_aux["final_states"].values())
    assert min(float(long_aux[k].min()) for k in ("wu", "wl", "wd", "s_next", "fr")) >= -1e-10
...
def test_controlled_float32_float64_forward_and_gradient_consistency():
    ...
    for full_cls, _lite_cls, specs in ALL_CASES:
        def run(dtype, forcing):
            ...
            q.sum().backward()
            grad = torch.cat([params[name].grad.reshape(-1).to(torch.float64) for name in specs])
...
def test_module_autograd_matches_normalized_finite_difference():
    ...
    loss = GE(compile_step=False)(er, wl, wd, lm, dm, normalized_to_gamma(n_gamma))[0].sum()
    loss.backward()
    fd_gamma = _central(lambda x: float(GE(compile_step=False)(er, wl, wd, lm, dm, normalized_to_gamma(torch.tensor([x], dtype=dtype)))[0]), .62)
    ...
    loss.backward()
    fd_beta = _central(lambda x: float(GR(compile_step=False)(r, z, tau, normalized_to_beta(torch.tensor([x], dtype=dtype)))[0]), .57)
    ...
    loss.backward()
    fd_tau = _central(lambda x: float(DR(compile_step=False)(r, z, normalized_to_tau0(torch.tensor([x], dtype=dtype)))[0]), .55)
...
def test_gamma_grid_is_finite_and_conservative(gamma):
    dtype = torch.float64
    x = torch.tensor([0.0, 1e-8, 1e-6, 1e-4, .01, .5, .999, 1.0], dtype=dtype, requires_grad=True)
    wl = 80.0 * x
    wd = 40.0 * x
    g = torch.full_like(x, gamma, requires_grad=True)
    out = GE(compile_step=False)(torch.full_like(x, 3.0), wl, wd, torch.full_like(x, 80.0), torch.full_like(x, 40.0), g)
...
def test_beta_grid_and_z0_conditioning(beta):
    ...
    for z0_value in (1e-3, 1.0, 1e3):
        ...
        z_raw = torch.tensor([0.0, 1e-8, 1e-6, 1e-3, .1, 1., 10., 100.], dtype=dtype, requires_grad=True)
        z = z_raw * z0_value
        tau = torch.tensor([.5, 1., 10., 10., 10., 22., 100., 1000.], dtype=dtype, requires_grad=True)
        b = torch.full_like(z, beta, requires_grad=True)
        out = GR(nearzero=1e-12, z0=z0, compile_step=False)(torch.zeros_like(z), z, tau, b)
        (out[0].sum() + out[1].sum()).backward()
        assert torch.isfinite(torch.stack(out)).all()
        assert torch.isfinite(z_raw.grad).all() and torch.isfinite(tau.grad).all() and torch.isfinite(b.grad).all()
        assert out[0][0] == 0 and out[1][0] == 0
        assert torch.all(out[0] >= 0) and torch.all(out[0] <= z)
        assert torch.all(out[1] >= 0)
        assert torch.isfinite(torch.log(torch.clamp(z.detach() / z0_value, min=1e-30))).all()


def test_native_kss_mapping_and_response_input():
    ki = torch.tensor([0.0, .3, .7, .7], dtype=torch.float64)
    kg = torch.tensor([0.0, .2, .7, .7], dtype=torch.float64)
    effective = native_effective_kss(ki, kg)
    assert torch.allclose(effective, torch.tensor([0.0, .5, .99999, .99999], dtype=torch.float64))
    assert torch.allclose(normalized_to_kss(torch.tensor([0., .5, 1.], dtype=torch.float64)), torch.tensor([0., .499995, .99999], dtype=torch.float64))


@pytest.mark.parametrize("full_cls,lite_cls,specs", ALL_CASES)
def test_controlled_variant_parameter_and_state_boundary_sweep(full_cls, lite_cls, specs):
    dtype = torch.float64
    forcing = _forcing(12, dtype)
    boundary_name = next((n for n in ("xaj_gamma", "xaj_beta", "xaj_tau0", "xaj_kss") if n in specs), None)
    ...
    for normalized in boundary_values:
        ...
        if boundary_name == "xaj_gamma":
            p[boundary_name] = normalized_to_gamma(torch.tensor([normalized], dtype=dtype))
        elif boundary_name == "xaj_beta":
            p[boundary_name] = normalized_to_beta(torch.tensor([normalized], dtype=dtype))
        elif boundary_name == "xaj_tau0":
            p[boundary_name] = normalized_to_tau0(torch.tensor([normalized], dtype=dtype))
        elif boundary_name == "xaj_kss":
            p[boundary_name] = normalized_to_kss(torch.tensor([normalized], dtype=dtype))
        for state_mode in ("zero", "full"):
            ...
            if "R" in full_cls.__name__:
                ...
                er = torch.clamp(forcing["pet"] * p["xaj_k"].unsqueeze(1) - aux["eu"], min=0.0)
                assert torch.all(aux["el"] + aux["ed"] <= er + 2e-10)


@pytest.mark.parametrize("full_cls,specs", ((XAJDR, XAJ_DR_PARAM_SPECS), (XAJGR, XAJ_GR_PARAM_SPECS)))
def test_controlled_kss_normalized_finite_difference(full_cls, specs):
    ...
    loss = full_cls(compile_step=False)(forcing, params, initial_states=_initial(full_cls, specs, dtype))[0].sum()
    loss.backward()

    def evaluate(normalized):
        candidate = {name: value.clone() for name, value in base.items()}
        candidate["xaj_kss"] = normalized_to_kss(torch.tensor([normalized], dtype=dtype))
        return float(full_cls(compile_step=False)(forcing, candidate, initial_states=_initial(full_cls, specs, dtype))[0].sum())
...
def test_beta_one_is_d_r_and_extinction_is_conservative():
    ...
    z = torch.tensor([0., 1e-8, .01], dtype=dtype)
    tau = torch.full((3,), 10., dtype=dtype)
    ...
    q, zn, za, *_ = _analytic_subsurface_response_step(
        torch.zeros(1, dtype=dtype), torch.zeros(1, dtype=dtype),
        torch.full((1,), .5, dtype=dtype), torch.full((1,), .5, dtype=dtype),
        torch.ones(1, dtype=dtype), 1e-12,
    )
