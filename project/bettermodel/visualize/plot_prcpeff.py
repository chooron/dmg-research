...
def load_model_inner_state(out_path):
    ...
    inner_soilwetness = np.load(os.path.join(out_path, "soilwetness_full.npy"))
    inner_soilwater = np.load(os.path.join(out_path, "SM_full.npy"))
    return inner_evap, inner_soilwetness, inner_soilwater
...
lstm_evap_full, lstm_soilwetness_full, lstm_sm_full = load_model_inner_state(
    lstm_config["out_path"]
)
hope_evap_full, hope_soilwetness_full, hope_sm_full = load_model_inner_state(
    hope_config["out_path"]
)
select_lstm_evap = lstm_evap_full[
    select_idx[0] : select_idx[1], select_basin_idx, :
]
select_lstm_soilwetness = lstm_soilwetness_full[
    select_idx[0] : select_idx[1], select_basin_idx, :
]
select_lstm_sm = lstm_sm_full[
    select_idx[0] : select_idx[1], select_basin_idx, :
]
select_hope_evap = hope_evap_full[
    select_idx[0] : select_idx[1], select_basin_idx, :
]
select_hope_soilwetness = hope_soilwetness_full[
    select_idx[0] : select_idx[1], select_basin_idx, :
]
...
lstm_timevar_beta_denormalized = lstm_timevar_params_denormalized[:, 0, :]
lstm_timevar_gamma_denormalized = lstm_timevar_params_denormalized[:, 2, :]
lstm_display_data = np.stack(
    [
        lstm_timevar_beta_denormalized,
        lstm_timevar_gamma_denormalized,
        select_lstm_sm,
        select_lstm_soilwetness,
        select_lstm_evap,
    ],
    axis=1,
)
...
hope_timevar_beta_denormalized = hope_timevar_params_denormalized[:, 0, :]
hope_timevar_gamma_denormalized = hope_timevar_params_denormalized[:, 2, :]
hope_display_data = np.stack(
    [
        hope_timevar_beta_denormalized,
        hope_timevar_gamma_denormalized,
        select_hope_sm,
        select_hope_soilwetness,
        select_hope_evap,
    ],
    axis=1,
)
...
plot_parameters(
    lstm_display_data,
    titles=[r"$\beta$",r"$\gamma$", "Soilwater", "Wetness", "ET"],
...
plot_temporal_volatility_boxplot(
    ...
    label_fontsize=20,
    tick_fontsize=18,
    var_names=[r"$\beta$", r"$\gamma$", "Soilwater", "Wetness", "ET"],
...
fig.savefig(
    os.path.join(
        os.getenv("PROJ_PATH"),
        f"project/better_estimate/visualize/figures/{basin_id}_recharge_inner_process.png",
    ),
    dpi=300,
)
