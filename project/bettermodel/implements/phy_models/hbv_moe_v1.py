...
from project.bettermodel.implements.neural_networks.layers.moe import MoeLayer


class HbvMoeV1(torch.nn.Module):
    """HBV 2.0 ~.

    HBV 模型夹带 Moe模块, 让他自动分配每个输出结果的权重值
    其中HBV预测的参数都是静态参数,无需LSTM等模型预测动态参数
    ...
    def set_parameters(self) -> None:
        ...
        self.learnable_param_count1 = 0
        self.learnable_param_count2 = len(
            self.phy_param_names
        ) * self.nmul + len(self.routing_param_names)
        self.learnable_param_count = self.learnable_param_count2
    ...
    def unpack_parameters(
        ...
        phy_static_params = parameters[
            :, : phy_param_count * self.nmul
        ].view(
            parameters.shape[0],
            phy_param_count,
            self.nmul,
        ...
        routing_params = None
        if self.routing:
            routing_params = parameters[:, phy_param_count * self.nmul :]

        return phy_static_params, routing_params
    ...
    def forward(
        ...
        # Unpack parameters.
        phy_static_params, routing_params = self.unpack_parameters(parameters)

        if self.routing:
            self.routing_param_dict = self.descale_route_parameters(
                routing_params
            )
        ...
        phy_params_dict = self.descale_phy_stat_parameters(
            phy_static_params,
            stat_list=[
                param
    ...
    def PBM(
        ...
        Qsimmu = (
            torch.zeros(Pm.size(), dtype=torch.float32, device=self.device)
            + 0.001
        )
        Q0_sim = (
            torch.zeros(Pm.size(), dtype=torch.float32, device=self.device)
        ...
        excs_sim = torch.zeros(
            Pm.size(), dtype=torch.float32, device=self.device
        )
        evapfactor_sim = torch.zeros(
            Pm.size(), dtype=torch.float32, device=self.device
        )
        tosoil_sim = torch.zeros(
            Pm.size(), dtype=torch.float32, device=self.device
        )
        PERC_sim = torch.zeros(
            Pm.size(), dtype=torch.float32, device=self.device
        )
        SWE_sim = torch.zeros(
            Pm.size(), dtype=torch.float32, device=self.device
        ...
        param_dict = phy_static_params_dict
        for t in range(n_steps):
            ...
            SUZ = SUZ - PERC
            Q0 = param_dict["parK0"] * torch.clamp(
                SUZ - param_dict["parUZL"], min=0.0
            )
            SUZ = SUZ - Q0
        ...
        # Moe layer
        moe_input = torch.stack(
            [SWE_sim, SM_sim, SUZ_sim, SLZ_sim, Qsimmu], dim=-1
        )
        output_weights = self.moe_layer(moe_input).squeeze()
        Qsimavg = (Qsimmu * output_weights).sum(-1)
        ...
        if self.routing:
            ...
            UH = uh_gamma(
                self.routing_param_dict["rout_a"]
                .repeat(n_steps, 1)
                .unsqueeze(-1),
                self.routing_param_dict["rout_b"]
                .repeat(n_steps, 1)
                .unsqueeze(-1),
                lenF=15,
            )
            rf = torch.unsqueeze(Qsim, -1).permute(
                [1, 2, 0]
            )  # [gages,vars,time]
        ...
        if self.initialize:
            ...
            BFI_sim = (
                100
                * (
                    torch.sum(Q2_rout, dim=0)
                    / (torch.sum(Qs, dim=0) + self.nearzero)
                )[:, 0]
            )
            ...
            out_dict = {
                ...
                "srflow_no_rout": Q0_sim.mean(
                    -1, keepdim=True
                ),  # Surface runoff
                "ssflow_no_rout": Q1_sim.mean(
                    -1, keepdim=True
                ),  # Subsurface flow
                "gwflow_no_rout": Q2_sim.mean(
                    -1, keepdim=True
                ),  # Groundwater flow
                ...
                "capillary": capillary_sim.mean(
                    -1, keepdim=True
                ),  # Capillary rise
                "moe_weight": output_weights.squeeze(),
            }
