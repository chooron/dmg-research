...
class Hmets(torch.nn.Module):
    ...
    def descale_phy_parameters(
        self, phy_params: torch.Tensor, dy_list: list
    ) -> dict:
        """将归一化的物理参数去归一化到其物理范围。"""
        ...
        for i, name in enumerate(self.phy_param_names):
            # 静态参数使用最后一个时间步的值
            staPar = (
                phy_params[-1, :, i, :].unsqueeze(0).repeat([n_steps, 1, 1])
            )
            if name in dy_list:
                dynPar = phy_params[:, :, i, :]
                ...
                param_dict[name] = change_param_range(
                    comPar, self.parameter_bounds[name]
                )
            else:
                param_dict[name] = change_param_range(
                    staPar, self.parameter_bounds[name]
                )
        return param_dict
    ...
    def forward(
        ...
        vadose_max_init = change_param_range(
            torch.sigmoid(
                phy_params_last_step[
                    :, self.phy_param_names.index("vadose_max_level")
                ]
            ),
    ...
    def PBM(
        ...
        for t in range(n_steps):
            ...
            ddf = torch.min(
                ...
                param_dict["ddf_min"] * (1 + param_dict["Kcum"] * CUMSNOWMELT),
            )
            snowmelt_potential = torch.clamp(
                ddf * (Tm[t] - param_dict["Tbm"]), min=0
            )

            snowmelt = torch.min(snowmelt_potential, SNOW_ON_GROUND + SNOW)
            SNOW_ON_GROUND = SNOW_ON_GROUND + SNOW - snowmelt

            CUMSNOWMELT = (CUMSNOWMELT + snowmelt) * (
                SNOW_ON_GROUND > self.nearzero
            ).float()

            water_retention_fraction = torch.clamp(
                (param_dict["fcmin"] + param_dict["fcmin_plus"])
