...
from project.bettermodel.implements.phy_models.fluxes import *
...
class DropoutMean(nn.Module):
    ...
    def forward(self, x):
        # 步骤 1: 创建一个掩码，标记出 x 中原始的非零元素位置
        # (x != 0) 会生成一个布尔张量，例如 [True, False, True, ...]
        original_nonzero_mask = (x != 0)
        ...
        if self.training:
            ...
            # 步骤 3: 结合掩码进行计算
            # 我们只关心那些“原始非零”且“经过dropout后依然非零”的元素

            # 计算求和：只对原始非零位置的值求和。
            # dropout_result * original_nonzero_mask 会将原始为0的位置彻底清零，
            # 保证了我们只计算我们关心的部分。
            sum_result = torch.sum(dropout_result * original_nonzero_mask, dim=1)

            # 计算数量：只统计原始非零位置中，经过dropout后依然非零的元素数量。
            # (dropout_result != 0) 找出dropout后的非零元素
            # & original_nonzero_mask 确保我们只关注原始非零的那些
            # .sum(dim=1) 统计数量
            survivor_counts = ((dropout_result != 0) & original_nonzero_mask).sum(dim=1)

            # 步骤 4: 计算均值
            correct_mean = sum_result / (survivor_counts + 1e-8)

        else:  # 如果是评估模式 (self.eval())
            # Dropout 不起作用，直接计算原始非零元素的均值
            correct_mean = torch.mean(x, dim=1)

        return correct_mean.unsqueeze(1)


class blendv5(torch.nn.Module):
    """
    ...
    def __init__(
        ...
        self.nmul = 1
        self.nearzero = 1e-6
        self.infil_dropout = DropoutMean(p=0.25)
        self.bf1_dropout = DropoutMean(p=0.25)
        self.bf2_dropout = DropoutMean(p=0.25)
        ...
        self.parameter_bounds = {
            ...
            'bf1_power_bfc': [-3.0, -0.0],  # [BF1-PL] Power Law Baseflow BFc1
            'bf1_power_bfn': [0.2, 2.0],  # [BF1-PL] Power Law Baseflow BFn1
            'bf1_vic_bfn': [0.5, 2.0],  # [BF1-PL] Power Law Baseflow BFn1
            'bf1_top_bfn': [0.5, 2.0],  # [BF1-PL] Power Law Baseflow BFn1
            'bf1_thr_bfn': [0.5, 2.0],  # [BF1-PL] Power Law Baseflow BFn1
            'bf1_bfmax': [0.1, 200.0],  # [BF1-VIC] VIC Method BFmax1
            'bf1_lam': [1.0, 20.0],  # [BF1-TOPMOD] TOPMODEL Method lam1
            'bf1_thresh': [0.0, 0.2],  # [BF1-THRESH] Threshold Baseflow BFThresh1
            'bf1_exp': [0.0001, 1.0],  # [BF1-THRESH] Threshold Baseflow BFThresh1
            ...
            'bf2_gr4j_x3': [0.00001, 200.0],  # [BF2-GR4J] GR4J Method X3-2
            'bf2_lin_bfc': [-3.0, -0.0],  # [BF2-LIN] Linear Baseflow BFc2
            'bf2_pl_bfc': [-3.0, -0.0],  # [BF2-LIN] Linear Baseflow BFc2
            'bf2_pl_bfn': [1.0, 5.0],  # [BF2-PL] Power Law Baseflow BFn2
            'bf2_vic_bfn': [1.0, 5.0],  # [BF2-PL] Power Law Baseflow BFn2
    ...
    def PBM(
        ...
        for t in range(n_steps):
            ...
            # -- 基流 --
            # baseflow_gr4j_ = baseflow_gr4j_exchange(soilwater1_, param_dict['bf1_gr4j_x3'], param_dict['bf1_bfmax'])
            baseflow1_power_ = baseflow_power_law(soilwater1_, param_dict['bf1_power_bfc'], param_dict['bf1_power_bfn'],
                                                  param_dict['bf1_bfmax'])
            baseflow1_exp_ = baseflow_exp(soilwater1_, param_dict['max_soilwater1'], param_dict['bf1_bfmax'], param_dict['bf1_exp'])
            baseflow1_vic_ = baseflow_vic(soilwater1_, param_dict['max_soilwater1'],
                                          param_dict['bf1_bfmax'], param_dict['bf1_vic_bfn'])
            baseflow1_topmodel_ = baseflow_topmodel(soilwater1_,
                                                    param_dict['max_soilwater1'], param_dict['bf1_bfmax'],
                                                    param_dict['bf1_lam'], param_dict['bf1_top_bfn'])
            baseflow1_threshold_ = baseflow_threshold(soilwater1_,
                                                      param_dict['max_soilwater1'], param_dict['bf1_bfmax'],
                                                      param_dict['bf1_thr_bfn'], param_dict['bf1_thresh'])
            baseflow1_arr = torch.concatenate([baseflow1_power_, baseflow1_exp_, baseflow1_vic_,
            ...
            baseflow2_lin_ = baseflow_linear(soilwater2_, param_dict['bf2_lin_bfc'], param_dict['bf2_bfmax'])
            baseflow2_pl_ = baseflow_power_law(soilwater2_, param_dict['bf2_pl_bfc'], param_dict['bf2_pl_bfn'],
                                               param_dict['bf2_bfmax'])
            baseflow2_vic_ = baseflow_vic(soilwater2_, param_dict['max_soilwater2'],
                                          param_dict['bf2_bfmax'], param_dict['bf2_vic_bfn'])
            ...
            for i, baseflow1 in enumerate([baseflow1_exp_,  # baseflow1_exp_,
