#!/usr/bin/env bash
# Run KGE-baseline checkpoint testing with MC-mean evaluation.
# Usage:
#   bash scripts/run_test_kge_dhbv.sh
#   bash scripts/run_test_kge_dhbv.sh A
#   bash scripts/run_test_kge_dhbv.sh A 42
#   bash scripts/run_test_kge_dhbv.sh A 42 50
#
# Optional environment overrides:
#   CONFIG=./conf/config_kge_dhbv.yaml
#   CLUSTERS="A B C"
#   SEEDS="42 123 456"
#   EPOCH=50
#   MC_SAMPLES=100
...
CONFIG=${CONFIG:-${PROJECT_DIR}/conf/config_kge_dhbv.yaml}
SCRIPT=${SCRIPT:-test_kge_baseline.py}
AGG_SCRIPT=${AGG_SCRIPT:-aggregate_holdout_results.py}
DATASET=${DATASET:-holdout}
EPOCH=${EPOCH:-50}
MC_SAMPLES=${MC_SAMPLES:-10}
SUMMARY_OUTPUT=${SUMMARY_OUTPUT:-./outputs/kge_dhbv_test_${DATASET}_epoch${EPOCH}_mc${MC_SAMPLES}_summary.csv}
...
run_single() {
    ...
    echo "========================================"
    echo " KGE Test | held-out cluster ${cluster} | seed ${seed} | epoch ${epoch}"
...
aggregate_results() {
    ...
    uv run python "${AGG_SCRIPT}" \
        --input-glob "./outputs/kge_dhbv_held_out_*_seed*/test_${DATASET}_epoch${epoch}_mc${MC_SAMPLES}/results_held_out_*_seed*.csv" \
