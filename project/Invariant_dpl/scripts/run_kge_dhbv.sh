#!/usr/bin/env bash
# Run KGE-baseline dPL leave-one-effective-cluster validation.
# Usage:
#   bash scripts/run_kge_dhbv.sh
#   bash scripts/run_kge_dhbv.sh A
#   bash scripts/run_kge_dhbv.sh A 42
#
...
CONFIG=${CONFIG:-${REPO_ROOT}/conf/config_kge_dhbv.yaml}
SCRIPT=${SCRIPT:-train_kge_baseline.py}
AGG_SCRIPT=${AGG_SCRIPT:-aggregate_holdout_results.py}
MODE=${MODE:-train_test}
OUTPUT_DIR=${OUTPUT_DIR:-./outputs}
OUTPUT_MEAN=${OUTPUT_MEAN:-${OUTPUT_DIR}/kge_dhbv_7fold_summary_mc_mean.csv}
OUTPUT_AVG=${OUTPUT_AVG:-${OUTPUT_DIR}/kge_dhbv_7fold_summary_mc_selected.csv}
...
run_single() {
    ...
    echo "========================================"
    echo " KGE Baseline | held-out cluster ${cluster} | seed ${seed}"
...
aggregate_results() {
    ...
    uv run python "${AGG_SCRIPT}" \
        --input-glob "./outputs/kge_dhbv_held_out_*_seed*/results_held_out_*_seed*.csv" \
        --output "${OUTPUT_MEAN}" \
        --expect-repeats "${repeats}"

    uv run python "${AGG_SCRIPT}" \
        --input-glob "./outputs/kge_dhbv_held_out_*_seed*/results_avg_held_out_*_seed*.csv" \
        --output "${OUTPUT_AVG}" \
        --expect-repeats "${repeats}"
}
