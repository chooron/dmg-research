#!/usr/bin/env python3
"""Final Evaluation and 6-Way Benchmark for R17-B Confidence-Weighted Structural Supervision.

Evaluates R17-B across 5114-day window (671 basins, routed rows 0..5113 vs target[365:365+5114]):
  ...
  4. 6-Way Benchmark Comparison (Baseline, R8, R10-B, R15-A, R17-A, R17-B)
  5. Weight norm and selective separation analysis for w_int

Outputs saved to:
  results/intercept_r17b/E_S0_r17b/eval_summary.json
  results/intercept_r17b/E_S0_r17b/process_oracle_table_ep10.csv
  results/intercept_r17b/E_S0_r17b/benchmark_comparison.csv
  results/intercept_r17b/E_S0_r17b/epoch_trajectory.csv
"""
...
OUT_ROOT = Path("results/intercept_r17b/E_S0_r17b")
...
def main():
    ...
    c_cli = parse_args(["--config", cfg_path, "--gpu-id", "0",
                        "--output-root", "results/intercept_r17b",
                        "--run-name", "E_S0_r17b"])
    ...
    for proc in PROCESSES:
        ...
        print(f"\n[{proc}] Ep10 Oracle Evaluation (R17-B):")
    ...
    # Add R17-B
    r17b_ep10_traj = df_traj[df_traj["epoch"] == 10].iloc[0]
    for proc in PROCESSES:
        p_res = process_eval_summary[proc]
        bench_rows.append({
            "run": "R17-B (CF Dual-Opt Conf-Weighted)",
            "process": proc,
            "median_nse": r17b_ep10_traj["median_nse"],
            "mean_nse": r17b_ep10_traj["mean_nse"],
    ...
    full_summary = {
        "overall_evaluation": {
            "median_nse_ep10": float(r17b_ep10_traj["median_nse"]),
            "mean_nse_ep10": float(r17b_ep10_traj["mean_nse"]),
            "frac_nse_gt0": float(r17b_ep10_traj["frac_nse_gt0"]),
            "frac_nse_gt05": float(r17b_ep10_traj["frac_nse_gt05"]),
            "weights_head_w_norm_ep10": float(r17b_ep10_traj["weights_head_w_norm"]),
        },
