#!/usr/bin/env python3
"""Final Evaluation and 4-Process Benchmark for R15 Counterfactual Structural Supervision.

Evaluates R15-A across 5114-day window (671 basins, routed rows 0..5113 vs target[365:365+5114]):
  1. Full-period NSE and KGE metrics
  2. Four-process exact continuous Oracle grid sweep (w* in {0.0, 0.1, 0.25, 0.5, 0.75, 1.0})
  3. Precision, Recall, FPR, FNR, and Spearman correlation with Oracle w*
  4. Comparison against Baseline E-S0, R8, and R10-B
  5. Epoch-by-epoch gate and target trajectory (ep1..ep10)
  6. Representative basin case studies (strongly recovered vs missed)

Outputs saved to:
  results/intercept_cf_supervision/E_S0_cf_supervision/eval_summary.json
  results/intercept_cf_supervision/E_S0_cf_supervision/process_oracle_table_ep10.csv
  results/intercept_cf_supervision/E_S0_cf_supervision/benchmark_comparison.csv
  results/intercept_cf_supervision/E_S0_cf_supervision/epoch_trajectory.csv
"""
...
OUT_ROOT = Path("results/intercept_cf_supervision/E_S0_cf_supervision")
...
def main():
    ...
    for ep in range(1, 11):
        ...
        for proc in PROCESSES:
            ...
            t_row[f"{proc}_p95"] = float(np.percentile(pw, 95))
        ...
        print(f"  Ep {ep:>2d}: median_NSE={t_row['median_nse']:.4f}, mean_NSE={t_row['mean_nse']:.4f} | "
              f"w_int: mean={t_row['w_int_mean']:.4f}, frac>0.01={t_row['w_int_frac_gt001']*100:.1f}%, frac>0.1={t_row['w_int_frac_gt01']*100:.1f}%")

    df_traj = pd.DataFrame(traj_rows)
    df_traj.to_csv(OUT_ROOT / "epoch_trajectory.csv", index=False)

    # 2. Four-Process Exact Continuous Oracle Evaluation at ep10
    ...
    for proc in PROCESSES:
        ...
        print(f"\n[{proc}] Ep10 Oracle Evaluation:")
        print(f"  Oracle Pos: {n_orc_pos} ({process_eval_summary[proc]['frac_oracle_positive']*100:.1f}%) | "
              f"Learned Active (>0.01): {n_learned_pos} ({process_eval_summary[proc]['frac_learned_active_gt001']*100:.1f}%) | "
              f"Learned High (>0.1): {n_learned_high}")
        print(f"  Recall: {recall*100:.1f}% ({tp}/{n_orc_pos}) | Precision: {precision*100:.1f}% | High-Act Precision: {precision_high*100:.1f}%")
        print(f"  FPR: {fpr*100:.1f}% | FNR: {fnr*100:.1f}% | Spearman r: {sp_corr:+.4f}")
        print(f"  Learned w in Pos: {process_eval_summary[proc]['learned_w_oracle_pos_mean']:.4f} vs in Zero: {process_eval_summary[proc]['learned_w_oracle_zero_mean']:.4f}")
    ...
    # 3. Load Benchmarks for Baseline, R8, R10-B
    print("\n[3/4] Compiling 4-Way Benchmark Comparison (Baseline vs R8 vs R10-B vs R15-A)...")
    benchmarks = [
        ...
        if Path(fpath).exists():
            ...
            for proc in PROCESSES:
                ...
                bench_rows.append({
                    ...
                    "learned_w_median": float(np.median(wl[valid])),
    ...
    # Add R15-A
    r15_ep10_traj = df_traj[df_traj["epoch"] == 10].iloc[0]
    for proc in PROCESSES:
        p_res = process_eval_summary[proc]
        bench_rows.append({
            "run": "R15-A (CF Structural Supervision)",
            "process": proc,
            "median_nse": r15_ep10_traj["median_nse"],
            "mean_nse": r15_ep10_traj["mean_nse"],
            ...
            "learned_w_median": p_res["learned_w_median"],
    ...
    # 4. Representative Basin Case Studies for w_int
    print("\n[4/4] Extracting representative basin case studies for w_int...")
    wint_sub = df_orc[df_orc["process"] == "w_int"].copy()
    # Recovered positives (w_learn > 0.05 and w_star > 0)
    recovered = wint_sub[wint_sub["oracle_pos"] & (wint_sub["learned_w"] > 0.05)].sort_values("learned_w", ascending=False)
    # Missed positives (w_star > 0 and w_learn < 0.01)
    missed = wint_sub[wint_sub["oracle_pos"] & (wint_sub["learned_w"] < 0.01)].sort_values("fit_imp_opt", ascending=False)
    # False positives (w_star == 0 and w_learn > 0.05)
    false_pos = wint_sub[(~wint_sub["oracle_pos"]) & (wint_sub["learned_w"] > 0.05)].sort_values("learned_w", ascending=False)

    cases = {
        "recovered_positives": recovered.head(10).to_dict(orient="records"),
        "missed_positives": missed.head(10).to_dict(orient="records"),
        "false_positives": false_pos.head(10).to_dict(orient="records"),
    }

    full_summary = {
        "overall_evaluation": {
            "median_nse_ep10": float(r15_ep10_traj["median_nse"]),
            "mean_nse_ep10": float(r15_ep10_traj["mean_nse"]),
            "frac_nse_gt0": float(r15_ep10_traj["frac_nse_gt0"]),
            "frac_nse_gt05": float(r15_ep10_traj["frac_nse_gt05"]),
        },
        "processes": process_eval_summary,
        "case_studies_w_int": cases,
    }

    sum_json = OUT_ROOT / "eval_summary.json"
    sum_json.write_text(json.dumps(full_summary, indent=2))
