#!/usr/bin/env python3
"""Final Evaluation and 6-Way Benchmark for R18-Hybrid Dedicated Structure Encoder.

Evaluates R18-Hybrid across 5114-day window (671 basins, routed rows 0..5113 vs target[365:365+5114]):
  ...
  4. 6-Way Benchmark Comparison (Baseline, R10-B, R15-A, R17-A, R17-B, R18-Hybrid)
  5. Weight norm, variance, and selective separation analysis for w_int

Outputs saved to:
  results/intercept_r18a/E_S0_r18a/eval_summary.json
  results/intercept_r18a/E_S0_r18a/process_oracle_table_ep10.csv
  results/intercept_r18a/E_S0_r18a/benchmark_comparison.csv
  results/intercept_r18a/E_S0_r18a/epoch_trajectory.csv
"""
...
OUT_ROOT = Path("results/intercept_r18a/E_S0_r18a")
...
def main():
    ...
    for ep in range(1, 11):
        ...
        with torch.no_grad():
            ...
            # Structure encoder parameter norms
            struct_norms = [float(torch.norm(p).item()) for p in nn.structure_encoder.parameters()]
        ...
        t_row = {
            ...
            "struct_encoder_total_norm": float(np.sum(struct_norms)),
        }
    ...
    for proc in PROCESSES:
        ...
        print(f"\n[{proc}] Ep10 Oracle Evaluation (R18-Hybrid):")
    ...
    # 3. 6-Way Benchmark Comparison Table
    print("\n[3/4] Compiling 6-Way Benchmark Comparison...")
    benchmarks = [
        ...
        ("R17-A (CF Dual-Opt Unweighted)", "results/intercept_r17a/E_S0_r17a/process_oracle_table_ep10.csv", 0.6429, 0.5616),
        ("R17-B (CF Dual-Opt Conf-Weighted)", "results/intercept_r17b/E_S0_r17b/process_oracle_table_ep10.csv", 0.6421, 0.5624),
    ]
    ...
    # Add R18-Hybrid
    r18_ep10_traj = df_traj[df_traj["epoch"] == 10].iloc[0]
    for proc in PROCESSES:
        p_res = process_eval_summary[proc]
        bench_rows.append({
            "run": "R18-Hybrid (Dedicated Structure MLP)",
            "process": proc,
            "median_nse": r18_ep10_traj["median_nse"],
            "mean_nse": r18_ep10_traj["mean_nse"],
    ...
    full_summary = {
        "overall_evaluation": {
            "median_nse_ep10": float(r18_ep10_traj["median_nse"]),
            "mean_nse_ep10": float(r18_ep10_traj["mean_nse"]),
            "frac_nse_gt0": float(r18_ep10_traj["frac_nse_gt0"]),
            "frac_nse_gt05": float(r18_ep10_traj["frac_nse_gt05"]),
        },
