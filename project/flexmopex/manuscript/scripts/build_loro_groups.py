"""Build LORO groups by intersecting frozen region groups with a basin manifest."""
...
def load_manifest(path: Path) -> list[int]:
    if path.suffix == ".npy":
        ids = [int(x) for x in np.load(path, allow_pickle=True).tolist()]
    else:
        value = ast.literal_eval(path.read_text())
        ids = [int(x) for x in value]
    if len(ids) != len(set(ids)):
        raise ValueError(f"Basin manifest contains duplicate IDs: {path}")
    return ids
...
def main() -> None:
    ...
    for region in range(7):
        ...
        if not held_out:
            raise ValueError(f"group_{group_id} has no basin intersection")
