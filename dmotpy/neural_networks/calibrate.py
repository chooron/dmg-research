...
def softsin(x: torch.Tensor) -> torch.Tensor:
    """Smoothly squash values to [-1, 1] with a cheap closed-form transform."""
...
class Calibrate(torch.nn.Module):
    ...
    def forward(
        ...
        return None, self.transform_params(cur_params)

    @staticmethod
    def transform_params(params: torch.Tensor) -> torch.Tensor:
        return torch.sigmoid(params)


class Calibratev2(torch.nn.Module):
    def __init__(
        self,
        *,
        nx: int,
    ...
    def _initialize_params(self, strategy: str) -> nn.Parameter:
        ...
        print(
            "[Calibratev2] Initialized with per-basin Latin Hypercube Sampling "
            f"(LHS) + Logit With {self.num_start} Starts."
        )
        return nn.Parameter(init_val)
