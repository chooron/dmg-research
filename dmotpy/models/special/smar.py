...
from dmot.models.hydrology_model import HydrologyModel

# 引入通量计算函数
from dmot.models.flux.effective import effective_1
from dmot.models.flux.saturation import saturation_1, saturation_6
from dmot.models.flux.infiltration import infiltration_4
from dmot.models.flux.evap import evap_13, evap_14
from dmot.models.flux.split import split_1
from dmot.models.flux.baseflow import baseflow_1
from dmot.models.unithydro.base import DplUHBase
...
class Smar(HydrologyModel):
    ...
    def _init_states(self, n_grid: int, nmul: Optional[int] = None) -> Tuple[torch.Tensor, ...]:
        ...
        states = tuple(torch.zeros((n_grid, nmul), device=self.device) + self.nearzero for _ in range(6))
        return states
    ...
    def _run_model(
        ...
        with torch.no_grad():
            for t in range(warm_up):
                _, _, _, S1, S2, S3, S4, S5, S6 = self.production_step(
                    P_seq[t], PET_seq[t], S1, S2, S3, S4, S5, S6, h_runoff, y_inf, smax, c_evap, g_rech, kg, nearzero
                )
        ...
        for t in range(n_steps):
            flux_qr_in, flux_qg, flux_ea, S1, S2, S3, S4, S5, S6 = self.production_step(
                P_seq[t],
                PET_seq[t],
                S1,
                S2,
