...
from dmot.models.unithydro.uh_half_1 import DplHalf1
from dmot.models.unithydro.uh_full_2 import DplFull2
...
def _calc_percolation_analytical(S: torch.Tensor, x1: torch.Tensor, nearzero: float) -> torch.Tensor:
    ratio_perc = (4.0 / 9.0) * (S / (x1 + nearzero))
    term_perc = (1.0 + ratio_perc.pow(4)).pow(-0.25)
    return S * (1.0 - term_perc)


def _calc_routing_outflow_analytical(S2: torch.Tensor, x3: torch.Tensor, nearzero: float) -> torch.Tensor:
...
class Gr4j(HydrologyModel):
    ...
    def _run_model(
        ...
        for t in range(n_steps):
            flux_pr, e_phys, S1 = self.production_step(P_seq[t], PET_seq[t], S1, x1, nearzero)
        ...
        q9_seq = routed_q9_flat.view(n_grid, nmul, n_steps).permute(2, 0, 1).unbind(0)
        q1_seq = routed_q1_flat.view(n_grid, nmul, n_steps).permute(2, 0, 1).unbind(0)
        e_phys_seq = e_phys_stack.unbind(0)

        Qsim_list = []
        for t in range(n_steps):
            q_total, ea_balanced, S2 = self.routing_step(q9_seq[t], q1_seq[t], S2, x2, x3, e_phys_seq[t], nearzero)
